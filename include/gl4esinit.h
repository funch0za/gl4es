...
// set driver GetProcAddress implementation. required for hardext detection with
// NOEGL or when loader is disabled
void set_getprocaddress(void *(APIENTRY_GL4ES *new_proc_address)(const char *));
// reguired with NOEGL
void set_getmainfbsize(void(APIENTRY_GL4ES *new_getMainFBSize)(int *width,
                                                               int *height));
