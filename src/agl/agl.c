...
void agl_context_find(void *ctx) {
  ...
  if (!agl_context) {
    agl_context_cap = 10;
    agl_context =
        (agl_ctx_glstate *)malloc(sizeof(agl_ctx_glstate) * agl_context_cap);
    memset(agl_context, 0, sizeof(agl_ctx_glstate) * agl_context_cap);
  }
  int idx = 0;
  while (idx < agl_context_len && agl_context[idx].context != ctx)
    idx++;
  if (idx == agl_context_len) {
    agl_context = (agl_ctx_glstate *)realloc(
        agl_context, sizeof(agl_ctx_glstate) * (agl_context_cap + 10));
...
void agl_context_remove(void *ctx) {
  ...
  // shrink if possible
  while (agl_context_len && !agl_context[agl_context_len - 1].context)
    --agl_context_len;
  if (!agl_context_len) {
    agl_context_cap = 0;
    free(agl_context);
...
void aglMakeCurrent(void *context) {
  ...
  if (IOGLES2) {
    ...
    if (context)
      agl_context_find(
          context); // activate (and create if needed) the correct glstate
  }
}
