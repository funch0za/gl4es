...
int createPBuffer(Display *dpy, const EGLint *egl_attribs, EGLSurface *Surface,
                  EGLContext *Context, EGLConfig *Config, int redBits,
                  int greenBits, int blueBits, int alphaBits, int samplebuffers,
                  int samples);
GLXPbuffer addPixBuffer(Display *dpy, EGLSurface surface, EGLConfig Config,
                        int Width, int Height, EGLContext Context,
...
static int swapinterval =
    1; // default value. Also, should be tracked by drawable...
...
// Generic create native window to use with "LIBGL_FB=1" (so with
// EGL_DEFAULT_DISPLAY and without X11)
static void *create_native_window(int w, int h) {
#if !defined(ANDROID) && !defined(AMIGAOS4)
  if (bcm_host)
    return create_rpi_window(w, h);
#endif
#ifndef NO_GBM
  if (globals4es.usegbm)
    return CreateGBMWindow(w, h);
#endif
  return NULL;
}
...
#if !defined(ANDROID) && !defined(AMIGAOS4)
  if (bcm_host)
    return delete_rpi_window(win);
#endif
#ifndef NO_GBM
  if (globals4es.usegbm)
    return DeleteGBMWindow(win);
#endif
}

#ifndef NOEGL
static EGLint egl_context_attrib_es2[] = {EGL_CONTEXT_CLIENT_VERSION, 2,
                                          EGL_NONE};
...
static int get_config_default(Display *display, int attribute, int *value) {
  ...
  case GLX_VISUAL_ID: {
    ...
    int n;
    XVisualInfo *visuals =
        XGetVisualInfo(display, VisualDepthMask | VisualClassMask, &xvinfo, &n);
    if (!n) {
      LOGD("Warning, get_config_default: XGetVisualInfo gives 0 VisualInfo for "
           "%d depth and TrueColor class\n",
           glx_default_depth);
      *value = 0;
    } else {
...
static void fill1GLXFBConfig(Display *display, EGLConfig eglConfig, int DB,
                             GLXFBConfig fbConfig) {
  LOAD_EGL(eglGetConfigAttrib);

  EGLint tmp;
  egl_eglGetConfigAttrib(eglDisplay, eglConfig, EGL_RED_SIZE,
                         &fbConfig->redBits);
  egl_eglGetConfigAttrib(eglDisplay, eglConfig, EGL_GREEN_SIZE,
                         &fbConfig->greenBits);
  egl_eglGetConfigAttrib(eglDisplay, eglConfig, EGL_BLUE_SIZE,
                         &fbConfig->blueBits);
  egl_eglGetConfigAttrib(eglDisplay, eglConfig, EGL_ALPHA_SIZE,
                         &fbConfig->alphaBits);
  egl_eglGetConfigAttrib(eglDisplay, eglConfig, EGL_DEPTH_SIZE,
                         &fbConfig->depthBits);
  egl_eglGetConfigAttrib(eglDisplay, eglConfig, EGL_STENCIL_SIZE,
                         &fbConfig->stencilBits);
  egl_eglGetConfigAttrib(eglDisplay, eglConfig, EGL_SAMPLES,
                         &fbConfig->multiSampleSize);
  egl_eglGetConfigAttrib(eglDisplay, eglConfig, EGL_SAMPLE_BUFFERS,
                         &fbConfig->nMultiSampleBuffers);
  egl_eglGetConfigAttrib(eglDisplay, eglConfig, EGL_SURFACE_TYPE, &tmp);
  fbConfig->drawableType = 0;
  if (tmp & EGL_WINDOW_BIT)
    fbConfig->drawableType |= GLX_WINDOW_BIT;
  if (tmp & EGL_PBUFFER_BIT)
    fbConfig->drawableType |= GLX_PBUFFER_BIT;
  if (tmp & EGL_PIXMAP_BIT)
    fbConfig->drawableType |= GLX_PIXMAP_BIT;
  egl_eglGetConfigAttrib(eglDisplay, eglConfig, EGL_MAX_PBUFFER_WIDTH,
                         &fbConfig->maxPbufferWidth);
  egl_eglGetConfigAttrib(eglDisplay, eglConfig, EGL_MAX_PBUFFER_HEIGHT,
                         &fbConfig->maxPbufferHeight);
  egl_eglGetConfigAttrib(eglDisplay, eglConfig, EGL_MAX_PBUFFER_PIXELS,
                         &fbConfig->maxPbufferPixels);
  egl_eglGetConfigAttrib(eglDisplay, eglConfig, EGL_NATIVE_VISUAL_ID,
                         &fbConfig->associatedVisualId);
  if (!fbConfig->associatedVisualId || globals4es.usefb || globals4es.usefbo ||
      globals4es.usepbuffer) {
    // when using some FB driver, lets take a default VisualID, as the one from
    // the EGLConfig is probably not the correct one
    ...
    XVisualInfo *visuals =
        XGetVisualInfo(display, VisualDepthMask | VisualClassMask, &xvinfo, &n);
    if (!n) {
      LOGD("Warning, fillGLXFBConfig: XGetVisualInfo gives 0 VisualInfo for %d "
           "depth and TrueColor class\n",
           glx_default_depth);
...
static void init_eglconfig(Display *display) {
  ...
  EGLint configAttribs[] = {EGL_RENDERABLE_TYPE,
                            (hardext.esversion == 1) ? EGL_OPENGL_ES_BIT
                                                     : EGL_OPENGL_ES2_BIT,
                            EGL_NONE};
  int configsFound;
  ...
  egl_eglChooseConfig(eglDisplay, configAttribs, allConfigs, configsFound,
                      &configsFound);
  maxEGLConfig = -1;
  EGLint confID;
  ...
  ++maxEGLConfig;
  allFBConfig =
      (GLXFBConfig)calloc(maxEGLConfig * 2, sizeof(struct __GLXFBConfigRec));
  for (int i = 0; i < configsFound; ++i) {
    egl_eglGetConfigAttrib(eglDisplay, allConfigs[i], EGL_CONFIG_ID, &confID);
    fill1GLXFBConfig(display, allConfigs[i], 0, &allFBConfig[confID]);
    fill1GLXFBConfig(display, allConfigs[i], 1,
                     &allFBConfig[confID + maxEGLConfig]);
  }
}
...
static void xrefresh() { int dummy = system("xrefresh"); }
...
void pandora_set_gamma() {
  {
    char buf[50];
    if (globals4es.gamma > 0.0f)
      sprintf(buf, "sudo /usr/pandora/scripts/op_gamma.sh %.2f",
              globals4es.gamma);
    else
...
static void init_liveinfo() {
  ...
  if (sendto(sock, test_string, strlen(test_string), 0, (struct sockaddr *)&sun,
             sizeof(sun)) < 0) {
...
void glx_init() {
  ...
  if (globals4es.xrefresh || globals4es.stacktrace) {
    // TODO: a bit gross. Maybe look at this:
    // http://stackoverflow.com/a/13290134/293352
...
GLXContext gl4es_glXCreateContext(Display *display, XVisualInfo *visual,
                                  GLXContext shareList, Bool isDirect) {
  DBG(printf("glXCreateContext(%p, %p, %p, %i) fbcontext_count=%d ", display,
             visual, shareList, isDirect, fbcontext_count);)
  ...
  if (visualfbconfig)
    ...
    default_glxfbconfig.redBits = (visual == 0)           ? 0
                                  : (visual->depth == 16) ? 5
                                                          : 8;
    default_glxfbconfig.greenBits = (visual == 0)           ? 0
                                    : (visual->depth == 16) ? 6
                                                            : 8;
    default_glxfbconfig.blueBits = (visual == 0)           ? 0
                                   : (visual->depth == 16) ? 5
                                                           : 8;
    default_glxfbconfig.alphaBits = (visual == 0)           ? 0
                                    : (visual->depth != 32) ? 0
                                                            : 8;
#ifdef PANDORA
  ...
  if (depthBits == 32)
    depthBits =
        (glxfbconfig->stencilBits == 8 && hardext.esversion == 2) ? 24 : 16;
  if (depthBits == 24 && glxfbconfig->stencilBits == 8 &&
      !(globals4es.usefbo || globals4es.usepbuffer || hardext.esversion == 2))
    depthBits = 16;
  else if (depthBits == 16 && glxfbconfig->stencilBits == 8 &&
           hardext.esversion == 2)
    depthBits = 24;
#endif

  DBG(printf("Creating R:%d G:%d B:%d A:%d visual deth=%d Depth:%d Stencil:%d "
             "Multisample:%d/%d Doublebuff=%d\n",
             glxfbconfig->redBits, glxfbconfig->greenBits,
             glxfbconfig->blueBits, glxfbconfig->alphaBits,
             visual ? visual->depth : 0, depthBits, glxfbconfig->stencilBits,
             glxfbconfig->nMultiSampleBuffers, glxfbconfig->multiSampleSize,
             glxfbconfig->doubleBufferMode);)
  EGLint configAttribs[] = {
#ifdef PANDORA
      EGL_RED_SIZE, 5, EGL_GREEN_SIZE, 6, EGL_BLUE_SIZE, 5,
#else
      EGL_RED_SIZE, glxfbconfig->redBits, EGL_GREEN_SIZE,
      glxfbconfig->greenBits, EGL_BLUE_SIZE, glxfbconfig->blueBits,
      ...
      EGL_SAMPLE_BUFFERS, glxfbconfig->nMultiSampleBuffers, EGL_SAMPLES,
      glxfbconfig->multiSampleSize,

      EGL_SURFACE_TYPE,
      (globals4es.usegbm)
          ? EGL_WINDOW_BIT
          : (globals4es.usepbuffer ? EGL_PBUFFER_BIT
  ...
  result = egl_eglChooseConfig(eglDisplay, configAttribs, fake->eglConfigs, 64,
                               &fake->eglConfigsCount);
  if (fake->eglConfigsCount && globals4es.usegbm)
    fake->eglconfigIdx =
        FindGBMConfig(eglDisplay, fake->eglConfigs, fake->eglConfigsCount);

  CheckEGLErrors();
  if (result != EGL_TRUE || fake->eglConfigsCount == 0) {
    DBG(printf(" => %p\n", NULL);)
    LOGE("No EGL configs found (depth=%d, stencil=%d).\n", depthBits,
         glxfbconfig->stencilBits);
    CheckEGLErrors();
    free(fake);
    return 0;
  }
  EGLContext shared = (shareList) ? shareList->eglContext : EGL_NO_CONTEXT;
  fake->eglContext = egl_eglCreateContext(
      eglDisplay, fake->eglConfigs[fake->eglconfigIdx], shared,
...
GLXContext createPBufferContext(Display *display, GLXContext shareList,
                                GLXFBConfig config) {
  ...
  result = egl_eglChooseConfig(eglDisplay, configAttribs, pbufConfigs, 1,
                               &configsFound);
  ...
  fake->eglContext = egl_eglCreateContext(
      eglDisplay, fake->eglConfigs[0], shared,
      (hardext.esversion == 1) ? egl_context_attrib : egl_context_attrib_es2);
  ...
  egl_eglGetConfigAttrib(eglDisplay, fake->eglConfigs[0], EGL_RED_SIZE,
                         &fake->rbits);
  egl_eglGetConfigAttrib(eglDisplay, fake->eglConfigs[0], EGL_GREEN_SIZE,
                         &fake->gbits);
  egl_eglGetConfigAttrib(eglDisplay, fake->eglConfigs[0], EGL_BLUE_SIZE,
                         &fake->bbits);
  egl_eglGetConfigAttrib(eglDisplay, fake->eglConfigs[0], EGL_ALPHA_SIZE,
                         &fake->abits);
  egl_eglGetConfigAttrib(eglDisplay, fake->eglConfigs[0], EGL_DEPTH_SIZE,
                         &fake->depth);
  egl_eglGetConfigAttrib(eglDisplay, fake->eglConfigs[0], EGL_STENCIL_SIZE,
                         &fake->stencil);
  egl_eglGetConfigAttrib(eglDisplay, fake->eglConfigs[0], EGL_SAMPLES,
                         &fake->samples);
  egl_eglGetConfigAttrib(eglDisplay, fake->eglConfigs[0], EGL_SAMPLE_BUFFERS,
                         &fake->samplebuffers);
...
GLXContext gl4es_glXCreateContextAttribsARB(Display *display,
                                            GLXFBConfig config,
                                            GLXContext share_context,
                                            Bool direct,
                                            const int *attrib_list) {
  DBG(printf("glXCreateContextAttribsARB(%p, %p, %p, %d, %p) ", display, config,
             share_context, direct, attrib_list);
      if (config)
          printf("config is RGBA:%d%d%d%d, depth=%d, stencil=%d, "
                 "multisample=%d/%d doublebuff=%d, drawable=%d\n",
                 config->redBits, config->greenBits, config->blueBits,
                 config->alphaBits, config->depthBits, config->stencilBits,
                 config->multiSampleSize, config->nMultiSampleBuffers,
  ...
  if (config->drawableType == GLX_PBUFFER_BIT) {
    ...
    result = egl_eglChooseConfig(eglDisplay, configAttribs, fake->eglConfigs,
                                 64, &fake->eglConfigsCount);
    if (fake->eglConfigsCount && globals4es.usegbm)
      fake->eglconfigIdx =
          FindGBMConfig(eglDisplay, fake->eglConfigs, fake->eglConfigsCount);
    else
    ...
    EGLContext shared =
        (share_context) ? share_context->eglContext : EGL_NO_CONTEXT;
    fake->eglContext = egl_eglCreateContext(
        eglDisplay, fake->eglConfigs[fake->eglconfigIdx], shared,
        (hardext.esversion == 1) ? egl_context_attrib : egl_context_attrib_es2);
    ...
    fake->xid = 1; // TODO: Proper handling of that id...
    fake->contextType =
        (config->drawableType & GLX_WINDOW_BIT) ? 0 : 2; // Window:Pixmap
    fake->doublebuff = config->doubleBufferMode;

    egl_eglGetConfigAttrib(eglDisplay, fake->eglConfigs[fake->eglconfigIdx],
                           EGL_RED_SIZE, &fake->rbits);
    egl_eglGetConfigAttrib(eglDisplay, fake->eglConfigs[fake->eglconfigIdx],
                           EGL_GREEN_SIZE, &fake->gbits);
    egl_eglGetConfigAttrib(eglDisplay, fake->eglConfigs[fake->eglconfigIdx],
                           EGL_BLUE_SIZE, &fake->bbits);
    egl_eglGetConfigAttrib(eglDisplay, fake->eglConfigs[fake->eglconfigIdx],
                           EGL_ALPHA_SIZE, &fake->abits);
    egl_eglGetConfigAttrib(eglDisplay, fake->eglConfigs[fake->eglconfigIdx],
                           EGL_DEPTH_SIZE, &fake->depth);
    egl_eglGetConfigAttrib(eglDisplay, fake->eglConfigs[fake->eglconfigIdx],
                           EGL_STENCIL_SIZE, &fake->stencil);
    egl_eglGetConfigAttrib(eglDisplay, fake->eglConfigs[fake->eglconfigIdx],
                           EGL_SAMPLES, &fake->samples);
    egl_eglGetConfigAttrib(eglDisplay, fake->eglConfigs[fake->eglconfigIdx],
                           EGL_SAMPLE_BUFFERS, &fake->samplebuffers);
    egl_eglGetConfigAttrib(eglDisplay, fake->eglConfigs[fake->eglconfigIdx],
                           EGL_MIN_SWAP_INTERVAL, &minswap);
    egl_eglGetConfigAttrib(eglDisplay, fake->eglConfigs[fake->eglconfigIdx],
                           EGL_MAX_SWAP_INTERVAL, &maxswap);
    DBG(printf(" => return %p (eglContext=%p, context->shared=%p)\n", fake,
               fake->eglContext, fake->shared);)
    return fake;
  }
}
...
void gl4es_glXDestroyContext(Display *display, GLXContext ctx) {
  ...
  if (ctx != NULL && ctx->eglContext) {
    ...
    if (eglSurface != ctx->eglSurface || eglContext != ctx->eglContext) {
      DBG(printf("  temporary switch to eglSurface:%p(from %p), "
                 "eglContext:%p(from %p)\n",
                 ctx->eglSurface, eglSurface, ctx->eglContext, eglContext);)
#ifndef NO_GBM
      if (globals4es.usegbm)
        GBMMakeCurrent(eglDisplay, ctx->eglSurface, ctx->eglSurface,
                       ctx->eglContext);
      else
#endif
        egl_eglMakeCurrent(eglDisplay, ctx->eglSurface, ctx->eglSurface,
                           ctx->eglContext);
    }
...
#ifndef NO_GBM
      if (globals4es.usegbm)
        GBMMakeCurrent(eglDisplay, eglContext ? eglSurface : NULL,
                       eglContext ? eglSurface : NULL, eglContext);
      else
#endif
        egl_eglMakeCurrent(eglDisplay, eglContext ? eglSurface : NULL,
                           eglContext ? eglSurface : NULL, eglContext);
    }
    ...
    ctx->eglContext = 0;
    if (ctx->eglSurface != 0) {
      if (globals4es.usefb != 1 ||
          !fbcontext_count) { // (This may cause troble on Pandora, has some
                              // driver doesn't seems to like to many Creation
                              // of the surface)
        int destroySurf = 1;
        ...
        if (destroySurf) {
          if (!globals4es.glxrecycle) {
            DBG(printf("  egDestroySurface(%p, %p), drawable=%p\n", eglDisplay,
                       ctx->eglSurface, (void *)ctx->drawable);)
...
XVisualInfo *gl4es_glXChooseVisual(Display *display, int screen,
                                   int *attributes) {
  ...
  // create a new attribute list for glXChooseFBConfig based on the attributes
  // liste given...
  int attr[50];
  int idx = 0;
  int cur = 0;
  ...
  if (glx_default_depth != 16 && glx_default_depth != 24 &&
      glx_default_depth != 32)
...
*/  // this makes window of TokiTory transparent...
  ...
  XVisualInfo *visuals =
      XGetVisualInfo(display, VisualDepthMask | VisualClassMask, &xvinfo, &n);
  if (!n) {
    LOGD("Warning, gl4es_glXChooseVisual: XGetVisualInfo gives 0 VisualInfo "
         "for %d depth and TrueColor class\n",
         glx_default_depth);
    return NULL;
  }
...
Bool gl4es_glXMakeCurrent(Display *display, GLXDrawable drawable,
                          GLXContext context) {
#ifdef NOX11
  DBG(printf(
          "glXMakeCurrent(%p, %p, %p), context->drawable=%p, "
          "context->eglSurface=%p(%p), context->doublebuff=%d, glxContext=%p\n",
          display, (void *)drawable, context,
          (void *)(context ? context->drawable : 0),
          context ? context->eglSurface : 0, eglSurface,
          context ? context->doublebuff : 0, glxContext);)
#else
  DBG(printf("glXMakeCurrent(%p, %p, %p), isPBuffer(drawable)=%d, "
             "context->drawable=%p, context->eglSurface=%p(%p), "
             "context->doublebuff=%d, glxContext=%p\n",
             display, (void *)drawable, context, isPBuffer(drawable),
             (void *)(context ? context->drawable : 0),
             context ? context->eglSurface : 0, eglSurface,
             context ? context->doublebuff : 0, glxContext);)
#endif
  ...
  if (glxContext && glxContext->drawable == drawable &&
      glxContext->eglSurface && !context->eglSurface &&
      ((glxContext->shared == context->shared && glxContext->shared) ||
       glxContext == context->shared || context == glxContext->shared)) {

    context->eglSurface =
        glxContext->eglSurface; // lets hope eglContexts are compatible
    ...
    DBG(printf(
            "Same drawable and compatible context: sharing everything...\n");)
  }
  if (context && glxContext && context->drawable == drawable &&
    ...
    DBG(printf(" => True\n");)
    // same context, all is done bye (only one context per surface anyway in
    // EGL, iirc)
  ...
  if (context) {
    if (context->drawable == drawable && context->eglSurface) {
      // same-same, recycling...
      // eglSurf = context->eglSurface;    // no need, it's done after the
      // ifs...
    } else if (glxContext && glxContext->drawable == drawable &&
               glxContext->eglSurface && !context->eglSurface) {
      context->eglSurface =
          glxContext->eglSurface; // lets hope eglContexts are compatible
      if (!glxContext->shared_eglsurface)
      ...
      if (created) {
        ...
        eglSurf = context->eglSurface =
            pbuffersize[created - 1].Surface; //(EGLSurface)drawable;
        eglCtx = context->eglContext =
            pbuffersize[created - 1]
                .Context; // this context is ok for the PBuffer
        eglConfig = context->eglConfigs[context->eglconfigIdx] =
            pbuffersize[created - 1].Config;
        /*if (context->contextType != pbuffersize[created-1].Type) {    //
        Context / buffer not aligned, create a new glstate tracker
            if(context->glstate)
        ...
        if (globals4es.usepbuffer) {
          ...
          if (createPBuffer(display, egl_attribs, &eglSurf, &eglCtx, &eglConfig,
                            (depth > 16) ? 8 : 5,
                            (depth == 15)  ? 5
                            : (depth > 16) ? 8
                                           : 6,
                            (depth > 16) ? 8 : 5, (depth == 32) ? 8 : 0,
                            context->samplebuffers, context->samples) == 0) {
            // fail too, abort
          ...
          addPixBuffer(display, eglSurf, eglConfig, Width, Height, eglCtx,
                       drawable, depth, 2);
...
#endif
        {
          if (globals4es.usefb || globals4es.usefbo || globals4es.usegbm) {
            if (eglSurface) // cannot create multiple eglSurface for the same
                            // Framebuffer?
...
#endif
                eglSurf = egl_eglCreateWindowSurface(
                    eglDisplay, context->eglConfigs[context->eglconfigIdx],
            ...
            if (context->eglSurface) {
              int destroySurf = 1;
              if (context->shared_eglsurface &&
                  (--(*context->shared_eglsurface)) > 0)
                destroySurf = 0;
              if (destroySurf) {
                if (!globals4es.glxrecycle) {
            ...
            if (eglSurf == EGL_NO_SURFACE) {
              eglSurf = context->eglSurface = egl_eglCreateWindowSurface(
                  eglDisplay, context->eglConfigs[0], drawable, attrib_list);
            } else {
              DBG(printf("LIBGL: eglSurf Recycled\n");)
              context->eglSurface = eglSurf;
            }
  ...
  // update MapDrawable
  {
    ...
    if (k == kh_end(MapDrawable)) {
      ...
      map = kh_value(MapDrawable, k) =
          (map_drawable_t *)malloc(sizeof(map_drawable_t));
  ...
  if (context) {
    ...
    if (!context->glstate) {
      ...
      if (created && pbuffersize[created - 1].Type >= 3) {
        ...
        ((glstate_t *)context->glstate)->emulatedWin =
            pbuffersize[created - 1].Type == 4 ? 1 : 0;
      }
    }
...
Bool gl4es_glXMakeContextCurrent(Display *display, int drawable, int readable,
                                 GLXContext context) {
...
#define TARGET_FPS 1000000000LL / 61
  ...
  if (PBuffer && glstate->emulatedPixmap) {
    ...
    // CheckEGLErrors();     // not sure it's a good thing to call a
    // eglGetError() after all eglSwapBuffers, performance wize (plus result is
    // discarded anyway)
#ifdef PANDORA
    ...
    if (frame == 1) {
      ...
      if (last_frame < now) {
        ...
        if (sock > -1) {
          ...
          sendto(sock, tmp, strlen(tmp), 0, (struct sockaddr *)&sun,
                 sizeof(sun));
        }
#endif
      }
...
int gl4es_glXQueryContext(Display *dpy, GLXContext ctx, int attribute,
                          int *value) {
...
GLXFBConfig *gl4es_glXChooseFBConfig(Display *display, int screen,
  ...
  *count = 1;
  ...
  configs[0]->redBits = configs[0]->greenBits = configs[0]->blueBits =
      configs[0]->alphaBits = 0;
    ...
    while (attrib_list[i] != 0) {
      ...
      case GLX_X_VISUAL_TYPE:
        ...
        if (!(globals4es.usepbuffer || globals4es.usefb || globals4es.usefbo ||
              globals4es.glxnative)) {
          if (tmp != GLX_TRUE_COLOR) { // GLX_TRUE_COLOR is ok, don't add it to
                                       // the list
  ...
  attr[1] |= (globals4es.usepbuffer) ? (/*EGL_PBUFFER_BIT|*/ EGL_PIXMAP_BIT)
                                     : EGL_WINDOW_BIT;
  ...
  if (eglInitialized == false) {
    if (!InitEGL((globals4es.usefb || globals4es.usepbuffer) ? g_display
                                                             : display)) {
      CheckEGLErrors();
  ...
  if (glxconfig == -1) {
    ...
    if ((*count == 0) && (globals4es.usepbuffer)) {
      DBG(printf("glXChooseFBConfig found 0 config with PixMap, trying again "
                 "with PBuffer\n");)
    ...
    if ((*count == 0) && (!globals4es.usepbuffer) && ca && attr[ca + 1]) {
      DBG(printf("glXChooseFBConfig found 0 config with an Alpha channel, "
                 "trying without\n");)
      attr[ca] = 0;
      egl_eglChooseConfig(eglDisplay, attr, NULL, 0, count);
    }
    if ((*count == 0) && (!globals4es.usepbuffer) && ca && attr[ca + 1] &&
      ...
      DBG(printf("glXChooseFBConfig found 0 config without an Alpha channel, "
                 "but 8bits rgb, trying lowering bitness\n");)
    ...
    if ((*count == 0) && (!globals4es.usepbuffer) &&
        (attr[cr] > 5 || attr[cg] > 5 || attr[cb] > 5)) {
      DBG(printf("glXChooseFBConfig found 0 config with 8bits rgb, trying "
                 "lowering bitness\n");)
      attr[cr] = attr[cg] = attr[cb] = 5;
      egl_eglChooseConfig(eglDisplay, attr, NULL, 0, count);
    }
    ...
    /*if((*count==0) && (vt) && (attr[vt]!=-1)) {
        DBG(printf("glXChooseFBConfig found 0 config with VisualType, trying
  ...
  GLXFBConfig *configs =
      fillGLXFBConfig(eglConfigs, *count, doublebuffer, display);
  if (doublebuffer == 2)
    *count *= 2;
  free(eglConfigs);
...
#ifdef NO_EGL
  // this is wrong! The config table should be a static one built according to
  // EGL Config capabilities...
  *count = 1;
  // this is to only do 1 malloc instead of 1 for the array and one for the
  // element...
  GLXFBConfig *configs = (GLXFBConfig *)malloc(sizeof(GLXFBConfig) +
  ...
  int tmp;
  attr[cur++] = EGL_SURFACE_TYPE;
  attr[cur++] = (globals4es.usepbuffer) ? (EGL_PBUFFER_BIT | EGL_PIXMAP_BIT)
                                        : EGL_WINDOW_BIT;
  attr[cur++] = EGL_RENDERABLE_TYPE;
  attr[cur++] =
      (hardext.esversion == 1) ? EGL_OPENGL_ES_BIT : EGL_OPENGL_ES2_BIT;
...
int gl4es_glXGetFBConfigAttrib(Display *display, GLXFBConfig config,
                               int attribute, int *value) {
  DBG(printf("glXGetFBConfigAttrib(%p, %p, 0x%04X, %p)", display, config,
             attribute, value);)
  if (!config) {
    return get_config_default(display, attribute, value);
  }
...
XVisualInfo *gl4es_glXGetVisualFromFBConfig(Display *display,
                                            GLXFBConfig config) {
  ...
  /*if(config) {  // seems to upset Colin McRea Rally (Unity game)
      int d = config->redBits + config->greenBits + config->blueBits +
  config->alphaBits; if(d==16 || d==24 || d==32) depth = d;
  }*/
  XVisualInfo xvinfo = {0};
  xvinfo.depth = depth;
  xvinfo.class = TrueColor;
  ...
  XVisualInfo *visuals =
      XGetVisualInfo(display, VisualDepthMask | VisualClassMask, &xvinfo, &n);
  if (!n) {
    LOGD("Warning, gl4es_glXGetVisualFromFBConfig: XGetVisualInfo gives 0 "
         "VisualInfo for %d depth and TrueColor class\n",
         glx_default_depth);
    return NULL;
  }
  return visuals;
...
GLXContext gl4es_glXCreateNewContext(Display *display, GLXFBConfig config,
  ...
  if (config && (config->drawableType == GLX_PBUFFER_BIT)) {
    ...
    return gl4es_glXCreateContextAttribsARB(display, config, share_list,
                                            is_direct, NULL);
  ...
  if (glxContext) {
    if (globals4es.usepbuffer) {
      ...
      if (interval < minswap || interval > maxswap) {
        SHUT_LOGE(
            "Warning, Swap Interval %d is out of possible values %d, %d\n",
...
// misc stubs
void gl4es_glXCopyContext(Display *display, GLXContext src, GLXContext dst,
                          GLuint mask) {
...
Window gl4es_glXCreateWindow(Display *display, GLXFBConfig config, Window win,
                             int *attrib_list) {
  // should return GLXWindow
  DBG(printf("glXCreateWindow(%p, %p, %p, %p)\n", display, config, (void *)win,
             attrib_list);)
  return win;
}
...
void gl4es_glXUseXFont(Font font, int first, int count, int listBase) {
  ...
  if (0 /*globals4es.usefb*/) {
    ...
    win = glxContext->drawable; // TODO, check that drawable is a window and not
                                // a pixmap ?
  }
...
int gl4es_glXQueryDrawable(Display *dpy, GLXDrawable draw, int attribute,
  ...
  *value = 0;
  ...
  case GLX_MAX_SWAP_INTERVAL_EXT:
    *value = maxswap; // fake, should eglQuery the Config for
                      // EGL_MAX_SWAP_INTERVAL (and EGL_MIN_SWAP_INTERVAL)
...
GLXPbuffer addPBuffer(EGLSurface surface, int Width, int Height,
                      EGLContext Context, EGLConfig Config) {
  if (pbufferlist_cap <= pbufferlist_size) {
    pbufferlist_cap += 4;
    pbufferlist = (GLXPbuffer *)realloc(pbufferlist,
                                        sizeof(GLXPbuffer) * pbufferlist_cap);
    pbuffersize = (glx_buffSize *)realloc(pbuffersize, sizeof(glx_buffSize) *
                                                           pbufferlist_cap);
  }
...
int createPBuffer(Display *dpy, const EGLint *egl_attribs, EGLSurface *Surface,
  ...
  result =
      egl_eglChooseConfig(eglDisplay, configAttribs, Config, 1, &configsFound);
  ...
  (*Context) = egl_eglCreateContext(
      eglDisplay, Config[0], EGL_NO_CONTEXT,
      (hardext.esversion == 1) ? egl_context_attrib : egl_context_attrib_es2);
  CheckEGLErrors();
...
GLXPbuffer gl4es_glXCreatePbuffer(Display *dpy, GLXFBConfig config,
                                  const int *attrib_list) {
  ...
  if (createPBuffer(dpy, egl_attribs, &Surface, &Context, Config,
                    config->redBits, config->greenBits, config->blueBits,
                    config->alphaBits, config->nMultiSampleBuffers,
                    config->multiSampleSize) == 0) {
    return 0;
  }
...
GLXPbuffer addPixBuffer(Display *dpy, EGLSurface surface, EGLConfig Config,
  ...
  pbuffersize[pbufferlist_size].gc =
      (emulated) ? XCreateGC(dpy, pixmap, 0, NULL) : NULL;
  pbuffersize[pbufferlist_size].frame = NULL;

  pbuffersize[pbufferlist_size].Type =
      2 + emulated; // 2 = pixmap, 3 = emulated pixmap, 4 = emulated win
  return pbufferlist[pbufferlist_size++];
}
...
int createPixBuffer(Display *dpy, int bpp, const EGLint *egl_attribs,
                    NativePixmapType nativepixmap, EGLSurface *Surface,
                    EGLContext *Context) {
  LOAD_EGL(eglChooseConfig);
  ...
  if (eglInitialized == false) {
    result =
        InitEGL((globals4es.usefb || globals4es.usepbuffer) ? g_display : dpy);
    if (!result) {
      CheckEGLErrors();
      LOGE("Unable to initialize EGL display.\n");
      return 0;
  ...
  result = egl_eglChooseConfig(eglDisplay, configAttribs, pixbufConfigs, 1,
                               &configsFound);
  ...
  (*Surface) = egl_eglCreatePixmapSurface(eglDisplay, pixbufConfigs[0],
                                          nativepixmap, egl_attribs);
  ...
  (*Context) = egl_eglCreateContext(
      eglDisplay, pixbufConfigs[0], EGL_NO_CONTEXT,
      (hardext.esversion == 1) ? egl_context_attrib : egl_context_attrib_es2);
  CheckEGLErrors();
...
GLXPixmap gl4es_glXCreateGLXPixmap(Display *display, XVisualInfo *visual,
                                   Pixmap pixmap) {
  ...
  int x, y;
  int emulated = 0;
  XGetGeometry(display, pixmap, &root, &x, &y, &width, &height, &border,
               &depth);
  // let's try to create a PixmapSurface directly
  if (globals4es.usefb ||
      createPixBuffer(display, depth, NULL, (NativePixmapType)pixmap, &Surface,
                      &Context) == 0) {
    ...
    if (createPBuffer(display, egl_attribs, &Surface, &Context, Config,
                      (depth > 16) ? 8 : 5,
                      (depth == 15)  ? 5
                      : (depth > 16) ? 8
                                     : 6,
  ...
  return addPixBuffer(display, Surface, Config[0], Width, Height, Context,
                      pixmap, depth, emulated);
}

GLXPixmap gl4es_glXCreatePixmap(Display *dpy, GLXFBConfig config, Pixmap pixmap,
                                const int *attrib_list) {
  DBG(printf("glXCreatePixmap(%p, %p, %p, %p)\n", dpy, config, (void *)pixmap,
             attrib_list);)
  // Check that the config is for PBuffer
  if (!(config->drawableType & GLX_PIXMAP_BIT))
    return 0;
...
void actualBlit(int reverse, int Width, int Height, int Depth, Display *dpy,
                Pixmap drawable, GC gc, XImage *frame, uintptr_t pix,
                void *tmp) {
  ...
  if (tmp) {
    if (reverse) {
      ...
      pixel_convert(tmp, (void **)&pix, Width, Height, GL_BGRA,
                    GL_UNSIGNED_BYTE, GL_RGB, GL_UNSIGNED_SHORT_5_6_5, 0,
...
void BlitEmulatedPixmap(int win) {
  ...
  if (reverse) {
    ...
    if (width != Width || height != Height /*|| depth!=Depth*/) {
      ...
      Surface =
          egl_eglCreatePbufferSurface(eglDisplay, buff->Config, egl_attribs);
      if (Surface == EGL_NO_SURFACE)
        LOGE("Warning, Recration of pbuffer failed (from %dx%dx%d => "
             "%dx%dx%d)\n",
...
#endif
    frame = buff->frame =
        XCreateImage(dpy, NULL /*visual*/, Depth, ZPixmap, 0, malloc(sz), Width,
                     Height, (Depth == 16) ? 16 : 32, 0);
  }
  ...
  if (direct_copy) {
    ...
    if (!egl_eglCopyBuffers(eglDisplay, buff->Surface,
                            (EGLNativePixmapType)frame)) {
  ...
  if (hardext.esversion == 1) {
    if (Depth == 16) {
      ...
      gles_glReadPixels(0, 0, Width, Height, GL_BGRA, GL_UNSIGNED_BYTE,
                        (void *)pix);
    }
  } else if (Depth == 16)
    gles_glReadPixels(0, 0, Width, Height, GL_RGB, GL_UNSIGNED_SHORT_5_6_5,
                      (void *)pix);
  else
#endif
    gl4es_glReadPixels(0, 0, Width, Height, (Depth == 16) ? GL_RGB : GL_BGRA,
                       (Depth == 16) ? GL_UNSIGNED_SHORT_5_6_5
                                     : GL_UNSIGNED_BYTE,
                       (void *)pix);
...
GLXContext gl4es_glXCreateContextAttribs(Display *dpy, GLXFBConfig config,
  ...
  if (majver * 10 + minver != 0) {
    ...
    if (majver * 10 + minver > globals4es.gl) {
      LOGE("Asked for unsupported context version %d.%d (max version is "
           "%d.%d)\n",
  ...
  if (mask & GLX_CONTEXT_ES2_PROFILE_BIT_EXT)
    globales2 = 1;
  GLXContext context = gl4es_glXCreateNewContext(dpy, config, GLX_RGBA_TYPE,
...
void refreshMainFBO() {
  ...
  if (eglSurface != EGL_NO_SURFACE) {
    LOAD_EGL(eglQuerySurface);
    EGLint tmp;
    if (egl_eglQuerySurface(eglDisplay, eglSurface, EGL_WIDTH, &tmp) ==
        EGL_TRUE)
      glstate->fbo.mainfbo_width = tmp;
    if (egl_eglQuerySurface(eglDisplay, eglSurface, EGL_HEIGHT, &tmp) ==
        EGL_TRUE)
...
const char *gl4es_glXQueryExtensionsString(Display *display, int screen) {
  ...
  static const char *basic_extensions = "GLX_ARB_create_context "
  ...
  static const char *es2_profile = "GLX_EXT_create_context_es2_profile ";
...
AliasExport(GLXContext, glXCreateContext, ,
            (Display * display, XVisualInfo *visual, GLXContext shareList,
             Bool isDirect));
AliasExport(GLXContext, glXCreateContextAttribs, ARB,
            (Display * display, GLXFBConfig config, GLXContext share_context,
...
AliasExport(XVisualInfo *, glXChooseVisual, ,
            (Display * display, int screen, int *attributes));
AliasExport(Bool, glXMakeCurrent, ,
            (Display * display, GLXDrawable drawable, GLXContext context));
AliasExport(Bool, glXMakeContextCurrent, ,
            (Display * display, int drawable, int readable,
             GLXContext context));
AliasExport(void, glXSwapBuffers, , (Display * display, GLXDrawable drawable));
AliasExport(int, glXGetConfig, ,
            (Display * display, XVisualInfo *visual, int attribute,
             int *value));
AliasExport(int, glXQueryContext, ,
            (Display * dpy, GLXContext ctx, int attribute, int *value));
AliasExport(GLXContext, glXGetCurrentContext, , ());
AliasExport(GLXFBConfig *, glXChooseFBConfig, ,
            (Display * display, int screen, const int *attrib_list,
             int *count));
AliasExport(GLXFBConfig *, glXChooseFBConfig, SGIX,
            (Display * display, int screen, const int *attrib_list,
             int *count));
AliasExport(GLXFBConfig *, glXGetFBConfigs, ,
            (Display * display, int screen, int *count));
AliasExport(int, glXGetFBConfigAttrib, ,
            (Display * display, GLXFBConfig config, int attribute, int *value));
AliasExport(XVisualInfo *, glXGetVisualFromFBConfig, ,
...
AliasExport(GLXContext, glXCreateNewContext, ,
            (Display * display, GLXFBConfig config, int render_type,
             GLXContext share_list, Bool is_direct));
AliasExport(void, glXSwapIntervalEXT, ,
            (Display * display, int drawable, int interval));
AliasExport(void, glXCopyContext, ,
            (Display * display, GLXContext src, GLXContext dst, GLuint mask));
AliasExport(Window, glXCreateWindow, ,
            (Display * display, GLXFBConfig config, Window win,
             int *attrib_list));
...
AliasExport(void, glXUseXFont, ,
            (Font font, int first, int count, int listBase));
AliasExport(int, glXQueryDrawable, ,
            (Display * dpy, GLXDrawable draw, int attribute,
             unsigned int *value));
AliasExport(void, glXDestroyPbuffer, , (Display * dpy, GLXPbuffer pbuf));
AliasExport(GLXPbuffer, glXCreatePbuffer, ,
            (Display * dpy, GLXFBConfig config, const int *attrib_list));
AliasExport(GLXPixmap, glXCreateGLXPixmap, ,
            (Display * display, XVisualInfo *visual, Pixmap pixmap));
AliasExport(GLXPixmap, glXCreatePixmap, ,
...
AliasExport(const char *, glXQueryExtensionsString, ,
            (Display * display, int screen));
AliasExport(const char *, glXQueryServerString, ,
            (Display * display, int screen, int name));
AliasExport(Bool, glXQueryExtension, ,
            (Display * display, int *errorBase, int *eventBase));
AliasExport(Bool, glXQueryVersion, ,
