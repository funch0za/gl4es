...
typedef struct _hardext {
  ...
  int maxcolorattach; // GL_MAX_COLOR_ATTACHMENTS_EXT or 1 if
                      // GL_EXT_draw_buffers not supported
  int maxdrawbuffers; // GL_MAX_DRAW_BUFFERS_EXT or 1 if GL_EXT_draw_buffers not
                      // supported
  ...
  int cubelod;        // textureCubeLod doesn't need "EXT" even with
  ...
  int eglnoalpha; // EGL surface doesn't seems to have any alpha channel (auto
                  // detect)
