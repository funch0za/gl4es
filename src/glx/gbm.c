...
static int init_drm_and_gbm() {
  ...
  gbmsurf = gbmdrm_gbm_surface_create(
      gbmdev, drm_mode->hdisplay, drm_mode->vdisplay, GBM_FORMAT_XRGB8888,
...
static struct drm_fb *drm_fb_get_from_bo(struct gbm_bo *bo) {
  ...
  ret = gbmdrm_drmModeAddFB(drm_fd, width, height, 24, 32, stride, handle,
                            &fb->fb_id);
  if (ret) {
...
int FindGBMConfig(EGLDisplay eglDisp, EGLConfig *configs, int numFounds) {
  ...
  while (idx < numFounds) {
    EGLint gbm_format;
    if (egl_eglGetConfigAttrib(eglDisp, configs[idx], EGL_NATIVE_VISUAL_ID,
                               &gbm_format)) {
...
void *OpenGBMDisplay(void *display) { return NULL; }

void *CreateGBMWindow(int w, int h) { return NULL; }
