...
void Streaming_Initialize() {
  ...
  glTexBindStreamIMG =
      (PFNGLTEXBINDSTREAMIMGPROC *)egl_eglGetProcAddress("glTexBindStreamIMG");
  glGetTexAttrIMG =
      (PFNGLGETTEXSTREAMDEVICEATTRIBUTEIVIMGPROC *)egl_eglGetProcAddress(
          "glGetTexStreamDeviceAttributeivIMG");
  glGetTexDeviceIMG =
      (PFNGLGETTEXSTREAMDEVICENAMEIMGPROC *)egl_eglGetProcAddress(
...
int alloc_buff(int buff, int width, int height) {
  ...
  buf_param.fourcc = BC_PIX_FMT_RGB565; // only RGB565 here (other choices are
                                        // only some YUV formats)
  ...
  if (!bcdev) {
    ...
    printf("LIBGL: Streaming device = %s num: %d, width: %d, height: %d, "
           "format: 0x%x\n",
  ...
  /*	LOAD_GLES(glTexParameterf);
      gles_glTexParameterf(GL_TEXTURE_STREAM_IMG, GL_TEXTURE_MIN_FILTER,
     GL_LINEAR); gles_glTexParameterf(GL_TEXTURE_STREAM_IMG,
  ...
  if (ioctl(bc_cat[buff], BCIOGET_BUFFERPHYADDR, &ioctl_var) != 0) {
    ...
    buf_vaddr[buff] =
        (char *)mmap(NULL, width * height * 2, PROT_READ | PROT_WRITE,
                     MAP_SHARED, bc_cat[buff], buf_paddr[buff]);
...
// "ID". Return the ID or -1 if failed.
...
void ApplyFilterID(int ID, GLenum min_filter, GLenum mag_filter) {
  ...
  if (!stream_cache[ID].active)
    return;
  gl4es_glTexParameterf(GL_TEXTURE_STREAM_IMG, GL_TEXTURE_MIN_FILTER,
                        min_filter);
  gl4es_glTexParameterf(GL_TEXTURE_STREAM_IMG, GL_TEXTURE_MAG_FILTER,
                        mag_filter);
}
