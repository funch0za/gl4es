...
void GetHardwareExtensions(int notest) {
...
#ifdef PANDORA
      // on the Pandora, there don't seem to exist a 8888 PBuffer config for
      // GLES2.
      EGL_RED_SIZE,
  ...
  if (egl_eglInitialize(eglDisplay, NULL, NULL) != EGL_TRUE) {
    LOGE("Error while gathering supported extension (eglInitialize: %s), "
         "default to none\n",
         PrintEGLError(0));
  ...
  if (!configsFound) {
    ...
    egl_eglChooseConfig(eglDisplay, configAttribs, pbufConfigs, 1,
                        &configsFound);
    if (configsFound) {
      SHUT_LOGD("No alpha channel in PBuffer context, disabling EGL Alpha "
                "channel...\n");
...
#endif
  if (!configsFound) {
    SHUT_LOGE("Error while gathering supported extension (eglChooseConfig: "
              "%s), default to none\n",
              PrintEGLError(0));
    egl_eglTerminate(eglDisplay);
    return;
  ...
  eglContext = egl_eglCreateContext(
      eglDisplay, pbufConfigs[0], EGL_NO_CONTEXT,
      (hardext.esversion == 1) ? egl_context_attrib : egl_context_attrib_es2);
  if (!eglContext) {
    SHUT_LOGE("Error while gathering supported extension (eglCreateContext: "
              "%s), default to none\n",
              PrintEGLError(0));
    return;
  }
  eglSurface =
      egl_eglCreatePbufferSurface(eglDisplay, pbufConfigs[0], egl_attribs);
  if (!eglSurface) {
    SHUT_LOGE("Error while gathering supported extension "
              "(eglCreatePBufferSurface: %s), default to none\n",
              PrintEGLError(0));
...
#define S(A, B, C)                                                             \
  ...
  if (strstr(Exts, "GL_IMG_texture_npot "))
    hardext.npot = 1; // it should enable mipmap (so hardext.npot=2), but mipmap
                      // (so level > 0) needs to be POT-sized?!!
  ...
  if (hardext.esversion > 1) {
    if (!globals4es.nohighp) {
      ...
      if (!hardext.highp) {
        ...
        if (gles_glGetShaderPrecisionFormat) {
          GLint range[2] = {0};
          GLint precision = 0;
          gles_glGetShaderPrecisionFormat(GL_FRAGMENT_SHADER, GL_HIGH_FLOAT,
                                          range, &precision);
    ...
    if (hardext.shaderlod) {
      // test is textureCubeLod need EXT or not (seems to be a bug in some PVR
      // driver)
  ...
  if (hardext.maxtex > MAX_TEX)
    hardext.maxtex = MAX_TEX; // caping, as there are some fixed-sized array...
  ...
  if (hardext.maxplanes > MAX_CLIP_PLANES)
    hardext.maxplanes = MAX_CLIP_PLANES; // caping planes, even 6 should be the
                                         // max supported anyway
  SHUT_LOGD(
...
#ifndef NOEGL
  if (strstr(egl_eglQueryString(eglDisplay, EGL_EXTENSIONS),
