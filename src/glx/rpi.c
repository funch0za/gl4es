...
(DISPMANX_UPDATE_HANDLE_T, DISPMANX_DISPLAY_HANDLE_T, int32_t, VC_RECT_T *,
...
#define GO(A)                                                                  \
  ...
  if (A == NULL)                                                               \
    A = dlsym(vcos, #A);                                                       \
  if (A == NULL)                                                               \
  printf("LIBGL: Warning, " #A " is null")
...
void *create_rpi_window(int w, int h) {
  EGL_DISPMANX_WINDOW_T *nativewindow =
      (EGL_DISPMANX_WINDOW_T *)calloc(1, sizeof(EGL_DISPMANX_WINDOW_T));
  if (!bcm_host)
    return NULL;
  ...
  VC_DISPMANX_ALPHA_T alpha = {/*DISPMANX_FLAGS_ALPHA_FIXED_ALL_PIXELS*/ 1, 255,
                               0};
