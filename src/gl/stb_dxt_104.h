...
//
// version history:
//   v1.04  - (ryg) default to no rounding bias for lerped colors (as per
//   S3TC/DX10 spec);
...
#define STB_DXT_DITHER                                                         \
  1 // use dithering. dubious win. never use for normal maps and the like!
...
static unsigned short stb__As16Bit(int r, int g, int b) {
  return (stb__Mul8Bit(r, 31) << 11) + (stb__Mul8Bit(g, 63) << 5) +
         stb__Mul8Bit(b, 31);
}
...
// lerp RGB color
static void stb__Lerp13RGB(unsigned char *out, unsigned char *p1,
                           unsigned char *p2) {
...
static void stb__PrepareOptTable(unsigned char *Table,
  ...
  for (i = 0; i < 256; i++) {
    ...
    for (mn = 0; mn < size; mn++) {
      for (mx = 0; mx < size; mx++) {
        ...
        // distribution of
        // +-1.5% error, but nowhere in the spec does it say that the error has
        // to be unbiased - better safe than sorry).
...
static void stb__EvalColors(unsigned char *color, unsigned short c0,
                            unsigned short c1) {
...
static void stb__DitherBlock(unsigned char *dest, unsigned char *block) {
  ...
  for (ch = 0; ch < 3; ++ch) {
    ...
    for (y = 0; y < 4; ++y) {
      ...
      ep1[0] = bp[0] - dp[0];
      dp[4] =
          quant[bp[4] + ((7 * ep1[0] + 3 * ep2[2] + 5 * ep2[1] + ep2[0]) >> 4)];
      ep1[1] = bp[4] - dp[4];
      dp[8] =
          quant[bp[8] + ((7 * ep1[1] + 3 * ep2[3] + 5 * ep2[2] + ep2[1]) >> 4)];
      ep1[2] = bp[8] - dp[8];
...
static unsigned int stb__MatchColorsBlock(unsigned char *block,
                                          unsigned char *color, int dither) {
  unsigned int mask = 0;
...
static void stb__OptimizeColorsBlock(unsigned char *block,
...
static int stb__RefineBlock(unsigned char *block, unsigned short *pmax16,
...
static void stb__CompressColorBlock(unsigned char *dest, unsigned char *block,
                                    int mode) {
  unsigned int mask;
  int i;
  int dither;
...
static void stb__CompressAlphaBlock(unsigned char *dest, unsigned char *src,
                                    int mode) {
