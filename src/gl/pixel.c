...
static inline bool remap_pixel(const GLvoid *src, GLvoid *dst,
                               const colorlayout_t *src_color, GLenum src_type,
                               const colorlayout_t *dst_color,
                               GLenum dst_type) {
...
#define write_each(amod, vmod, hmod)                                           \
  ...
  if (src_color->green > max_a)
    max_a = src_color->green;
  if (src_color->blue > max_a)
    max_a = src_color->blue;
  if (src_color->alpha > max_a)
    max_a = src_color->alpha;
  switch (src_type) {
    type_case(GL_DOUBLE, GLdouble, read_each(, , )) type_case(GL_FLOAT, GLfloat,
                                                              read_each(, , ))
        ...
        : type_case(
              ...
              read_each(, / 255.0f, )) type_case(GL_UNSIGNED_SHORT, GLubyte,
                                                 read_each(, / 65535.0f, ))
                          ...
                          type_case(GL_UNSIGNED_SHORT_4_4_4_4_REV, GLushort,
                              ...
                              type_case(GL_UNSIGNED_SHORT_5_6_5, GLushort,
                                  ...
                                  type_case(GL_UNSIGNED_SHORT_5_5_5_1, GLushort,
                                            ...
                                            read_each(, / 31.0f, );)
                                      type_case(GL_UNSIGNED_SHORT_4_4_4_4,
                                                GLushort,
                                                s =
                                                    (const GLushort[]){
                                                        ((v >> 12) & 0x0f),
  ...
  if (dst_color->green > max_a)
    max_a = dst_color->green;
  if (dst_color->blue > max_a)
    max_a = dst_color->blue;
  if (dst_color->alpha > max_a)
    max_a = dst_color->alpha;
  if ((dst_color->red == dst_color->green) &&
      (dst_color->red == dst_color->blue)) {
    // special case
    GLfloat aa = (pixel.r + pixel.g + pixel.b) /
                 3.0f; //*TODO* find a better formula. real luminance is not
                       //just the mean value.
    pixel.r = pixel.g = pixel.b = aa;
  }
  switch (dst_type) {
    type_case(GL_FLOAT, GLfloat, write_each(, , )) type_case(
        GL_HALF_FLOAT_OES, halffloat_t, write_each(, , float_f2h))
        type_case(GL_BYTE, GLbyte, write_each(, *127.0f, )) type_case(
            GL_UNSIGNED_BYTE, GLubyte, write_each(, *255.0, ))
            type_case(GL_UNSIGNED_SHORT, GLushort, write_each(, *65535.0f, ))
                type_case(GL_UNSIGNED_INT, GLuint,
                          write_each(, *(float)0xffffffff, ))
                    type_case(GL_INT8_REV, GLubyte, write_each(, *255.0, ))
                        type_case(GL_INT8, GLubyte,
                                  write_each(max_a -, *255.0, ))
        // TODO: force 565 to RGB? then we can change [4] -> 3
        type_case(GL_UNSIGNED_SHORT_5_6_5, GLushort, GLfloat color[4];
            ...
            type_case(GL_UNSIGNED_SHORT_5_5_5_1, GLushort, GLfloat color[4];
                      color[dst_color->red] = pixel.r;
                      color[dst_color->green] = pixel.g;
                      color[dst_color->blue] = pixel.b;
                      color[dst_color->alpha] = pixel.a;
                          // TODO: can I macro this or something? it follows a
                          // pretty strict form.
                ...
                type_case(GL_UNSIGNED_SHORT_1_5_5_5_REV, GLushort,
                    ...
                    type_case(
                        GL_UNSIGNED_SHORT_4_4_4_4, GLushort, GLfloat color[4];
                        color[dst_color->red] = pixel.r;
                        color[dst_color->green] = pixel.g;
                        color[dst_color->blue] = pixel.b;
                        color[dst_color->alpha] = pixel.a;
...
static inline bool transform_pixel(const GLvoid *src, GLvoid *dst,
                                   const colorlayout_t *src_color,
                                   GLenum src_type, const GLfloat *scale,
                                   const GLfloat *bias) {
  ...
  switch (src_type) {
    type_case(GL_DOUBLE, GLdouble, read_each(, , ))
        type_case(GL_FLOAT, GLfloat, read_each(, , ))
            type_case(GL_HALF_FLOAT_OES, halffloat_t,
                      read_each(, , float_h2f)) case GL_INT8_REV
...
static inline bool half_pixel(const GLvoid *src0, const GLvoid *src1,
                              const GLvoid *src2, const GLvoid *src3,
                              GLvoid *dst, const colorlayout_t *src_color,
                              GLenum src_type) {
...
static inline bool quarter_pixel(const GLvoid *src[16], GLvoid *dst,
                                 const colorlayout_t *src_color,
                                 GLenum src_type) {
...
#define write_each(amod, vmod, hmod)                                           \
  ...
  pixel.r = (pix[0].r + pix[1].r + pix[2].r + pix[3].r + pix[4].r + pix[5].r +
             pix[6].r + pix[7].r + pix[8].r + pix[9].r + pix[10].r + pix[11].r +
             pix[12].r + pix[13].r + pix[14].r + pix[15].r) *
            0.0625f;
  pixel.g = (pix[0].g + pix[1].g + pix[2].g + pix[3].g + pix[4].g + pix[5].g +
             pix[6].g + pix[7].g + pix[8].g + pix[9].g + pix[10].g + pix[11].g +
             pix[12].g + pix[13].g + pix[14].g + pix[15].g) *
            0.0625f;
  pixel.b = (pix[0].b + pix[1].b + pix[2].b + pix[3].b + pix[4].b + pix[5].b +
             pix[6].b + pix[7].b + pix[8].b + pix[9].b + pix[10].b + pix[11].b +
             pix[12].b + pix[13].b + pix[14].b + pix[15].b) *
            0.0625f;
  pixel.a = (pix[0].a + pix[1].a + pix[2].a + pix[3].a + pix[4].a + pix[5].a +
             pix[6].a + pix[7].a + pix[8].a + pix[9].a + pix[10].a + pix[11].a +
             pix[12].a + pix[13].a + pix[14].a + pix[15].a) *
            0.0625f;
...
bool pixel_convert(const GLvoid *src, GLvoid **dst, GLuint width, GLuint height,
  ...
  if (src_type == GL_HALF_FLOAT)
    src_type = GL_HALF_FLOAT_OES;
  if (dst_type == GL_HALF_FLOAT)
    dst_type = GL_HALF_FLOAT_OES;
  ...
  if (!remap_pixel((const GLvoid *)src_pos, (GLvoid *)dst_pos, src_color,
                   src_type, dst_color, dst_type)) {
  ...
  if (src_format == GL_COLOR_INDEX) {
    ...
    for (int i = 0; i < height; i++) {
      for (int j = 0; j < width; j++) {
        ...
        if (glstate->raster.map_i2i_size - 1)
          idx =
              glstate->raster.map_i2i[idx & (glstate->raster.map_i2i_size - 1)];
        tmp[0] =
            glstate->raster.map_i2r[idx & (glstate->raster.map_i2r_size - 1)];
        tmp[1] =
            glstate->raster.map_i2g[idx & (glstate->raster.map_i2g_size - 1)];
        tmp[2] =
            glstate->raster.map_i2b[idx & (glstate->raster.map_i2b_size - 1)];
        tmp[3] =
            glstate->raster.map_i2a[idx & (glstate->raster.map_i2a_size - 1)];
        remap_pixel((const GLvoid *)tmp, (GLvoid *)dst_pos, src_color, GL_FLOAT,
                    dst_color, dst_type);
        src_pos += src_stride;
        dst_pos += dst_stride;
      }
    ...
    for (int i = 0; i < height; i++) {
      for (int j = 0; j < width; j++) {
        remap_pixel((const GLvoid *)src_pos, (GLvoid *)dst_pos, src_color,
                    src_type, dst_color, dst_type);
        src_pos += src_stride;
        dst_pos += dst_stride;
      }
...
bool pixel_transform(const GLvoid *src, GLvoid **dst, GLuint width,
                     GLuint height, GLenum src_format, GLenum src_type,
  ...
  if (!transform_pixel((const GLvoid *)src_pos, (GLvoid *)dst_pos, src_color,
                       src_type, scales, bias)) {
    // fake convert, to get if it's ok or not
    return false;
  }
  for (int aa = 0; aa < dst_size; aa++) {
    for (int i = 0; i < pixels; i++) {
      transform_pixel((const GLvoid *)src_pos, (GLvoid *)dst_pos, src_color,
                      src_type, scales, bias);
      src_pos += src_stride;
      dst_pos += src_stride;
    }
...
bool pixel_scale(const GLvoid *old, GLvoid **new, GLuint width, GLuint height,
                 GLuint new_width, GLuint new_height, GLenum format,
                 GLenum type) {
  GLuint pixel_size;
...
bool pixel_halfscale(const GLvoid *old, GLvoid **new, GLuint width,
                     GLuint height, GLenum format, GLenum type) {
  if (!old) {
    *new = NULL;
    return 1;
  }
  ...
  // new_width, new_height, PrintEnum(format), PrintEnum(type));
  const colorlayout_t *src_color;
...
bool pixel_thirdscale(const GLvoid *old, GLvoid **new, GLuint width,
                      GLuint height, GLenum format, GLenum type) {
...
bool pixel_quarterscale(const GLvoid *old, GLvoid **new, GLuint width,
                        GLuint height, GLenum format, GLenum type) {
  GLuint pixel_size, new_width, new_height;
  new_width = width / 4;
  if (!new_width)
  ...
  src = (uintptr_t)old;
  pos = (uintptr_t)dst;
  const int dxs[4] = {0, width > 1 ? 1 : 0, width > 2 ? 2 : 0,
                      width > 3   ? 3
                      : width > 1 ? 1
                                  : 0};
    ...
    for (int x = 0; x < new_width; x++) {
      for (int dx = 0; dx < 4; dx++) {
        for (int dy = 0; dy < 4; dy++) {
          pix[dx + dy * 4] =
              src +
              ((x * 4 + dxs[dx]) + (y * 4 + dys[dy]) * width) * pixel_size;
        }
      }
...
bool pixel_doublescale(const GLvoid *old, GLvoid **new, GLuint width,
                       GLuint height, GLenum format, GLenum type) {
  if (!old) {
    *new = NULL;
    return true;
...
bool pixel_to_ppm(const GLvoid *pixels, GLuint width, GLuint height,
  ...
  if (format == GL_RGB && type == GL_UNSIGNED_BYTE) {
    ...
    if (!pixel_convert(pixels, (GLvoid **)&src, width, height, format, type,
                       GL_RGB, GL_UNSIGNED_BYTE, 0, align)) {
      return false;
    }
  }
