...
void APIENTRY_GL4ES gl4es_glTexEnvf(GLenum target, GLenum pname,
                                    GLfloat param) {
  DBG(printf(
          "glTexEnvf(%s, %s, 0x%04X(%s)), tmu=%d, pending=%d, compiling=%d\n",
          PrintEnum(target), PrintEnum(pname), (GLenum)param,
          PrintEnum((GLenum)param), glstate->texture.active,
          glstate->list.pending, glstate->list.compiling);)
  if (!glstate->list.pending) {
    PUSH_IF_COMPILING(glTexEnvf);
  }
  // Handling GL_EXT_DOT3, wrapping to standard dot3 (???)
  if (param == GL_DOT3_RGB_EXT)
    param = GL_DOT3_RGB;
  if (param == GL_DOT3_RGBA_EXT)
    param = GL_DOT3_RGBA;
  ...
  case GL_TEXTURE_ENV: {
    ...
    case GL_COMBINE_RGB:
      if (t->combine_rgb == param)
        return;
      if ((param == GL_MODULATE_ADD_ATI ||
           param == GL_MODULATE_SIGNED_ADD_ATI ||
           param == GL_MODULATE_SUBTRACT_ATI)) {
        if (hardext.esversion == 1) {
          errorShim(GL_INVALID_ENUM);
          return;
        }
      } else if (param != GL_REPLACE && param != GL_MODULATE &&
                 param != GL_ADD && param != GL_ADD_SIGNED &&
                 param != GL_INTERPOLATE && param != GL_SUBTRACT &&
                 param != GL_DOT3_RGB && param != GL_DOT3_RGBA) {
        errorShim(GL_INVALID_ENUM);
        return;
      }
      FLUSH_BEGINEND;
      ...
      if ((param == GL_ZERO || param == GL_ONE ||
        ...
        if (hardext.esversion == 1) {
          ...
          return;
                 ...
                 !(param >= GL_TEXTURE0 &&
                   param < GL_TEXTURE0 + hardext.maxtex) &&
                 param != GL_CONSTANT && param != GL_PRIMARY_COLOR &&
                 param != GL_PREVIOUS) {
        errorShim(GL_INVALID_ENUM);
        return;
      }
    ...
    case GL_SRC3_RGB:
      ...
      if (param != GL_TEXTURE &&
          !(param >= GL_TEXTURE0 && param < GL_TEXTURE0 + hardext.maxtex) &&
          param != GL_CONSTANT && param != GL_PRIMARY_COLOR &&
          param != GL_PREVIOUS && param != GL_ZERO) {
        errorShim(GL_INVALID_ENUM);
        return;
      }
      FLUSH_BEGINEND;
    ...
    case GL_SRC0_ALPHA:
      ...
      if (glstate->fpe_state) {
        int state = FPE_SRC_TEXTURE;
        if (t->src0_alpha >= GL_TEXTURE0 &&
            t->src0_alpha < GL_TEXTURE0 + MAX_TEX) {
          ...
          case GL_ZERO:
            state = FPE_SRC_ZERO;
            break;
            // case GL_SECONDARY_COLOR_ATIX: state=FPE_SRC_SECONDARY_COLOR;
            // break; case GL_TEXTUTRE_OUTPUT: unknown, so fall back to
            // texture...
          }
    ...
    case GL_SRC1_ALPHA:
      ...
      if (glstate->fpe_state) {
        int state = FPE_SRC_TEXTURE;
        if (t->src1_alpha >= GL_TEXTURE0 &&
            t->src1_alpha < GL_TEXTURE0 + MAX_TEX) {
    ...
    case GL_SRC2_ALPHA:
      ...
      if (glstate->fpe_state) {
        int state = FPE_SRC_TEXTURE;
        if (t->src2_alpha >= GL_TEXTURE0 &&
            t->src2_alpha < GL_TEXTURE0 + MAX_TEX) {
    ...
    case GL_OPERAND0_ALPHA:
      ...
      if (glstate->fpe_state) {
        int state = FPE_OP_ALPHA;
        if (t->op0_alpha == GL_ONE_MINUS_SRC_ALPHA)
          state = FPE_OP_MINUSALPHA;
    ...
    case GL_OPERAND1_ALPHA:
      ...
      if (glstate->fpe_state) {
        int state = FPE_OP_ALPHA;
        if (t->op1_alpha == GL_ONE_MINUS_SRC_ALPHA)
          state = FPE_OP_MINUSALPHA;
    ...
    case GL_OPERAND2_ALPHA:
      ...
      if (glstate->fpe_state) {
        int state = FPE_OP_ALPHA;
        if (t->op2_alpha == GL_ONE_MINUS_SRC_ALPHA)
          state = FPE_OP_MINUSALPHA;
    ...
    case GL_OPERAND3_ALPHA:
      ...
      if (glstate->fpe_state) {
        int state = FPE_OP_ALPHA;
        if (t->op3_alpha == GL_ONE_MINUS_SRC_ALPHA)
          state = FPE_OP_MINUSALPHA;
...
void APIENTRY_GL4ES gl4es_glTexEnvfv(GLenum target, GLenum pname,
                                     const GLfloat *param) {
  DBG(printf("glTexEnvfv(%s, %s, %p)->", PrintEnum(target), PrintEnum(pname),
             param);)
...
void APIENTRY_GL4ES gl4es_glTexEnviv(GLenum target, GLenum pname,
                                     const GLint *param) {
  DBG(printf("glTexEnviv(%s, %s, %p)->", PrintEnum(target), PrintEnum(pname),
             param);)
  if (glstate->list.compiling && glstate->list.active &&
      !glstate->list.pending) {
