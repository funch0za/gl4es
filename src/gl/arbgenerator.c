...
#define FAIL(str)                                                              \
  ...
  *error_msg = strdup(str);                                                    \
  return
void generateVariablePre(sCurStatus *curStatusPtr, int vertex, char **error_msg,
                         sVariable *varPtr) {
  (void)vertex;
...
void generateInstruction(sCurStatus *curStatusPtr, int vertex, char **error_msg,
...
#define ASSERT_COUNT(cnt)                                                      \
  if (((cnt < MAX_OPERANDS) && instPtr->vars[cnt].var) ||                      \
      (cnt && !instPtr->vars[cnt - 1].var)) {                                  \
    FAIL("Invalid instruction (not enough/too many arguments)");               \
  }
#define ASSERT_MASKDST(i)                                                      \
  if ((instPtr->vars[i].var->type != VARTYPE_TEMP) &&                          \
      (instPtr->vars[i].var->type != VARTYPE_OUTPUT) &&                        \
...
#define ASSERT_VECTSRC(i)                                                      \
  if ((instPtr->vars[i].var->type != VARTYPE_TEMP) &&                          \
      (instPtr->vars[i].var->type != VARTYPE_ATTRIB) &&                        \
      (instPtr->vars[i].var->type != VARTYPE_PARAM) &&                         \
      (instPtr->vars[i].var->type != VARTYPE_CONST) &&                         \
...
#define INST_SAMPLE                                                                                      \
  ...
  // Misc pushing
/* Append a DeSTination MASK (i is destination index, b is base swizzle vector
 * or destination) */
...
#define PUSH_DESTLEN(i)                                                        \
  for (dstSwizLen = 0; (dstSwizLen < 4) && (SWIZ(i, dstSwizLen) != SWIZ_NONE); \
       ++dstSwizLen)                                                           \
    ;                                                                          \
  switch (dstSwizLen) {                                                        \
  case 1:                                                                      \
...
#define FINISH_INST(dst)                                                       \
  ...
  case INST_TEX:
    ...
    APPEND_OUTPUT("texture",
                  7) // Deprecated! (but texture is not official until 1.30)
    PUSH_SAMPFCN(3)
    APPEND_OUTPUT("(", 1)
    PUSH_SAMPLER(2, 3)
    APPEND_OUTPUT(", ", 2)
...
void generateVariablePst(sCurStatus *curStatusPtr, int vertex, char **error_msg,
                         sVariable *varPtr) {
  (void)vertex;
