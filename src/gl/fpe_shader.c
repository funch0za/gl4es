...
//                          2D              Rectangle      3D          CubeMap
//                          Stream
const char *texname[] = {
    "texture2DProj", "texture2D", "texture2D", "textureCube",
    "textureStreamIMG"}; // textureRectange and 3D are emulated with 2D
const char *texnoproj[] = {
    "texture2D", "texture2D", "texture2D", "textureCube",
    "textureStreamIMG"}; // textureRectange and 3D are emulated with 2D
const char *texsampler[] = {"sampler2D", "sampler2D", "sampler2D",
                            "samplerCube", "samplerStreamIMG"};
...
const char *fpe_texenvSrc(int src, int tmu, int twosided) {
  ...
  case FPE_SRC_TEXTURE15:
    sprintf(buff, "texColor%d",
            src - FPE_SRC_TEXTURE0); // should check if texture is enabled
    break;
...
char *fpe_binary(int x, int s) { return fpe_packed(x, s, 1); }

const char *const *fpe_VertexShader(shaderconv_need_t *need,
                                    fpe_state_t *state) {
  // vertex is first called, so 1st time init is only here
  if (!shad_cap)
    shad_cap = 1024;
  if (!shad)
    shad = (char *)malloc(shad_cap);
  ...
  int secondary = (state->colorsum && !(lighting && light_separate)) ||
                  fpe_texenvSecondary(state) || (need && need->need_secondary);
  ...
  DBG(comments = 1 - comments;) // When DEBUG is activated, the effect of
                                // LIBGL_COMMENTS is reversed

  if (comments) {
    sprintf(
        buff,
        "// ** Vertex Shader **\n// ligthting=%d (twosided=%d, separate=%d, "
        "color_material=%d)\n// secondary=%d, planes=%s\n// point=%d%s\n",
        lighting, twosided, light_separate, color_material, secondary,
        fpe_binary(planes, 6), point, need ? " with need" : "");
    ShadAppend(buff);
    headers += gl4es_countline(buff);
    if (need) {
      sprintf(buff, "// need: color=%d, texs=%s, fogcoord=%d\n",
              need->need_color, fpe_binary(need->need_texs, 16),
              need->need_fogcoord);
      ShadAppend(buff);
      headers += gl4es_countline(buff);
    }
  ...
  if (lighting) {
    ...
    for (int i = 0; i < hardext.maxlights; i++) {
      if (state->light & (1 << i)) {
        sprintf(buff,
                "uniform _gl4es_FPELightSourceParameters%d "
                "_gl4es_LightSource_%d;\n",
  ...
  if (!lighting) {
    ...
    for (int i = 0; i < hardext.maxlights; i++) {
      if (state->light & (1 << i)) {
        if (comments) {
          sprintf(buff,
                  "// light %d on, light_direction=%d, light_cutoff180=%d\n", i,
                  (state->light_direction >> i & 1),
                  (state->light_cutoff180 >> i & 1));
          ShadAppend(buff);
        }
        // enabled light i
        // att depend on light position w
        if ((state->light_direction >> i & 1) ==
            0) { // flag is 1 if light is has w!=0
          ShadAppend("att = 1.0;\n");
        ...
        if ((state->light_cutoff180 >> i & 1) == 0) {
          ...
          /*if((state->light_direction>>i&1)==0) {
              sprintf(buff, "spot = max(dot(-normalize(vertex.xyz),
          _gl4es_LightSource_%d.spotDirection), 0.);\n", i); if(!need_vertex)
          need_vertex=1; } else*/
          {
            sprintf(buff,
                    "spot = max(dot(-VP, _gl4es_LightSource_%d.spotDirection), "
                    "0.);\n",
                    i);
          }
          ShadAppend(buff);
          sprintf(buff,
                  "if(spot<_gl4es_LightSource_%d.spotCosCutoff) spot=0.0; else "
                  "spot=pow(spot, _gl4es_LightSource_%d.spotExponent);\n",
                  i, i);
          ShadAppend(buff);
          ShadAppend("att *= spot;\n");
        }
        ...
        if (twosided) {
          sprintf(buff,
                  "back_dd = (nVP<0.)?(-nVP * %s%d.diffuse.xyz):vec3(0.);\n",
                  bm_diffuse, i);
          ShadAppend(buff);
          need_lightproduct[1][i] = 1;
        }
        ...
        if (cm_front_nullexp)
          sprintf(buff,
                  "ss = (nVP>0. && lVP>0.)?(pow(lVP, "
                  "%s)*%s%d.specular.xyz):vec3(0.);\n",
                  (color_material) ? "gl_FrontMaterial.shininess"
                                   : "_gl4es_FrontMaterial_shininess",
                  fm_specular, i);
        else
          sprintf(buff,
                  "ss = (nVP>0. && lVP>0.)?(%s%d.specular.xyz):vec3(0.);\n",
                  fm_specular, i);
        ShadAppend(buff);
        if (twosided) {
          if (state->cm_back_nullexp) // 1, exp is not null
            sprintf(buff,
                    "back_ss = (nVP<0. && lVP<0.)?(pow(-lVP, "
                    "%s)*%s%d.specular.xyz):vec3(0.);\n",
                    (color_material) ? "gl_BackMaterial.shininess"
                                     : "_gl4es_BackMaterial_shininess",
                    bm_specular, i);
          else
            sprintf(
                buff,
                "back_ss = (nVP<0. && lVP<0.)?(%s%d.specular.xyz):vec3(0.);\n",
                bm_specular, i);
          ShadAppend(buff);
        }
    ...
    if (t) {
      ...
      if (comments) {
        sprintf(buff, "// texture %d active: %X %s %s\n", i, t,
                mat ? "with matrix" : "", adjust ? "npot adjusted" : "");
        ShadAppend(buff);
      }
      char texcoord[50];
          ...
          if (tg[j] == FPE_TG_NORMALMAP) {
            ...
            if (j == 0 && tg[j + 1] == FPE_TG_NORMALMAP) {
              sprintf(buff, "tmp_tcoor.%c%c=normal.%c%c;\n", texcoordxy[j],
                      texcoordxy[j + 1], texcoordxy[j], texcoordxy[j + 1]);
              ++j;
            } else
            ...
            if (!spheremap) {
              ...
              if (j == 0 && tg[j + 1] == FPE_TG_SPHEREMAP)
                sprintf(buff, "tmp_tcoor.xy = "
                              "tmpsphere.xy*(0.5*inversesqrt(dot(tmpsphere, "
                              "tmpsphere))) + vec2(0.5);");
              else
                ShadAppend("tmpsphere.xy = "
                           "tmpsphere.xy*(0.5*inversesqrt(dot(tmpsphere, "
                           "tmpsphere))) + vec2(0.5);");
            }
            if (j == 0 && tg[j + 1] == FPE_TG_SPHEREMAP)
            ...
            if (!reflectmap) {
              ...
              if (!need_vertex)
                need_vertex = 1;
              need_normal = 1;
              if (j == 0 && tg[j + 1] == FPE_TG_REFLECMAP &&
                  tg[j + 2] == FPE_TG_REFLECMAP)
                sprintf(buff, "tmp_tcoor.xyz = reflect(normalize(vertex.xyz), "
                              "normal);\n");
              else
                ShadAppend("vec3 tmpreflect = reflect(normalize(vertex.xyz), "
                           "normal);\n");
            }
            if (j == 0 && tg[j + 1] == FPE_TG_REFLECMAP &&
                tg[j + 2] == FPE_TG_REFLECMAP)
  ...
  if (point) {
    ...
    sprintf(
        buff,
        "gl_PointSize = "
        "clamp(gl_Point.size*inversesqrt(gl_Point.distanceConstantAttenuation "
        "+ ps_d*(gl_Point.distanceLinearAttenuation + "
        "ps_d*gl_Point.distanceQuadraticAttenuation)), gl_Point.sizeMin, "
        "gl_Point.sizeMax);\n");
    ShadAppend(buff);
  }
  ...
  if (need_vertex) {
    ...
    shad = gl4es_inplace_insert(gl4es_getline(shad, normal_line + headers),
                                buff, shad, &shad_cap);
  ...
  if (need_normal) {
    ...
    if (state->rescaling || state->normalize || globals4es.normalize)
      ...
      // strcpy(buff, "vec3 normal = (vec4(gl_Normal,
      // (gl_Vertex.w==0.0)?0.0:(-dot(gl_Normal,
      // gl_Vertex.xyz)/gl_Vertex.w))*gl_ModelViewMatrixInverse).xyz;\n");
      strcpy(buff, "vec3 normal = gl_NormalMatrix * gl_Normal;\n");
#endif
    shad = gl4es_inplace_insert(gl4es_getline(shad, normal_line + headers),
                                buff, shad, &shad_cap);
  ...
  if (buff[0] != '\0') {
    shad = gl4es_inplace_insert(gl4es_getline(shad, headers), buff, shad,
                                &shad_cap);
    headers += gl4es_countline(buff);
  }
  if (fog) {
...
const char *const *fpe_FragmentShader(shaderconv_need_t *need,
                                      fpe_state_t *state) {
  // state can be NULL, so provide a 0 default
  if (!shad_cap)
    shad_cap = 1024;
  ...
  int secondary = is_default
                      ? ((state->colorsum && !(lighting && light_separate)) ||
  ...
  if (comments) {
    sprintf(buff,
            "// ** Fragment Shader **\n// lighting=%d, alpha=%d, secondary=%d, "
            "planes=%s, texturing=%d point=%d\n",
            lighting, alpha_test, secondary, fpe_binary(planes, 6), texturing,
            point);
    ShadAppend(buff);
    headers += gl4es_countline(buff);
  }
  ...
  if (planes) {
    ...
    for (int i = 0; i < hardext.maxplanes; i++) {
      if ((planes >> i) & 1) {
        // sprintf(buff, "%smin(0., dot(clipvertex, gl_ClipPlane[%d]))",
        // k?"+":"",  i);
      ...
      if (t) {
        if (point && pointsprite && pointsprite_coord) {
          if (pointsprite_upper)
            sprintf(buff,
                    "vec4 texColor%d = %s(_gl4es_TexSampler_%d, "
    ...
    for (int i = 0; i < hardext.maxtex; i++) {
      ...
      if (t) {
        ...
        case FPE_DECAL:
          sprintf(
              buff,
              "fColor.rgb = mix(fColor.rgb, texColor%d.rgb, texColor%d.a);\n",
              i, i);
          ShadAppend(buff);
          needclamp = 0;
          break;
        case FPE_BLEND:
          ...
          if (texformat != FPE_TEX_ALPHA) {
            sprintf(buff,
                    "fColor.rgb = mix(fColor.rgb, "
                    "_gl4es_TextureEnvColor_%d.rgb, texColor%d.rgb);\n",
                    i, i);
            ShadAppend(buff);
          }
          switch (texformat) {
          ...
          case FPE_TEX_DEPTH:
            sprintf(buff,
                    "fColor.a = mix(fColor.a, _gl4es_TextureEnvColor_%d.a, "
                    "texColor%d.a);\n",
                    i, i);
            ShadAppend(buff);
            break;
        ...
        case FPE_COMBINE4: {
          ...
          if (combine_rgb == FPE_CR_DOT3_RGBA) {
            ...
            if (combine_alpha == FPE_CR_REPLACE) {
              ...
              op_a[1] = op_a[2] = op_a[3] = -1;
                       ...
                       combine_alpha == FPE_CR_INTERPOLATE) {
              // need all 3
              src_a[3] = -1;
              op_a[3] = -1;
            } else {
              if (texenv == FPE_COMBINE4 &&
                  (combine_alpha == FPE_CR_ADD ||
                   combine_alpha == FPE_CR_ADD_SIGNED)) {
                // need all 4
              } else {
                src_a[2] = src_a[3] = -1;
                op_a[2] = op_a[3] = -1;
            ...
            if (combine_rgb == FPE_CR_REPLACE) {
              ...
              if (texenv == FPE_COMBINE4 &&
                  (combine_rgb == FPE_CR_ADD ||
                   combine_rgb == FPE_CR_ADD_SIGNED)) {
                // need all 4
              } else {
                src_r[2] = src_r[3] = -1;
                op_r[2] = op_r[3] = -1;
          ...
          if (comments) {
            sprintf(buff,
                    " //  Combine RGB: fct=%d, Src/Op: 0=%d/%d 1=%d/%d 2=%d/%d "
                    "3=%d/%d\n",
                    combine_rgb, src_r[0], op_r[0], src_r[1], op_r[1], src_r[2],
                    op_r[2], src_r[3], op_r[3]);
            ShadAppend(buff);
            sprintf(buff,
                    " //  Combine Alpha: fct=%d, Src/Op: 0=%d/%d 1=%d/%d "
                    "2=%d/%d 3=%d/%d\n",
                    combine_alpha, src_a[0], op_a[0], src_a[1], op_a[1],
                    src_a[2], op_a[2], src_a[3], op_a[3]);
            ShadAppend(buff);
          }
          if (constant) {
            // yep, create the Uniform
            sprintf(buff, "uniform lowp vec4 _gl4es_TextureEnvColor_%d;\n", i);
            shad = gl4es_inplace_insert(gl4es_getline(shad, headers), buff,
                                        shad, &shad_cap);
            headers += gl4es_countline(buff);
          }
          for (int j = 0; j < 4; j++) {
            if (src_r[j] == src_a[j] && op_r[j] == FPE_OP_SRCCOLOR &&
                op_a[j] == FPE_OP_ALPHA) {
              ...
              if (op_a[j] != -1)
                ...
                case FPE_OP_ALPHA:
                  sprintf(buff, "Arg%d.a = %s.a;\n", j,
                          fpe_texenvSrc(src_a[j], i, twosided));
                  ShadAppend(buff);
                  break;
                case FPE_OP_MINUSALPHA:
          ...
          if (combine_rgb != FPE_CR_DOT3_RGBA &&
              combine_rgb != FPE_CR_DOT3_RGB && combine_rgb == combine_alpha) {
            ...
            case FPE_CR_ADD:
              if (texenv == FPE_COMBINE4)
                ShadAppend(
                    "fColor.rgb = Arg0.rgb*Arg1.rgb + Arg2.rgb*Arg3.rgb;\n");
              else
                ShadAppend("fColor.rgb = Arg0.rgb + Arg1.rgb;\n");
              break;
            case FPE_CR_ADD_SIGNED:
              ...
              break;
            case FPE_CR_INTERPOLATE:
              ShadAppend("fColor.rgb = Arg0.rgb*Arg2.rgb + "
                         "Arg1.rgb*(vec3(1.)-Arg2.rgb);\n");
              break;
            case FPE_CR_SUBTRACT:
              ShadAppend("fColor.rgb = Arg0.rgb - Arg1.rgb;\n");
              break;
            case FPE_CR_DOT3_RGB:
              ShadAppend("fColor.rgb = "
                         "vec3(4.*((Arg0.r-0.5)*(Arg1.r-0.5)+(Arg0.g-0.5)*("
                         "Arg1.g-0.5)+(Arg0.b-0.5)*(Arg1.b-0.5)));\n");
              break;
            case FPE_CR_DOT3_RGBA:
              ShadAppend("fColor = "
                         "vec4(4.*((Arg0.r-0.5)*(Arg1.r-0.5)+(Arg0.g-0.5)*("
                         "Arg1.g-0.5)+(Arg0.b-0.5)*(Arg1.b-0.5)));\n");
              break;
            case FPE_CR_MOD_ADD:
            ...
            case FPE_CR_MOD_ADD_SIGNED:
              ShadAppend(
                  "fColor.rgb = Arg0.rgb*Arg2.rgb + Arg1.rgb - vec3(0.5);\n");
              break;
            case FPE_CR_MOD_SUB:
              ShadAppend("fColor.rgb = Arg0.rgb*Arg2.rgb - Arg1.rgb;\n");
              break;
            ...
            if (combine_rgb != FPE_CR_DOT3_RGBA)
              ...
              case FPE_CR_ADD_SIGNED:
                if (texenv == FPE_COMBINE4)
                  ShadAppend(
                      "fColor.a = Arg0.a*Arg1.a + Arg2.a*Arg3.a - 0.5;\n");
                else
  ...
  if (fog) {
    ...
    case FPE_FOG_EXP2:
      sprintf(buff,
              "%s float FogF = clamp(exp(-(gl_Fog.density * "
              "fog_c)*(gl_Fog.density * fog_c)), 0., 1.);\n",
              fogp);
      break;
    case FPE_FOG_LINEAR:
      sprintf(buff, "%s float FogF = clamp((gl_Fog.end - fog_c) %s, 0., 1.);\n",
              fogp,
              hardext.highp ? "* gl_Fog.scale"
  ...
  if (shaderblend) {
    if (comments) {
      sprintf(buff, "//Blend: src=%d/%d, dst=%d/%d, eq=%d/%d\n",
              state->blendsrcrgb, state->blendsrcalpha, state->blenddstrgb,
              state->blenddstalpha, state->blendeqrgb, state->blendeqalpha);
      ShadAppend(buff);
    }
...
const char *const *fpe_CustomVertexShader(const char *initial,
                                          fpe_state_t *state,
  ...
  int color = default_fragment ? (strstr(initial, "_gl4es_Color") ? 0 : 1)
                               : 0; // need to add a simple color variant?
  if (default_fragment)
    printf("fpe_CustomVertexShader(%p, %p, %d)\n%s\ncolor=%d\n", initial, state,
           default_fragment, initial, color);
...
const char *const *fpe_CustomFragmentShader(const char *initial,
                                            fpe_state_t *state) {
  ...
  if (shaderblend &&
      ((state->blendsrcrgb >= FPE_BLEND_CONSTANT_COLOR &&
        state->blendsrcrgb <= FPE_BLEND_ONE_MINUS_CONSTANT_ALPHA) ||
       (state->blenddstrgb >= FPE_BLEND_CONSTANT_COLOR &&
        state->blenddstrgb <= FPE_BLEND_ONE_MINUS_CONSTANT_ALPHA) ||
       (state->blendsrcalpha >= FPE_BLEND_CONSTANT_COLOR &&
        state->blendsrcalpha <= FPE_BLEND_ONE_MINUS_CONSTANT_ALPHA) ||
       (state->blenddstalpha >= FPE_BLEND_CONSTANT_COLOR &&
        state->blenddstalpha <= FPE_BLEND_ONE_MINUS_CONSTANT_ALPHA))) {
  ...
  if (alpha_test || planes || shaderblend) {
    ...
    if (is_fragcolor) {
      int l_main =
          gl4es_getline_for(shad,
                            gl4es_prev_str(shad, strstr(shad, "_gl4es_main"))) -
          1;
      shad = gl4es_inplace_insert(gl4es_getline(shad, l_main),
                                  "lowp vec4 _gl4es_FragColor;\n", shad,
                                  &shad_cap);
      shad = gl4es_inplace_replace(shad, &shad_cap, "gl_FragColor",
                                   "_gl4es_FragColor");
    }
  }
  if (strstr(shad, "_gl4es_main")) {
