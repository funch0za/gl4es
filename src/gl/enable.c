...
generate_changetexgen(s) generate_changetexgen(t) generate_changetexgen(r)
...
static void proxy_glEnable(GLenum cap, bool enable,
                           void(APIENTRY_GLES *next)(GLenum)) {
...
#define GO(constant, name)                                                     \
  case constant:                                                               \
    if (glstate->list.pending && glstate->enable.name != enable)               \
      gl4es_flush();                                                           \
    glstate->enable.name = enable;                                             \
    break;
...
#define clientGO_proxyFPE(constant, name)                                      \
  ...
  switch (cap) {
    ...
    proxy_GOFPE(GL_ALPHA_TEST, alpha_test,
                glstate->fpe_state->alphatest = enable);
  ...
  case GL_BLEND:
    ...
    GOFPE(GL_TEXTURE_GEN_S, texgen_s[glstate->texture.active],
          fpe_changetexgen_s(glstate->texture.active,
                             enable)); // TODO: FPE stuffs
    ...
    proxy_GOFPE(GL_NORMALIZE, normalize,
                glstate->fpe_state->normalize = enable);
    proxy_GOFPE(GL_RESCALE_NORMAL, normal_rescale,
                glstate->fpe_state->rescaling = enable);
    proxy_GOFPE(GL_COLOR_MATERIAL, color_material,
                glstate->fpe_state->color_material = enable);

    // point sprite
    proxy_GOFPE(GL_POINT_SPRITE, pointsprite,
                glstate->fpe_state->pointsprite =
                    enable); // TODO: plugin fpe stuffs
    ...
    // cannot use clientGO_proxyFPE here, has the ClientArray are really enabled
    // / disabled elsewhere in fact (inside glDraw or list_draw)
  ...
  case GL_TEXTURE_2D:
    if (glstate->list.pending &&
        ((glstate->enable.texture[glstate->texture.active] >> ENABLED_TEX2D) &
         1) != enable)
      gl4es_flush();
    if (enable ==
        ((glstate->enable.texture[glstate->texture.active] >> ENABLED_TEX2D) &
...
void APIENTRY_GL4ES gl4es_glEnableClientState(GLenum cap) {
  DBG(printf("glEnableClientState(%s), list.begin=%dn", PrintEnum(cap),
             glstate->list.begin);)
  ERROR_IN_BEGIN
  // should flush for now... to be optimized later!
  /*if (glstate->list.active && !glstate->list.compiling &&
     !glstate->list.pending) gl4es_flush();*/
...
GLboolean APIENTRY_GL4ES gl4es_glIsEnabled(GLenum cap) {
  DBG(printf(
          "glIsEnabed(%s), list.begin=%d, list.compiling=%d, list.pending=%d\n",
  ...
  switch (cap) {
    ...
    clientisenabled(GL_SECONDARY_COLOR_ARRAY,
                    vertexattrib[ATT_SECONDARY].enabled);
    clientisenabled(GL_FOG_COORD_ARRAY, vertexattrib[ATT_FOGCOORD].enabled);
  case GL_TEXTURE_1D:
    return glstate->enable.texture[glstate->texture.active] &
           (1 << ENABLED_TEX1D);
  case GL_TEXTURE_2D:
    return glstate->enable.texture[glstate->texture.active] &
           (1 << ENABLED_TEX2D);
