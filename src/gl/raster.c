...
void APIENTRY_GL4ES gl4es_glViewport(GLint x, GLint y, GLsizei width,
  ...
  if (glstate->fbo.current_fb->id != 0) {
    gltexture_t *tex = gl4es_getTexture(glstate->fbo.current_fb->t_color[0],
                                        glstate->fbo.current_fb->color[0]);
  ...
  if (glstate->raster.viewport.x != x || glstate->raster.viewport.y != y ||
    ...
    glstate->raster.viewport.height = height;
#if !defined(NO_EGL) && !defined(NOX11)
    if (!globals4es.usefbo && !globals4es.usefb &&
        glstate->fbo.fbo_draw->id == 0) {
      // check if underlying EGL surface change dimension, and reflect that to
      // main fbo size
      refreshMainFBO();
    }
#endif
...
#endif
  if (glstate->raster.scissor.x != x || glstate->raster.scissor.y != y ||
...
void popViewport() {
  LOAD_GLES(glViewport);
  gles_glViewport(glstate->raster.viewport.x, glstate->raster.viewport.y,
                  glstate->raster.viewport.width,
...
void init_raster(int width, int height) {
  ...
  if (glstate->raster.data) {
    if ((glstate->raster.raster_nwidth < width) ||
        (glstate->raster.raster_nheight < height)) {
      free(glstate->raster.data);
      glstate->raster.data = NULL;
    }
  }
  if (!glstate->raster.data) {
    glstate->raster.data = (GLubyte *)malloc(
        4 * w * h * sizeof(GLubyte)); // no need to memset to 0
...
GLubyte raster_transform(GLubyte pix, GLubyte number) {
  ...
  a = a * glstate->raster.raster_scale[number] +
      glstate->raster.raster_bias[number];
  if (a < 0.0f)
    return 0;
  if (a > 1.0f)
    return 255;
...
GLfloat FASTMATH raster_transformf(GLfloat pix, GLubyte number) {
  pix = pix * glstate->raster.raster_scale[number] +
        glstate->raster.raster_bias[number];
  if (pix < 0.0f)
    return 0.0f;
  if (pix > 1.0f)
...
int raster_need_transform() {
  for (int i = 0; i < 4; i++)
    if (glstate->raster.raster_scale[i] != 1.0f)
      return 1;
  for (int i = 0; i < 4; i++)
    if (glstate->raster.raster_bias[i] != 0.0f)
      return 1;
  return 0;
}
...
void raster_to_texture(rasterlist_t *r) {
  ...
  if (old_list)
    glstate->list.active = NULL; // deactivate list...
  ...
  if (r->texture == 0) {
    ...
    gl4es_glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_BASE_LEVEL,
                          0); // this is to be sure texture is not npot'ed ...
    gl4es_glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MAX_LEVEL,
                          0); // ... if not needed
    gl4es_glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, glstate->raster.raster_nwidth,
                       glstate->raster.raster_nheight, 0, GL_RGBA,
                       GL_UNSIGNED_BYTE, NULL);
  } else {
    gl4es_glBindTexture(GL_TEXTURE_2D, r->texture);
  }
  gl4es_glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, glstate->raster.raster_width,
                        glstate->raster.raster_height, GL_RGBA,
...
void bitmap_flush() {
  ...
  if (IS_TEX1D(old_active))
    gl4es_glDisable(GL_TEXTURE_1D);
  if (!IS_TEX2D(old_active))
    gl4es_glEnable(GL_TEXTURE_2D);
  if (IS_TEX3D(old_active))
    gl4es_glDisable(GL_TEXTURE_3D);
  if (IS_TEXTURE_RECTANGLE(old_active))
    gl4es_glDisable(GL_TEXTURE_RECTANGLE_ARB);
  if (IS_CUBE_MAP(old_active))
    gl4es_glDisable(GL_TEXTURE_CUBE_MAP);
  ...
  if (glstate->raster.bm_tnwidth < glstate->raster.bm_width ||
    ...
    glstate->raster.bm_tnwidth = (hardext.npot)
                                     ? glstate->raster.bm_width
                                     : npot(glstate->raster.bm_width);
    glstate->raster.bm_tnheight = (hardext.npot)
                                      ? glstate->raster.bm_height
                                      : npot(glstate->raster.bm_height);
    gl4es_glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, glstate->raster.bm_tnwidth,
                       glstate->raster.bm_tnheight, 0, GL_RGBA,
                       GL_UNSIGNED_BYTE, NULL);
  }
  ...
  if (sx == 0 && sy == 0 && ex == glstate->raster.bm_width &&
      ey == glstate->raster.bm_height) {
    gl4es_glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, glstate->raster.bm_width,
                          glstate->raster.bm_height, GL_RGBA, GL_UNSIGNED_BYTE,
                          glstate->raster.bitmap);
  } else {
    int alloc = 4 * ex * ey;
    gl4es_scratch(alloc);
    for (int i = 0; i < ey; i++)
      memcpy((char *)glstate->scratch + 4 * i * ex,
             (char *)glstate->raster.bitmap +
                 4 * (sx + (sy + i) * glstate->raster.bm_width),
             ex * 4);
    gl4es_glTexSubImage2D(GL_TEXTURE_2D, 0, sx, sy, ex, ey, GL_RGBA,
                          GL_UNSIGNED_BYTE, glstate->scratch);
  }

  gl4es_blitTexture(glstate->raster.bm_texture, sx, sy, ex, ey,
                    glstate->raster.bm_tnwidth, glstate->raster.bm_tnheight,
                    1.f, 1.f, 0, 0, // vp is default here
                    sx, sy, BLIT_ALPHA);

  glstate->raster.bm_drawing = 0;

  if (IS_TEX1D(old_active))
    gl4es_glEnable(GL_TEXTURE_1D);
  if (!IS_TEX2D(old_active))
    gl4es_glDisable(GL_TEXTURE_2D);
  if (IS_TEX3D(old_active))
    gl4es_glEnable(GL_TEXTURE_3D);
  if (IS_TEXTURE_RECTANGLE(old_active))
    gl4es_glEnable(GL_TEXTURE_RECTANGLE_ARB);
  if (IS_CUBE_MAP(old_active))
    gl4es_glEnable(GL_TEXTURE_CUBE_MAP);
...
void APIENTRY_GL4ES gl4es_glBitmap(GLsizei width, GLsizei height, GLfloat xorig,
  ...
  /*printf("glBitmap, xy={%f, %f}, xyorig={%f, %f}, size={%u, %u}, zoom={%f,
     %f}, viewport={%i, %i, %i, %i}\n", glstate->raster.rPos.x,
     glstate->raster.rPos.y, xorig, yorig, width, height,
     glstate->raster.raster_zoomx, glstate->raster.raster_zoomy,
     glstate->raster.viewport.x, glstate->raster.viewport.y,
  ...
  if (glstate->list.active) {
    ...
    if (list->bitmaps->count == list->bitmaps->cap) {
      list->bitmaps->cap += 8;
      list->bitmaps->list = (bitmap_list_t *)realloc(
          list->bitmaps->list, list->bitmaps->cap * sizeof(bitmap_list_t));
    }
  ...
  if (ry + sy < 0)
    sy = -ry;
  if (rx + ex > glstate->raster.viewport.width)
    ex = glstate->raster.viewport.width - rx;
  if (ry + ey > glstate->raster.viewport.height)
    ey = glstate->raster.viewport.height - ry;
  if (ex < sx) {
    int tmp = ex;
    ex = sx;
    sx = tmp;
  ...
  if (ex < 0 || ey < 0 || sx < 0 || sy < 0 || sx == ex ||
      sy == ey) // nothing to draw, no changes
    return;
  // create/realloc buffer if needed
  if (glstate->raster.bm_alloc <
      glstate->raster.viewport.width * glstate->raster.viewport.height * 4) {
    if (glstate->raster.bitmap)
      free(glstate->raster.bitmap);
    glstate->raster.bm_alloc =
        glstate->raster.viewport.width * glstate->raster.viewport.height * 4;
    glstate->raster.bitmap = (GLubyte *)malloc(glstate->raster.bm_alloc);
  }
  // clear buffer if needed
  if (!glstate->raster.bm_drawing) {
    memset(glstate->raster.bitmap, 0,
           glstate->raster.viewport.width * glstate->raster.viewport.height *
               4);
  ...
  if (pixtrans) {
    for (int y = sy; y < ey; ++y) {
      ...
      to = glstate->raster.bitmap +
           4 * (GLint)(rx + ((ry + y) * glstate->raster.bm_width));
      for (int x = sx; x < ex; ++x) {
...
void APIENTRY_GL4ES gl4es_glDrawPixels(GLsizei width, GLsizei height,
  ...
  /*printf("glDrawPixels, xy={%f, %f}, size={%i, %i}, format=%s, type=%s,
     zoom={%f, %f}, viewport={%i, %i, %i, %i}\n", glstate->raster.rPos.x,
     glstate->raster.rPos.y, width, height, PrintEnum(format), PrintEnum(type),
     glstate->raster.raster_zoomx, glstate->raster.raster_zoomy,
     glstate->raster.viewport.x, glstate->raster.viewport.y,
     glstate->raster.viewport.width, glstate->raster.viewport.height);*/
  ...
  GLsizei bmp_width = (glstate->texture.unpack_row_length)
                          ? glstate->texture.unpack_row_length
                          : width;

  if (!pixel_convert(data, &dst, bmp_width, height, format, type, GL_RGBA,
                     GL_UNSIGNED_BYTE, 0,
  ...
  if (pixtrans) {
    for (int y = 0; y < height; y++) {
      ...
      from = pixels + 4 * (glstate->texture.unpack_skip_pixels +
                           (y + glstate->texture.unpack_skip_rows) * bmp_width);
...
void render_raster_list(rasterlist_t *rast) {
  // printf("render_raster_list, rast->x/y=%f/%f rast->width/height=%i/%i,
  // rPos.x/y/z=%f/%f/%f, rast->zoomxy=%f/%f raster->texture=%u\n", rast->xorig,
  // rast->yorig, rast->width, rast->height, glstate->raster.rPos.x,
  // glstate->raster.rPos.y, glstate->raster.rPos.z, rast->zoomx, rast->zoomy,
  // rast->texture);
  if (rast->texture)
    gl4es_blitTexture(rast->texture, 0.f, 0.f, rast->width, rast->height,
                      rast->nwidth, rast->nheight, rast->zoomx, rast->zoomy, 0,
                      0, // vp is default here
...
void APIENTRY_GL4ES gl4es_glPixelMapfv(GLenum map, GLsizei mapsize,
...
void APIENTRY_GL4ES gl4es_glPixelMapuiv(GLenum map, GLsizei mapsize,
                                        const GLuint *values) {
  if (mapsize > MAX_MAP_SIZE) {
    errorShim(GL_INVALID_VALUE);
    return;
...
void APIENTRY_GL4ES gl4es_glPixelMapusv(GLenum map, GLsizei mapsize,
