...
void APIENTRY_GL4ES gl4es_glPushAttrib(GLbitfield mask) {
  ...
  if (glstate->stack == NULL) {
    ...
    glstate->stack->cap += STACK_SIZE;
    glstate->stack = (glstack_t *)realloc(
        glstate->stack, glstate->stack->cap * sizeof(glstack_t));
  }
  ...
  if (mask & GL_ENABLE_BIT) {
    ...
    cur->clip_planes_enabled =
        (GLboolean *)malloc(hardext.maxplanes * sizeof(GLboolean));
    ...
    cur->lights_enabled =
        (GLboolean *)malloc(hardext.maxlights * sizeof(GLboolean));
    ...
    cur->sample_alpha_to_coverage =
        gl4es_glIsEnabled(GL_SAMPLE_ALPHA_TO_COVERAGE);
  ...
  if (mask & GL_LIGHTING_BIT) {
    ...
    cur->lights_enabled =
        (GLboolean *)malloc(hardext.maxlights * sizeof(GLboolean));
    cur->lights =
        (GLfloat *)malloc(hardext.maxlights * sizeof(GLfloat) * (10 * 4));
    for (i = 0; i < hardext.maxlights; i++) {
      *(cur->lights_enabled + i) = gl4es_glIsEnabled(GL_LIGHT0 + i);
#define L(A)                                                                   \
  gl4es_glGetLightfv(GL_LIGHT0 + i, A, cur->lights + j);                       \
  j += 4
      L(GL_AMBIENT);
      L(GL_DIFFUSE);
      L(GL_SPECULAR);
...
#define M(A)                                                                   \
  gl4es_glGetMaterialfv(GL_BACK, A, cur->materials + j);                       \
  j += 4;                                                                      \
  gl4es_glGetMaterialfv(GL_FRONT, A, cur->materials + j);                      \
  j += 4
    M(GL_AMBIENT);
    M(GL_DIFFUSE);
    M(GL_SPECULAR);
...
#undef M
  ...
  if (mask & GL_PIXEL_MODE_BIT) {
    GLenum pixel_name[] = {GL_RED_BIAS,    GL_RED_SCALE,  GL_GREEN_BIAS,
  ...
  if (mask & GL_STENCIL_BUFFER_BIT) {
    ...
    gl4es_glGetIntegerv(GL_STENCIL_PASS_DEPTH_FAIL,
                        (GLint *)&cur->stencil_dpfail);
    gl4es_glGetIntegerv(GL_STENCIL_PASS_DEPTH_PASS,
                        (GLint *)&cur->stencil_dppass);
  ...
  if (mask & GL_TEXTURE_BIT) {
    ...
    for (a = 0; a < hardext.maxtex; a++) {
      ...
      cur->texgen_q[a] = glstate->enable.texgen_q[a];
      cur->texgen[a] =
          glstate->texgen[a]; // all mode and planes per texture in 1 line
...
void APIENTRY_GL4ES gl4es_glPushClientAttrib(GLbitfield mask) {
  ...
  if (glstate->clientStack == NULL) {
    glstate->clientStack =
        (glclientstack_t *)malloc(STACK_SIZE * sizeof(glclientstack_t));
    glstate->clientStack->len = 0;
    glstate->clientStack->cap = STACK_SIZE;
  } else if (glstate->clientStack->len == glstate->clientStack->cap) {
    glstate->clientStack->cap += STACK_SIZE;
    glstate->clientStack = (glclientstack_t *)realloc(
        glstate->clientStack,
        glstate->clientStack->cap * sizeof(glclientstack_t));
  }
  ...
  if (mask & GL_CLIENT_VERTEX_ARRAY_BIT) {
    memcpy(cur->vertexattrib, glstate->vao->vertexattrib,
           sizeof(glstate->vao->vertexattrib));
...
#define enable_disable(pname, enabled)                                         \
  if (enabled)                                                                 \
    gl4es_glEnable(pname);                                                     \
  else                                                                         \
    gl4es_glDisable(pname)
...
void APIENTRY_GL4ES gl4es_glPopAttrib(void) {
  ...
  if (cur->mask & GL_ENABLE_BIT) {
    ...
    if (glstate->texture.active != old_tex)
  ...
  if (cur->mask & GL_LIGHTING_BIT) {
    ...
    // Light position / direction is transformed. So load identity in modelview
    // to restore correct stuff
    int identity = is_identity(getMVMat());
    if (!identity) {
      if (old_matrixmode != GL_MODELVIEW)
...
#undef L
    ...
    if (!identity) {
      gl4es_glPopMatrix();
      if (old_matrixmode != GL_MODELVIEW)
        gl4es_glMatrixMode(old_matrixmode);
    }
    j = 0;
#define M(A)                                                                   \
  if (memcmp(cur->materials + j, cur->materials + j + 4,                       \
             4 * sizeof(GLfloat)) == 0) {                                      \
    gl4es_glMaterialfv(GL_FRONT_AND_BACK, A, cur->materials + j);              \
    j += 8;                                                                    \
  } else {                                                                     \
    gl4es_glMaterialfv(GL_BACK, A, cur->materials + j);                        \
    j += 4;                                                                    \
    gl4es_glMaterialfv(GL_FRONT, A, cur->materials + j);                       \
    j += 4;                                                                    \
  }
    M(GL_AMBIENT);
    M(GL_DIFFUSE);
...
#undef M
    ...
    if (hardext.pointsprite) {
      ...
      for (a = 0; a < hardext.maxtex; a++) {
        if (glstate->texture.pscoordreplace[a] != cur->pscoordreplace[a]) {
          if (glstate->texture.active != a)
            gl4es_glActiveTexture(GL_TEXTURE0 + a);
          gl4es_glTexEnvi(GL_POINT_SPRITE, GL_COORD_REPLACE,
                          cur->pscoordreplace[a]);
        }
      }
      if (glstate->texture.active != old_tex)
        gl4es_glActiveTexture(GL_TEXTURE0 + old_tex);
  ...
  if (cur->mask & GL_STENCIL_BUFFER_BIT) {
    ...
    // TODO: Stencil value mask
    gl4es_glStencilOp(cur->stencil_sfail, cur->stencil_dpfail,
                      cur->stencil_dppass);
  ...
  if (cur->mask & GL_TEXTURE_BIT) {
    ...
    for (a = 0; a < hardext.maxtex; a++) {
      ...
      glstate->enable.texgen_q[a] = cur->texgen_q[a];
      glstate->texgen[a] =
          cur->texgen[a]; // all mode and planes per texture in 1 line
      for (int j = 0; j < ENABLED_TEXTURE_LAST; j++)
  ...
  if (cur->mask & GL_VIEWPORT_BIT) {
    gl4es_glViewport(cur->viewport_size[0], cur->viewport_size[1],
                     cur->viewport_size[2], cur->viewport_size[3]);
...
void APIENTRY_GL4ES gl4es_glPopClientAttrib(void) {
  ...
  if (cur->mask & GL_CLIENT_VERTEX_ARRAY_BIT) {
    memcpy(glstate->vao->vertexattrib, cur->vertexattrib,
           sizeof(glstate->vao->vertexattrib));
