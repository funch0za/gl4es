...
char *gl4es_inplace_replace(char *pBuffer, int *size, const char *S,
                            const char *D);
char *gl4es_append(char *pBuffer, int *size, const char *S);
char *gl4es_inplace_insert(char *pBuffer, const char *S, char *master,
                           int *size);
char *gl4es_getline(char *pBuffer, int num);
int gl4es_countline(const char *pBuffer);
int gl4es_getline_for(
    const char *pBuffer,
    const char *S); // get the line number for 1st occurent of S in pBuffer
char *gl4es_str_next(
    char *pBuffer,
    const char *S); // mostly as strstr, but go after the substring if found
...
const char *gl4es_get_next_str(
    char *pBuffer); // get a (static) copy of next str (until next separator),
                    // can be a simple number or separator also

// those function don't try to be smart with separators...
int gl4es_countstring_simple(char *pBuffer, const char *S);
char *gl4es_inplace_replace_simple(char *pBuffer, int *size, const char *S,
                                   const char *D);
