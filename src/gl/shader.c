...
void APIENTRY_GL4ES gl4es_glCompileShader(GLuint shader) {
  ...
  if (gles_glCompileShader) {
    ...
    if (globals4es.logshader) {
      ...
      if (status != GL_TRUE) {
        printf("LIBGL: Error while compiling shader %d. Original source "
               "is:\n%s\n=======\n",
...
void APIENTRY_GL4ES gl4es_glShaderSource(GLuint shader, GLsizei count,
  ...
  if (gles_glShaderSource) {
    // adapt shader if needed (i.e. not an es2 context and shader is not
    // #version 100)
    if (glstate->glsl->es2 && !strncmp(glshader->source, "#version 100", 12))
      glshader->converted = strdup(glshader->source);
    else
      glshader->converted = ConvertShader(
          glshader->source, glshader->type == GL_VERTEX_SHADER ? 1 : 0,
          &glshader->need);
    // send source to GLES2 hardware if any
    gles_glShaderSource(shader, 1,
                        (const GLchar *const *)((glshader->converted)
                                                    ? (&glshader->converted)
                                                    : (&glshader->source)),
                        NULL);
    errorGL();
...
void accumShaderNeeds(GLuint shader, shaderconv_need_t *need) {
  ...
  if (!glshader->converted)
    return;
#define GO(A)                                                                  \
  if (need->need_##A < glshader->need.need_##A)                                \
    need->need_##A = glshader->need.need_##A;
...
static const char *GLES_NoGLSLSupport =
    "No Shader support with current backend";

void APIENTRY_GL4ES gl4es_glGetShaderInfoLog(GLuint shader, GLsizei maxLength,
...
void APIENTRY_GL4ES gl4es_glGetShaderPrecisionFormat(GLenum shaderType,
  ...
  if (gles_glGetShaderPrecisionFormat) {
    gles_glGetShaderPrecisionFormat(shaderType, precisionType, range,
                                    precision);
    errorGL();
  } else {
    errorShim(GL_INVALID_ENUM);
...
void APIENTRY_GL4ES gl4es_glShaderBinary(GLsizei count, const GLuint *shaders,
                                         GLenum binaryFormat,
...
AliasExport(void, glGetShaderInfoLog, ,
            (GLuint shader, GLsizei maxLength, GLsizei *length,
             GLchar *infoLog));
...
AliasExport(GLvoid, glGetShaderSource, ARB,
