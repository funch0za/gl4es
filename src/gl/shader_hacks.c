...
static const hack_t gl4es_hacks[] = {
    // this is for Guacamelee (yep, there is a lot of hacks, only int -> float
    // conversions)
    // 1
    {"float edgeGlow = step ( 0.2 , pow ( clamp ( ( dot ( vec2 ( 1 * sign ( "
     "v_texcoord3 . z ) , 1 ) , normalize ( quadCoord . xy - 0.5 ) ) - 0.4 + "
     "depth * 2.0 ) , 0.0  , 1.0  ) , 25 ) ) ;",
     1,
     {"float edgeGlow = step ( 0.2 , pow ( clamp ( ( dot ( vec2 ( 1.0 * sign ( "
      "v_texcoord3 . z ) , 1.0 ) , normalize ( quadCoord . xy - 0.5 ) ) - 0.4 "
      "+ depth * 2.0 ) , 0.0  , 1.0  ) , 25.0 ) ) ;"}},
    // 2
    {"float litfire = max ( dot ( normalize ( drops1 . rgb ) , normalize ( "
     "vec3 ( - 1 , 0 , pow ( max ( 1.0 - ocoord . x , 0.0 ) , 9 ) ) ) ) , 0 ) "
     ";",
     1,
     {"float litfire = max ( dot ( normalize ( drops1 . rgb ) , normalize ( "
      "vec3 ( - 1.0 , 0.0 , pow ( max ( 1.0 - ocoord . x , 0.0 ) , 9.0 ) ) ) ) "
      ", 0.0 ) ;"}},
    // 3
    ...
    {"gl_FragColor . a *= pow ( clamp ( ( depth + 1 ) , 0.0  , 1.0  ) , 70 ) ;",
     1,
     {"gl_FragColor . a *= pow ( clamp ( ( depth + 1.0 ) , 0.0  , 1.0  ) , "
      "70.0 ) ;"}},
    // 6
    ...
    {"   if(texColor.w == 0)\n       gl_FragColor = texColor;",
     ...
     {"   if(texColor.w == 0.0)\n       gl_FragColor = texColor;"}},
    // 2
    {"if(dist1 > 0)       {           float lightVal = (1-dist1) * "
     "light1Luminosity;",
     1,
     {"if(dist1 > 0.0)       {           float lightVal = (1.0-dist1) * "
      "light1Luminosity;"}},
    // 3
    ...
    {"gl_FragColor = (sample*(1-grayScaleFactor)) + (gray*grayScaleFactor);",
     1,
     {"gl_FragColor = (sample*(1.0-grayScaleFactor)) + "
      "(gray*grayScaleFactor);"}},

    // For Knytt Underground
    {"vec2 val = texture_coordinate1+coeff*2*(i/float(iterations-1.0) - 0.5);",
     1,
     {"vec2 val = texture_coordinate1+coeff*2.0*(float(i)/float(iterations-1) "
      "- 0.5);"}},
    ...
    {"float c[5] = float[5](1.0, 0.9238795325, 0.7071067812, 0.3826834324, "
     "0.0);",
     1,
     {"float c[5]; c[0]=1.0; c[1]=0.9238795325; c[2]=0.7071067812; "
      "c[3]=0.3826834324; c[4]=0.0;"}},

    {"float c[7] = float[7](1.0, 0.9659258263, 0.8660254038, 0.7071067812, "
     "0.5, 0.2588190451, 0.0);",
     1,
     {"float c[7]; c[0]=1.0; c[1]=0.9659258263; c[2]=0.8660254038; "
      "c[3]=0.7071067812; c[4]=0.5; c[5]=0.2588190451; c[6]=0.0;"}},

    {"float scale = 2.0 / r_shadowMapSize;",
     1,
     {"float scale = 2.0 / float(r_shadowMapSize);"}},
    ...
    {"vec2 poissonDisc[9] = vec2[9](\n"
     ...
     {"vec2 poissonDisc[9];\n",
      ...
      "void main()\n"
      "{\n"
      "poissonDisc[0] = vec2(-0.7055767, 0.196515);  poissonDisc[1] = "
      "vec2(0.3524343, -0.7791386);\n"
      "poissonDisc[2] = vec2(0.2391056, 0.9189604);  poissonDisc[3] = "
      "vec2(-0.07580382, -0.09224417);\n"
      "poissonDisc[4] = vec2(0.5784913, -0.002528916);poissonDisc[5]= "
      "vec2(0.192888, 0.4064181);\n"
      "poissonDisc[6] = vec2(-0.6335801, -0.5247476);poissonDisc[7] = "
      "vec2(-0.5579782, 0.7491854);\n"
    ...
    {"//float gauss[5] = float[5](0.30, 0.23, 0.097, 0.024, 0.0033);\n"
     ...
     {"//float gauss[5] = float[5](0.30, 0.23, 0.097, 0.024, 0.0033);\n"
      ...
      "void main()\n"
      "{\n"
      "        gauss[0]=0.40; gauss[1]=0.24; gauss[2]=0.054; "
      "gauss[3]=0.0044;\n"}},
    ...
    {"#version 140\r\n"
     ...
     "void main()\r\n"
     "{\r\n"
     "\tvar_out = (texture(SG3D_TEXTURE_DIFFUSE, "
     "var_uv+var_param.xy*-7.0)*55.0+\r\n"
     "\t\t\ttexture(SG3D_TEXTURE_DIFFUSE, var_uv+var_param.xy*-3.0)*330.0+\r\n"
     "\t\t\ttexture(SG3D_TEXTURE_DIFFUSE, var_uv)*252.0+\r\n"
     "\t\t\ttexture(SG3D_TEXTURE_DIFFUSE, var_uv+var_param.xy* 3.0)*330.0+\r\n"
     "\t\t\ttexture(SG3D_TEXTURE_DIFFUSE, var_uv+var_param.xy* "
     "7.0)*55.0)/1022.0;\r\n"
     "}\r\n",
     1,
     {"#version 120\n"
    ...
    {"#version 140\r\n"
     ...
     {"#version 120\n"
      ...
      "\n"
      "const mat3 yuv2rgb =   "
      "mat3(1.0,0.0,1.596,1.0,-0.391,-0.813,1.0,2.018,0.0);\n"
      "\n"
      "void main()\n"
      "{\n"
    ...
    // for KiCAD
    {"        float derivative   = length( dFdx( tex ) ) * u_fontTextureWidth "
     "/ 4;\n",
     1,
     {"        float derivative   = length( dFdx( tex ) ) * "
      "float(u_fontTextureWidth) / 4.0;\n"}},
...
// Disable hotspot shaders 1
    ...
    {"void main()\n"
     ...
     "    if (p.a != 0.0) {\n"
     "        float zoomFactor = (0.0 - (p.r + p.g + p.b) / 3.0) * "
     "magnification;\n"
     "        vec2 t = texture_coordinate1;\n"
...
static char *ShaderHacks_1(char *shader, char *Tmp, int *tmpsize) {
  ...
  for (int i = 0; i < sizeof(gl4es_hacks_1) / sizeof(gl4es_hacks_1[0]); i += 2)
    if (strstr(Tmp, gl4es_hacks_1[i])) {
      ...
      Tmp = gl4es_inplace_replace_simple(Tmp, tmpsize, gl4es_hacks_1[i],
                                         gl4es_hacks_1[i + 1]);
    }
  return Tmp;
}
...
static char *ShaderHacks_2_1(char *shader, char *Tmp, int *tmpsize, int i) {
  ...
  if (!m)
    return Tmp; // main signature not found
  if ((uintptr_t)p > (uintptr_t)m)
    return Tmp; // main is before, aborting...
  ...
  Tmp = gl4es_inplace_replace_simple(Tmp, tmpsize, gl4es_sign_2[i],
                                     gl4es_hacks_2_1[i]);
  return Tmp;
}
...
char *ShaderHacks(char *shader) {
  ...
  for (int i = 0; i < sizeof(gl4es_hacks) / sizeof(gl4es_hacks[0]); ++i) {
    ...
    if (strstr(Tmp, f)) {
      ...
      for (int j = 0; j < n; j += 2) {
        if (j)
          f = gl4es_hacks[i].next[j - 1];
        Tmp = gl4es_inplace_replace_simple(Tmp, &tmpsize, f,
                                           gl4es_hacks[i].next[j]);
      }
    }
  }
  return Tmp;
