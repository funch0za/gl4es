...
typedef struct {
  GLboolean line_stipple, auto_normal, normalize, normal_rescale, lighting,
      alpha_test, fog, color_material, blend, cull_face, color_sum, depth_test,
      stencil_test, pointsprite, point_size, texgen_s[MAX_TEX],
      texgen_t[MAX_TEX], texgen_r[MAX_TEX], texgen_q[MAX_TEX],
      plane[MAX_CLIP_PLANES], light[MAX_LIGHT], map1_color4, map1_index,
      map1_normal, map1_texture1, map1_texture2, map1_texture3, map1_texture4,
      map1_vertex3, map1_vertex4, map2_color4, map2_index, map2_normal,
      map2_texture1, map2_texture2, map2_texture3, map2_texture4, map2_vertex3,
      map2_vertex4, color_logic_op, line_smooth, point_smooth, polyfill_offset,
      vertex_arb, vertex_two_side_arb, fragment_arb, multisample,
      sample_coverage, sample_alpha_to_one, sample_alpha_to_coverage;
...
typedef struct {
  GLuint unpack_row_length, unpack_skip_pixels, unpack_skip_rows,
      unpack_image_height;
  ...
  gltexture_t
      *bound[MAX_TEX][ENABLED_TEXTURE_LAST]; // TODO: this should be shared
...
typedef struct {
  map_state_t *vertex3, *vertex4, *index, *color4, *normal, *texture1,
...
typedef struct glsl_s {
  float vtx_env_params[MAX_VTX_PROG_ENV_PARAMS *
                       4]; // ARB_vertex_program Program Env Parameters
  float frg_env_params[MAX_FRG_PROG_ENV_PARAMS *
                       4]; // ARB_fragment_program Program Env Parameters
...
typedef struct {
  ...
  GLuint secondarybuffer;  // secondary renderbuffer, if depth_stencil is not
...
typedef struct {
  ...
  GLuint depth;
  GLuint stencil;
  GLuint t_color[MAX_DRAW_BUFFERS]; // type for attachement_0 (GL_NONE,
                                    // GL_TEXTUREXX, GL_RENDERBUFFER)
  GLuint t_depth;
  GLuint t_stencil;
