...
#define clone_gl_pointer(t, s, n)                                              \
  ...
  t.stride = stride;                                                           \
  t.pointer =                                                                  \
      (void *)((char *)pointer + (uintptr_t)((glstate->vao->vertex)            \
                                                 ? glstate->vao->vertex->data  \
                                                 : 0));                        \
  t.real_buffer =                                                              \
      (glstate->vao->vertex) ? glstate->vao->vertex->real_buffer : 0;          \
  t.real_pointer = (glstate->vao->vertex) ? pointer : 0;                       \
  t.normalized = n;                                                            \
  t.divisor = 0
#define break_lockarrays(t)                                                    \
  if (glstate->vao->vertexattrib[t].real_buffer &&                             \
      glstate->vao->locked_mapped[t]) {                                        \
...
void APIENTRY_GL4ES gl4es_glColorPointer(GLint size, GLenum type,
  ...
  DBG((size > 4) ? printf("glColorPointer(%d, %s, %d, %p)\n", size,
                 ...
                 : printf("glColorPointer(%s, %s, %d, %p)\n", PrintEnum(size),
                          PrintEnum(type), stride, pointer);)
  if (!((size > 0 && size <= 4) ||
        (size == GL_BGRA && type == GL_UNSIGNED_BYTE))) {
    errorShim(GL_INVALID_VALUE);
    return;
  }
  noerrorShimNoPurge();
...
void APIENTRY_GL4ES gl4es_glTexCoordPointer(GLint size, GLenum type,
  ...
  DBG(printf("glTexCoordPointer(%d, %s, %d, %p), texture.client=%d\n", size,
             PrintEnum(type), stride, pointer, glstate->texture.client);)
  if (size < 1 || size > 4) {
    errorShim(GL_INVALID_VALUE);
    return;
  }
  ...
  clone_gl_pointer(
      glstate->vao->vertexattrib[ATT_MULTITEXCOORD0 + glstate->texture.client],
      size, GL_FALSE);
}
...
AliasExport(void, glTexCoordPointer, ,
            (GLint size, GLenum type, GLsizei stride, const GLvoid *pointer));
AliasExport(void, glSecondaryColorPointer, ,
            (GLint size, GLenum type, GLsizei stride, const GLvoid *pointer));
AliasExport(void, glFogCoordPointer, ,
            (GLenum type, GLsizei stride, const GLvoid *pointer));
AliasExport(void, glSecondaryColorPointer, EXT,
            (GLint size, GLenum type, GLsizei stride, const GLvoid *pointer));
AliasExport(void, glFogCoordPointer, EXT,
            (GLenum type, GLsizei stride, const GLvoid *pointer));

void APIENTRY_GL4ES gl4es_glInterleavedArrays(GLenum format, GLsizei stride,
                                              const GLvoid *pointer) {
  DBG(printf("glInterleavedArrays(%s, %d, %p)\n", PrintEnum(format), stride,
             pointer);)
  ...
  if (!stride)
    stride = tex * gl_sizeof(tf) + color * gl_sizeof(cf) +
             normal * gl_sizeof(nf) + vert * gl_sizeof(vf);
  if (tex) {
...
AliasExport(void, glInterleavedArrays, ,
...
void APIENTRY_GL4ES gl4es_glEnd(void) {
  ...
  // check if TEXTUREx is activate and no TexCoord (or texgen), in that case,
  // create a dummy one base on glstate->..
  for (int a = 0; a < hardext.maxtex; a++)
    if ((hardext.esversion == 1) && glstate->enable.texture[a] &&
      ...
      rlMultiTexCoord4f(glstate->list.active, GL_TEXTURE0 + a,
                        glstate->texcoord[a][0], glstate->texcoord[a][1],
  ...
  if (withColor)
    gl4es_glColor4f(glstate->color[0], glstate->color[1], glstate->color[2],
                    glstate->color[3]);
  noerrorShim();
}
...
void APIENTRY_GL4ES gl4es_glNormal3f(GLfloat nx, GLfloat ny, GLfloat nz) {
  if (glstate->list.active) {
    if (glstate->list.active->stage != STAGE_DRAW) {
      ...
      } else if (glstate->list.pending &&
                 glstate->list.active->stage == STAGE_POSTDRAW) {
...
void APIENTRY_GL4ES gl4es_glVertex4f(GLfloat x, GLfloat y, GLfloat z,
                                     GLfloat w) {
  if (glstate->list.active) {
...
void APIENTRY_GL4ES gl4es_glTexCoord4f(GLfloat s, GLfloat t, GLfloat r,
                                       GLfloat q) {
  if (glstate->list.active) {
    if (glstate->list.pending)
      gl4es_flush();
    else {
      // test if called between glBegin / glEnd but Texture is not active and
      // not using a program. In that case, ignore the call
      if (hardext.esversion == 1 || glstate->glsl->program ||
          (glstate->list.begin &&
           (glstate->list.compiling || glstate->enable.texture[0])))
...
void APIENTRY_GL4ES gl4es_glMultiTexCoord4f(GLenum target, GLfloat s, GLfloat t,
  ...
  if (glstate->list.active) {
    if (glstate->list.pending)
      ...
      // test if called between glBegin / glEnd but Texture is not active. In
      // that case, ignore the call
      if (hardext.esversion == 1 ||
          (glstate->list.begin &&
           (glstate->list.compiling ||
            glstate->enable.texture[target - GL_TEXTURE0])))
...
AliasExport(void, glMultiTexCoord4f, ARB,
            (GLenum target, GLfloat s, GLfloat t, GLfloat r, GLfloat q));
...
// TODO: between a lock and unlock, I can assume the array pointers are
// unchanged so I can build a renderlist_t on the first call and hold onto it
...
void APIENTRY_GL4ES gl4es_glUnlockArrays(void) {
  if (globals4es.usevbo > 1 && glstate->vao->locked == globals4es.usevbo)
    UnBuffer();
...
void ToBuffer(int first, int count) {
  // this is hacky. Only the fpe VA should be treated here (but then, the
  // consistancy check is a bit more difficult to do)
  if (count < 13)
    return; // no VBO for smallest ones (4 triangles)
  glstate->vao->locked = globals4es.usevbo;
  // Strategy: compile only VA that are interleaved. So only 1 "Buffer" is
  // compiled. Out of the buffer VA are not compiled That should works with
  // Quake3 engine that expect only Vertices array to be Compiled, but still
  // allow to build more complex arrays
  ...
  if (!stride)
    stride = gl_sizeof(glstate->vao->vertexattrib[ATT_VERTEX].type) *
...
// move "master" data if there
  ...
  gles_glBufferSubData(GL_ARRAY_BUFFER, ptr, stride * count,
                       (void *)(master + first * stride));
#endif
  for (int i = 0; i < NB_VA; i++) {
    if (glstate->vao->vertexattrib[i].enabled) {
      uintptr_t p = (uintptr_t)glstate->vao->vertexattrib[i].pointer;
      if (p >= master && p < master + stride) {
        glstate->vao->vertexattrib[i].real_pointer =
            (void *)(p - master - first * stride);
  ...
  // count, glstate->vao->vertexattrib[ATT_VERTEX].real_pointer,
  // PrintEnum(glstate->vao->vertexattrib[ATT_VERTEX].type),
  // glstate->vao->vertexattrib[ATT_VERTEX].size,
  // glstate->vao->vertexattrib[ATT_VERTEX].stride);
  //  unbind the buffer
...
void APIENTRY_GL4ES gl4es_glNewList(GLuint list, GLenum mode) {
  ...
  // TODO: if glstate->list.active is already defined, we probably need to clean
  // up here
  glstate->list.active = alloc_renderlist();
...
void APIENTRY_GL4ES gl4es_glCallList(GLuint list) {
  ...
  if (glstate->list.active) {
    glstate->list.active =
        append_calllist(glstate->list.active, gl4es_glGetList(list));
    return;
  }
...
void APIENTRY_GL4ES gl4es_glCallLists(GLsizei n, GLenum type,
                                      const GLvoid *lists) {
...
void gl4es_flush() {
  ...
  // flush internal list
  renderlist_t *mylist =
      glstate->list.active ? extend_renderlist(glstate->list.active) : NULL;
  if (mylist) {
    glstate->list.active = NULL;
    glstate->list.pending = 0;
    mylist = end_renderlist(mylist);
...
void APIENTRY_GL4ES gl4es_glIndexPointer(GLenum type, GLsizei stride,
                                         const GLvoid *pointer) {
...
AliasExport(void, glIndexPointer, ,
            (GLenum type, GLsizei stride, const GLvoid *pointer));

void APIENTRY_GL4ES gl4es_glEdgeFlagPointer(GLsizei stride,
                                            const GLvoid *pointer) {
  static bool warning = false;
  if (!warning) {
