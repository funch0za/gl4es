...
void APIENTRY_GL4ES gl4es_glMap2d(GLenum target, GLdouble u1, GLdouble u2,
  ...
  map_switch(2);
  map->points =
      copy_eval_double2(target, ustride, uorder, vstride, vorder, points);
}
...
void APIENTRY_GL4ES gl4es_glMap2f(GLenum target, GLfloat u1, GLfloat u2,
  ...
  map_switch(2);
  map->points =
      copy_eval_float2(target, ustride, uorder, vstride, vorder, points);
  ;
}
...
#define iter_maps(d, code)                                                     \
  ...
  p_map(d, texture4, glTexCoord4f, code) else p_map(                           \
      d, texture3, glTexCoord3f,                                               \
      code) else p_map(d, texture2, glTexCoord2f,                              \
                       code) else p_map(d, texture1, glTexCoord1f, code);      \
...
void APIENTRY_GL4ES gl4es_glEvalCoord1f(GLfloat u) {
  ...
  iter_maps(1, GLfloat uu = (u - map->u._1) * map->u.d;
            _math_horner_bezier_curve((GLfloat *)map->points, out, uu,
                                      map->width, map->u.order);)
}
...
void APIENTRY_GL4ES gl4es_glEvalCoord2f(GLfloat u, GLfloat v) {
  ...
  GLfloat out[4];
  iter_maps(
      2, GLfloat uu = (u - map->u._1) * map->u.d;
      ...
      } else _math_horner_bezier_surf((GLfloat *)map->points, out, uu, vv,
                                      map->width, map->u.order, map->v.order);)
}
...
void APIENTRY_GL4ES gl4es_glMapGrid2f(GLint un, GLfloat u1, GLfloat u2,
  ...
  glstate->map_grid[0]._1 = u1;
  glstate->map_grid[0]._2 = u2;
  glstate->map_grid[0].d = (glstate->map_grid[0]._2 - glstate->map_grid[0]._1) /
                           glstate->map_grid[0].n;
  glstate->map_grid[1].n = vn;
  glstate->map_grid[1]._1 = v1;
  glstate->map_grid[1]._2 = v2;
  glstate->map_grid[1].d = (glstate->map_grid[1]._2 - glstate->map_grid[1]._1) /
                           glstate->map_grid[1].n;
}
...
void APIENTRY_GL4ES gl4es_glEvalMesh2(GLenum mode, GLint i1, GLint i2, GLint j1,
                                      GLint j2) {
  GLenum renderMode = eval_mesh_prep(mode);
  if (!renderMode) {
    errorShim(GL_INVALID_ENUM);
...
#define GL_GET_MAP(t, type)                                                    \
  void APIENTRY_GL4ES gl4es_glGetMap##t##v(GLenum target, GLenum query,        \
                                           type *v) {                          \
    noerrorShim();                                                             \
