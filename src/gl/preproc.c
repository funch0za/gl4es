...
eTokenType NextToken(char **p, uToken *tok) {
  ...
  case '9':
    ...
    if (nextc == 'x' && nb == 0) {
      ...
      (*p)++;
      nextc = **p;
      while ((nextc >= '0' && nextc <= '9') || (nextc >= 'a' && nextc <= 'f') ||
             (nextc >= 'A' && nextc <= 'F')) {
        nb = nb * 16;
        if (nextc >= '0' && nextc <= '9')
          nb += nextc - '0';
...
char *preproc(const char *code, int keepcomments, int gl_es, extensions_t *exts,
  ...
  khash_t(alldefine) *alldefines = kh_init(
      alldefine); // will conatin all defines, even the one without int inside
  ...
  while (tok.type != TK_NULL) {
    ...
    if (tok.type != TK_NULL) {
      ...
      case 100:
        ...
        break;
      case 120:
        status = 100; // continue comment, what ever follow (NewLine or anything
                      // else)
        break;
      ...
      case 310:
        if (tok.type == TK_SPACE)
          ...
          if (gl_es && (strcmp(tok.str, "GL_ES") == 0))
            v = 1;
          else if (kh_get(alldefine, alldefines, tok.str) !=
                   kh_end(alldefines)) {
            v = 0;
        ...
        if (tok.type == TK_SPACE) {
          ...
          if (state != -1) {
            if (exts->size == exts->cap) {
              exts->cap += 4;
              exts->ext = (extension_t *)realloc(
                  exts->ext, sizeof(extension_t) * exts->cap);
            }
      ...
      case 630:
        if (tok.type == TK_SPACE) {
          ...
          status =
              0; // ok, define added to collection, left the line as-is anyway
        } else {
          status = 399; // fallback
        }
        break;
      ...
      case 710:
        if (tok.type == TK_SPACE) {
          ...
          if (!indefined)
            status = 0; // ok... no handling of #defined for now, so just write
                        // through the line
        }               /* else {
  ...
  kh_foreach_value(alldefines, defname, free(defname);)
