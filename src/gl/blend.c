...
void APIENTRY_GL4ES gl4es_glBlendColor(GLclampf red, GLclampf green,
  ...
  if (glstate->blend_color[0] == red && glstate->blend_color[1] == green &&
      glstate->blend_color[2] == blue && glstate->blend_color[3] == alpha)
    return;
...
AliasExport(void, glBlendColor, EXT,
            (GLclampf red, GLclampf green, GLclampf blue, GLclampf alpha));
AliasExport(void, glBlendColor, ARB,
            (GLclampf red, GLclampf green, GLclampf blue, GLclampf alpha));
...
void APIENTRY_GL4ES gl4es_glBlendFuncSeparate(GLenum sfactorRGB,
  ...
  if (sfactorRGB == glstate->blendsfactorrgb &&
      dfactorRGB == glstate->blenddfactorrgb &&
      sfactorAlpha == glstate->blendsfactoralpha &&
      dfactorAlpha == glstate->blenddfactoralpha)
    return; // no change...
  ...
  if (globals4es.shaderblend) {
    ...
    if (gles_glBlendFuncSeparate == NULL) {
      // some fallback function to have better rendering with SDL2, better then
      // nothing...
      ...
      else if (sfactorRGB == GL_SRC_ALPHA && dfactorRGB == GL_ONE &&
               sfactorAlpha == GL_ZERO && dfactorAlpha == GL_ONE)
        gl4es_glBlendFunc(GL_SRC_ALPHA, GL_ONE);
      else if (sfactorRGB == GL_ZERO && dfactorRGB == GL_SRC_COLOR &&
               sfactorAlpha == GL_ZERO && dfactorAlpha == GL_ONE)
...
#endif
      gles_glBlendFuncSeparate(sfactorRGB, dfactorRGB, sfactorAlpha,
                               dfactorAlpha);
  }
...
void APIENTRY_GL4ES gl4es_glBlendEquationSeparate(GLenum modeRGB,
  ...
  if (glstate->blendeqrgb == modeRGB && glstate->blendeqalpha == modeA)
    return;
...
void APIENTRY_GL4ES gl4es_glBlendFunc(GLenum sfactor, GLenum dfactor) {
  ...
  if (sfactor == glstate->blendsfactorrgb &&
      dfactor == glstate->blenddfactorrgb &&
      sfactor == glstate->blendsfactoralpha &&
      dfactor == glstate->blenddfactoralpha)
    return; // already set
  ...
  if (globals4es.shaderblend) {
    ...
    if ((globals4es.blendhack) && (sfactor == GL_SRC_ALPHA) &&
        (dfactor == GL_ONE)) {
      // special case, as seen in Xash3D, but it breaks torus_trooper, so behind
      // a parameter
      sfactor = GL_ONE;
    }
#ifdef ODROID
...
void APIENTRY_GL4ES gl4es_glBlendEquation(GLenum mode) {
  ...
  if (glstate->blendeqrgb == mode && glstate->blendeqalpha == mode)
    return;
