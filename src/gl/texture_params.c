...
void tex_coord_rect_arb(GLfloat *tex, int stride, GLsizei len, GLsizei width,
                        GLsizei height) {
  if (!tex || !width || !height)
    return;
...
void tex_coord_npot(GLfloat *tex, GLsizei len, GLsizei width, GLsizei height,
...
int tex_setup_needchange(GLuint itarget) {
  if (hardext.esversion > 1)
    return 0; // no text ajustement on ES2
  ...
  if ((itarget == ENABLED_TEXTURE_RECTANGLE) ||
      ...
      (hardext.esversion == 1 && !globals4es.texmat &&
...
void tex_setup_texcoord(GLuint len, int changes, GLuint itarget,
  ...
  if (changes) {
    ...
    if (glstate->helper_texlen[texunit] < len) {
      if (glstate->helper_tex[texunit])
        free(glstate->helper_tex[texunit]);
    ...
    // Normalize if needed
    if (itarget == ENABLED_TEXTURE_RECTANGLE)
      tex_coord_rect_arb(glstate->helper_tex[texunit], 4, len, bound->width,
                         bound->height);
    ...
    // NPOT adjust
    if (bound->adjust)
      tex_coord_npot(glstate->helper_tex[texunit], len, bound->width,
...
gltexture_t *gl4es_getTexture(GLenum target, GLuint texture) {
  ...
  if (texture == 0)
    return glstate->texture
        .zero; // texture 0 is a texture mostly like any other... it is not
               // "unbind" texture in fact, but it's not shared
  int ret;
  khint_t k;
...
void APIENTRY_GL4ES gl4es_glBindTexture(GLenum target, GLuint texture) {
  noerrorShim();
  DBG(printf("glBindTexture(%s, %u), active=%i, client=%i, list.active=%p "
             "(compiling=%d, pending=%d)\n",
             PrintEnum(target), texture, glstate->texture.active,
             glstate->texture.client, glstate->list.active,
             glstate->list.compiling, glstate->list.pending);)
  if ((target != GL_PROXY_TEXTURE_2D) && glstate->list.compiling &&
    ...
    switch (target) {
    // cube map are immediately bound, other are defered and will be applied
    // with realize_bound or realize_textures
...
GLenum get_texture_min_filter(gltexture_t *texture, glsampler_t *sampler) {
  ...
  if (texture->valid &&
      (texture->type == GL_FLOAT || texture->type == GL_HALF_FLOAT_OES)) {
...
GLenum get_texture_wrap(GLenum wrap, gltexture_t *texture) {
  ...
  case GL_MIRRORED_REPEAT_OES:
    if (globals4es.defaultwrap == 2 && hardext.npot < 3 && !texture->valid)
      ...
      wrap =
          GL_CLAMP_TO_EDGE; // repeat is not support on NPOT with limited_npot
    }
    break;
  }
  return wrap;
...
void APIENTRY_GL4ES gl4es_glTexParameterfv(GLenum target, GLenum pname,
  ...
  DBG(printf("glTexParameterfv(%s, %s, [%f(%s)...])\n", PrintEnum(target),
             PrintEnum(pname), params[0], PrintEnum(params[0]));)
...
GLboolean APIENTRY_GL4ES gl4es_glAreTexturesResident(GLsizei n,
  ...
  *params = 0;
  ...
  if (!getSamplerParameterfv(&bound->sampler, pname, params)) {
    ...
    case GL_TEXTURE_COMPRESSED_IMAGE_SIZE:
      if ((bound->orig_internal == GL_COMPRESSED_RGB) ||
          (bound->orig_internal == GL_COMPRESSED_RGBA)) {
        int w = nlevel((bound->width >> level), 2); // DXT works on 4x4
                                                    // blocks...
        ...
        if (bound->orig_internal == GL_COMPRESSED_RGB ||
            bound->wanted_internal ==
                GL_COMPRESSED_RGBA_S3TC_DXT1_EXT) // DXT1, 64bits (i.e. size=8)
                                                  // for a 4x4 block
          (*params) = (w * h) / 2;
    ...
    case GL_TEXTURE_INTENSITY_SIZE:
      if (bound)
        (*params) = 32; // is it correct ? GLES doesn't store Intensity... Shall
                        // I return 0 instead? Or fake it and return 8?
      else
        (*params) = 0;
      break;
    case GL_TEXTURE_LUMINANCE_SIZE:
      (*params) = (bound->orig_internal == GL_LUMINANCE ||
                   bound->orig_internal == GL_LUMINANCE_ALPHA)
                      ? 8
                      : 0;
      break;
    case GL_TEXTURE_MAX_ANISOTROPY:
...
void realize_bound(int TMU, GLenum target) {
  ...
  DBG(printf("realize_bound(%d, %s), glsate->actual_tex2d[%d]=%u / %u\n", TMU,
             PrintEnum(target), TMU, glstate->actual_tex2d[TMU], t);)
#ifdef TEXSTREAM
  LOAD_GLES(glEnable);
  LOAD_GLES(glDisable);
#endif
...
void realize_1texture(GLenum target, int wantedTMU, gltexture_t *tex,
                      glsampler_t *sampler) {
  DBG(printf("realize_1texture(%s, %d, %p[%u], %p)\n", PrintEnum(target),
             wantedTMU, tex, tex->glname, sampler);)
  LOAD_GLES(glActiveTexture);
  ...
  if (tex->actual.mag_filter != param) {
    ...
    DBG(printf("Adjusting %s[%d]:Texture[%u].mag_filter = %s (min=%s/%s)\n",
               PrintEnum(target), TMU, tex->glname, PrintEnum(param),
               PrintEnum(sampler->min_filter),
               PrintEnum(tex->actual.min_filter));)
    if (glstate->gleshard->active != TMU) {
      glstate->gleshard->active = TMU;
      gles_glActiveTexture(GL_TEXTURE0 + TMU);
    }
  ...
  if (tex->actual.wrap_s != param) {
    ...
    DBG(printf("Adjusting %s[%d]:Texture[%u].wrap_s = %s\n", PrintEnum(target),
               TMU, tex->glname, PrintEnum(param));)
    if (glstate->gleshard->active != TMU) {
      glstate->gleshard->active = TMU;
      gles_glActiveTexture(GL_TEXTURE0 + TMU);
    }
...
void realize_textures(int drawing) {
  ...
  void gles_glTexParameteri(
      glTexParameteri_ARG_EXPAND); // LOAD_GLES(glTexParameteri);
#ifdef TEXSTREAM
  DBG(printf("realize_textures(%d), glstate->bound_changed=%d, "
             "glstate->enable.texture[0]=%X glsate->actual_tex2d[0]=%u / "
             "glstate->bound_stream[0]=%u\n",
             drawing, glstate->bound_changed, glstate->enable.texture[0],
             glstate->actual_tex2d[0], glstate->bound_stream[0]);)
#else
  DBG(printf("realize_textures(%d), glstate->bound_changed=%d, "
             "glstate->enable.texture[0]=%X glsate->actual_tex2d[0]=%u\n",
             drawing, glstate->bound_changed, glstate->enable.texture[0],
             glstate->actual_tex2d[0]);)
#endif
  for (int i = 0; i < glstate->bound_changed; i++) {
    ...
    if (tgt != ENABLED_CUBE_MAP) { // CUBE MAP are immediately bound
      ...
      if (t != glstate->actual_tex2d[i]
        ...
        DBG(printf("Binding %s/%s[%d]:Texture[%u] (sampler[%d]=%p)\n",
                   PrintEnum(to_target(tgt)), PrintEnum(target), i, t, i,
                   glstate->samplers.sampler[i]);)
        gles_glBindTexture(GL_TEXTURE_2D, t);
        glstate->actual_tex2d[i] = t;
      }
    }
    ...
    if (drawing) {
      if ((globals4es.automipmap == 3) ||
          ((globals4es.automipmap == 1) && (tex->mipmap_auto == 0)) ||
          (tex->compressed && (tex->mipmap_auto == 0)))
        tex->mipmap_need = 0;
      else
        tex->mipmap_need = (is_mipmap_needed(&tex->sampler) &&
                            (hardext.esversion != 1) && !tex->npot)
                               ? 1
                               : 0;
      if (tex->mipmap_need && !tex->mipmap_done) {
        if (!tex->mipmap_auto) {
          // should check if glGenerateMipmap exist, and fall back to no mipmap
          // if not
          LOAD_GLES2_OR_OES(glGenerateMipmap);
...
AliasExport(void, glGetTexLevelParameterfv, ,
            (GLenum target, GLint level, GLenum pname, GLint *params));
AliasExport(GLboolean, glAreTexturesResident, ,
