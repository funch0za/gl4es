...
static void free_renderbuffer(glrenderbuffer_t *rend) {
  ...
  // the texture will be free by the free of the texture list, as it's
  // referenced there...
  free(rend);
}
...
static void free_texture(gltexture_t *tex) {
  ...
  if (tex->data)
    free(tex->data);
  // renderbuffer linked to this texture will be freed by the free_renderbuffer
  // function.
  free(tex);
}
...
void CopyGLEShard(void *dst, const void *src) {
  if (!dst || !src)
    return;
  memcpy(((glstate_t *)dst)->gleshard, ((const glstate_t *)src)->gleshard,
         sizeof(gleshard_t));
}

void *NewGLState(void *shared_glstate, int es2only) {
  glstate_t *glstate = (shared_glstate != DEFAULT_STATE)
                           ? ((glstate_t *)calloc(1, sizeof(glstate_t)))
                           : &default_glstate;
  ...
  if (shared_glstate) {
    ...
    // glstate->gleshard = copy_state->gleshard; // Not shared (at least not the
    // VA)
  ...
  // glsl
  glstate->gleshard =
      (gleshard_t *)calloc(1, sizeof(gleshard_t)); // Not shared!
  if (!shared_glstate) {
  ...
  */
  ...
  // init the textures
  {
    ...
    glstate->texture.zero = tex = kh_value(list, k) =
    malloc(sizeof(gltexture_t));*/
  ...
  glstate->light.ambient[0] = glstate->light.ambient[1] =
      glstate->light.ambient[2] = 0.2f;
  glstate->light.ambient[3] = 1.0f;
  glstate->light.lights[0].diffuse[0] = glstate->light.lights[0].diffuse[1] =
      glstate->light.lights[0].diffuse[2] =
          glstate->light.lights[0].diffuse[3] = 1.0f;
  memcpy(glstate->light.lights[0].specular, glstate->light.lights[0].diffuse,
         4 * sizeof(GLfloat));
  ...
  // Materials
  glstate->material.front.ambient[0] = glstate->material.front.ambient[1] =
      glstate->material.front.ambient[2] = 0.2f;
  glstate->material.front.ambient[3] = 1.0f;
  glstate->material.front.diffuse[0] = glstate->material.front.diffuse[1] =
  ...
  glstate->pointsprite.sizeMax =
      32.0f; // spec indicate 1., but it seems it's set to hardware limit, so
             // putting 32...
  ...
  for (int i = 0; i < hardext.maxtex; i++) {
    ...
    glstate->texenv[i].env.src0_rgb = glstate->texenv[i].env.src0_alpha =
        GL_TEXTURE;
    glstate->texenv[i].env.src1_rgb = glstate->texenv[i].env.src1_alpha =
        GL_PREVIOUS;
    glstate->texenv[i].env.src2_rgb = glstate->texenv[i].env.src2_alpha =
        GL_CONSTANT;
    glstate->texenv[i].env.op0_rgb = glstate->texenv[i].env.op1_rgb =
        GL_SRC_COLOR;
    glstate->texenv[i].env.op2_rgb = glstate->texenv[i].env.op0_alpha =
        glstate->texenv[i].env.op1_alpha = glstate->texenv[i].env.op2_alpha =
            GL_SRC_ALPHA;
  }
  // TexGen
  for (int i = 0; i < hardext.maxtex; i++) {
...
#endif
  // printf("LIBGL: Implementation Read is %s/%s\n", PrintEnum(glstate->readf),
  // PrintEnum(glstate->readt));
...
#define free_hashmap(T, N, K, F)                                               \
  ...
  if (!state->shared_cnt) {
    ...
    free_hashmap(glrenderbuffer_t, fbo.renderbufferlist, renderbufferlist_t,
                 free_renderbuffer);
    free_hashmap(glframebuffer_t, fbo.framebufferlist, framebufferlist_t,
                 free_framebuffer);
...
#undef freemap
  // free active list
  if (!state->shared_cnt && state->list.active)
...
void ActivateGLState(void *new_glstate) {
  glstate_t *newstate =
      (new_glstate) ? (glstate_t *)new_glstate : &default_glstate;
  if (glstate == newstate)
    return; // same state, nothing to do
            // check if viewport is correct
#if defined(AMIGAOS4) || defined(__EMSCRIPTEN__)
  if (glstate ||
      newstate != &default_glstate) // avoid getting gles info with no context
#endif
    if (new_glstate && (newstate->raster.viewport.width == 0 ||
                        newstate->raster.viewport.height == 0)) {
      LOAD_GLES(glGetIntegerv);
...
void gl_close() { DeleteGLState(DEFAULT_STATE); }
