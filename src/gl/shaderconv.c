...
const builtin_attrib_t builtin_attrib[] = {
    ...
    {"gl_MultiTexCoord0", "_gl4es_MultiTexCoord0", "vec4", "highp",
     ARB_MULTITEXCOORD0},
    {"gl_MultiTexCoord1", "_gl4es_MultiTexCoord1", "vec4", "highp",
     ARB_MULTITEXCOORD1},
    {"gl_MultiTexCoord2", "_gl4es_MultiTexCoord2", "vec4", "highp",
     ARB_MULTITEXCOORD2},
    {"gl_MultiTexCoord3", "_gl4es_MultiTexCoord3", "vec4", "highp",
     ARB_MULTITEXCOORD3},
    {"gl_MultiTexCoord4", "_gl4es_MultiTexCoord4", "vec4", "highp",
     ARB_MULTITEXCOORD4},
    {"gl_MultiTexCoord5", "_gl4es_MultiTexCoord5", "vec4", "highp",
     ARB_MULTITEXCOORD5},
    {"gl_MultiTexCoord6", "_gl4es_MultiTexCoord6", "vec4", "highp",
     ARB_MULTITEXCOORD6},
    {"gl_MultiTexCoord7", "_gl4es_MultiTexCoord7", "vec4", "highp",
     ARB_MULTITEXCOORD7},
    {"gl_MultiTexCoord8", "_gl4es_MultiTexCoord8", "vec4", "highp",
     ARB_MULTITEXCOORD8},
    {"gl_MultiTexCoord9", "_gl4es_MultiTexCoord9", "vec4", "highp",
     ARB_MULTITEXCOORD9},
    {"gl_MultiTexCoord10", "_gl4es_MultiTexCoord10", "vec4", "highp",
     ARB_MULTITEXCOORD10},
    {"gl_MultiTexCoord11", "_gl4es_MultiTexCoord11", "vec4", "highp",
     ARB_MULTITEXCOORD11},
    {"gl_MultiTexCoord12", "_gl4es_MultiTexCoord12", "vec4", "highp",
     ARB_MULTITEXCOORD12},
    {"gl_MultiTexCoord13", "_gl4es_MultiTexCoord13", "vec4", "highp",
     ARB_MULTITEXCOORD13},
    {"gl_MultiTexCoord14", "_gl4es_MultiTexCoord14", "vec4", "highp",
     ARB_MULTITEXCOORD14},
    {"gl_MultiTexCoord15", "_gl4es_MultiTexCoord15", "vec4", "highp",
     ARB_MULTITEXCOORD15},
    {"gl_SecondaryColor", "_gl4es_SecondaryColor", "vec4", "lowp",
     ARB_SECONDARY},
...
const builtin_matrix_t builtin_matrix[] = {
    {"gl_ModelViewMatrixInverseTranspose", "_gl4es_ITModelViewMatrix", "mat4",
     0, MAT_MV_IT},
    {"gl_ModelViewMatrixInverse", "_gl4es_IModelViewMatrix", "mat4", 0,
     MAT_MV_I},
    {"gl_ModelViewMatrixTranspose", "_gl4es_TModelViewMatrix", "mat4", 0,
     MAT_MV_T},
    {"gl_ModelViewMatrix", "_gl4es_ModelViewMatrix", "mat4", 0, MAT_MV},
    {"gl_ProjectionMatrixInverseTranspose", "_gl4es_ITProjectionMatrix", "mat4",
     0, MAT_P_IT},
    {"gl_ProjectionMatrixInverse", "_gl4es_IProjectionMatrix", "mat4", 0,
     MAT_P_I},
    {"gl_ProjectionMatrixTranspose", "_gl4es_TProjectionMatrix", "mat4", 0,
     MAT_P_T},
    ...
    {"gl_ModelViewProjectionMatrixTranspose",
     "_gl4es_TModelViewProjectionMatrix", "mat4", 0, MAT_MVP_T},
    {"gl_ModelViewProjectionMatrix", "_gl4es_ModelViewProjectionMatrix", "mat4",
     0, MAT_MVP},
    // non standard version to avoid useless array of Matrix Uniform (in case
    // the compiler as issue optimising this)
    ...
    {"gl_TextureMatrixInverseTranspose", "_gl4es_ITTextureMatrix", "mat4", 1,
     MAT_T0_IT},
...
static const char *gl4es_LightSourceParametersSource =
    ...
    "   vec4 specular;\n"
    "   vec4 position;\n"
    "   vec4 halfVector;\n" // halfVector = normalize(normalize(position) +
                            // vec3(0,0,1) if vbs==FALSE)
    "   vec3 spotDirection;\n"
    "   float spotExponent;\n"
    "   float spotCutoff;\n"
    "   float spotCosCutoff;\n"
...
static const char *gl4es_ClipVertex_clip =
    ...
    " || any(greaterThanEqual(gl4es_ClipVertex.xyz, "
    "vec3(gl4es_ClipVertex.w)))) discard;\n";

static const char *gl_TexCoordSource = "gl_TexCoord[";

static const char *gl_TexMatrixSources[] = {
    "gl_TextureMatrixInverseTranspose[", "gl_TextureMatrixInverse[",
...
static const char *texture2DGradAlt =
    "vec4 _gl4es_texture2DGrad(sampler2D sampler, vec2 coord, vec2 dPdx, vec2 "
    "dPdy) {\n"
    " return texture2D(sampler, coord);\n"
    "}\n";

static const char *texture2DProjGradAlt =
    "vec4 _gl4es_texture2DProjGrad(sampler2D sampler, vec3 coord, vec2 dPdx, "
    "vec2 dPdy) {\n"
    " return texture2DProj(sampler, coord);\n"
    "}\n"
    "vec4 _gl4es_texture2DProjGrad(sampler2D sampler, vec4 coord, vec2 dPdx, "
    "vec2 dPdy) {\n"
    " return texture2DProj(sampler, coord);\n"
    "}\n";
static const char *textureCubeGradAlt =
    "vec4 _gl4es_textureCubeGrad(samplerCube sampler, vec3 coord, vec2 dPdx, "
    "vec2 dPdy) {\n"
    " return textureCube(sampler, coord);\n"
    "}\n";
...
static const char *gl4es_Samplers1D_uniform =
    "uniform sampler2D _gl4es_Sampler1D_";
static const char *gl4es_Samplers2D_uniform =
    "uniform sampler2D _gl4es_Sampler2D_";
static const char *gl4es_Samplers3D_uniform =
    "uniform sampler2D _gl4es_Sampler3D_";
static const char *gl4es_SamplersCube_uniform =
...
#define ShadAppend(S) Tmp = gl4es_append(Tmp, &tmpsize, S)
  ...
  if (!fpeShader) {
    ...
    // preproc first
    pBuffer = preproc(pHacked, comments, globals4es.shadernogles, &exts,
                      &versionString);
    ...
    if (strstr(pBuffer, "\nprecision")) {
      ...
      pBuffer =
          gl4es_inplace_replace(pBuffer, &sz, "\nprecision", "\n//precision");
    }
  ...
  // const char* GLESUseFragHighp = "#extension GL_OES_fragment_precision_high :
  // enable\n"; // is this needed?
  char GLESFullHeader[512];
  int wanthighp = !fpeShader;
  if (wanthighp && !hardext.highp)
    wanthighp = 0;
  int versionHeader = 0;
#if 0
...
#endif
  // sprintf(GLESFullHeader, GLESHeader, (wanthighp && hardext.highp==1 &&
  // !isVertex)?GLESUseFragHighp:"", (wanthighp)?"highp":"mediump",
  ...
  while (strstr(Tmp, "#extension") &&
    ...
    Tmp = gl4es_inplace_insert(gl4es_getline(Tmp, headline - 2), e, Tmp,
                               &tmpsize);
    ++headline;
  }
  ...
  if (fragdepth) {
    /* If #extension is used, it should be placed before the second line of the
     * header. */
    if (hardext.fragdepth)
      Tmp = gl4es_inplace_insert(gl4es_getline(Tmp, 1), GLESUseFragDepth, Tmp,
                                 &tmpsize);
    else
      Tmp = gl4es_inplace_insert(gl4es_getline(Tmp, headline - 1),
                                 GLESFakeFragDepth, Tmp, &tmpsize);
    headline++;
  }

  const char *GLESUseShaderNonConstantGlobalInitialzers =
      "#extension GL_EXT_shader_non_constant_global_initializers : enable\n";
  Tmp = gl4es_inplace_insert(gl4es_getline(Tmp, 1),
                             GLESUseShaderNonConstantGlobalInitialzers, Tmp,
                             &tmpsize);
  ++headline;
  ...
  int derivatives = (strstr(pBuffer, "dFdx(") || strstr(pBuffer, "dFdy(") ||
                     strstr(pBuffer, "fwidth("))
                        ? 1
                        : 0;
  const char *GLESUseDerivative =
      "#extension GL_OES_standard_derivatives : enable\n";
  // complete fake value... A better thing should be use....
  const char *GLESFakeDerivative =
      ...
      "fwidth(vec2 p) {return abs(dFdx(p))+abs(dFdy(p));}\n"
      "vec3 fwidth(vec3 p) {return abs(dFdx(p))+abs(dFdy(p));}\n";
  if (derivatives) {
    /* If #extension is used, it should be placed before the second line of the
     * header. */
    if (hardext.derivatives)
      Tmp = gl4es_inplace_insert(gl4es_getline(Tmp, 1), GLESUseDerivative, Tmp,
                                 &tmpsize);
    else
      Tmp = gl4es_inplace_insert(gl4es_getline(Tmp, headline - 1),
                                 GLESFakeDerivative, Tmp, &tmpsize);
    headline++;
  }
  // check if draw_buffers may be used (no fallback here :( )
  if (hardext.maxdrawbuffers > 1 && strstr(pBuffer, "gl_FragData[")) {
    Tmp = gl4es_inplace_insert(gl4es_getline(Tmp, 1), useEXTDrawBuffers, Tmp,
                               &tmpsize);
  }
  ...
  if (!fpeShader && !globals4es.nointovlhack) {
    if (strstr(Tmp, "pow(") || strstr(Tmp, "pow (")) {
      Tmp = gl4es_inplace_insert(gl4es_getline(Tmp, headline), HackAltPow, Tmp,
                                 &tmpsize);
    }
    if (strstr(Tmp, "max(") || strstr(Tmp, "max (")) {
      Tmp = gl4es_inplace_insert(gl4es_getline(Tmp, headline), HackAltMax, Tmp,
                                 &tmpsize);
    }
    if (strstr(Tmp, "min(") || strstr(Tmp, "min (")) {
      Tmp = gl4es_inplace_insert(gl4es_getline(Tmp, headline), HackAltMin, Tmp,
                                 &tmpsize);
    }
    if (strstr(Tmp, "clamp(") || strstr(Tmp, "clamp (")) {
      Tmp = gl4es_inplace_insert(gl4es_getline(Tmp, headline), HackAltClamp,
                                 Tmp, &tmpsize);
    }
    if (strstr(Tmp, "mod(") || strstr(Tmp, "mod (")) {
      Tmp = gl4es_inplace_insert(gl4es_getline(Tmp, headline), HackAltMod, Tmp,
                                 &tmpsize);
    }
  }
  if (!isVertex && hardext.shaderlod &&
    ...
    const char *GLESUseShaderLod =
        "#extension GL_EXT_shader_texture_lod : enable\n";
    Tmp = gl4es_inplace_insert(gl4es_getline(Tmp, 1), GLESUseShaderLod, Tmp,
                               &tmpsize);
  }
  if (!isVertex && (gl4es_find_string(Tmp, "texture2DLod"))) {
    if (hardext.shaderlod) {
      Tmp = gl4es_inplace_replace(Tmp, &tmpsize, "texture2DLod",
                                  "texture2DLodEXT");
    } else {
      Tmp = gl4es_inplace_replace(Tmp, &tmpsize, "texture2DLod",
                                  "_gl4es_texture2DLod");
      Tmp = gl4es_inplace_insert(gl4es_getline(Tmp, headline), texture2DLodAlt,
                                 Tmp, &tmpsize);
    }
  }
  if (!isVertex && (gl4es_find_string(Tmp, "texture2DProjLod"))) {
    if (hardext.shaderlod) {
      Tmp = gl4es_inplace_replace(Tmp, &tmpsize, "texture2DProjLod",
                                  "texture2DProjLodEXT");
    } else {
      Tmp = gl4es_inplace_replace(Tmp, &tmpsize, "texture2DProjLod",
                                  "_gl4es_texture2DProjLod");
      Tmp = gl4es_inplace_insert(gl4es_getline(Tmp, headline),
                                 texture2DProjLodAlt, Tmp, &tmpsize);
    }
  }
  if (!isVertex && (gl4es_find_string(Tmp, "textureCubeLod"))) {
    if (hardext.shaderlod) {
      if (!hardext.cubelod)
        Tmp = gl4es_inplace_replace(Tmp, &tmpsize, "textureCubeLod",
                                    "textureCubeLodEXT");
    } else {
      Tmp = gl4es_inplace_replace(Tmp, &tmpsize, "textureCubeLod",
                                  "_gl4es_textureCubeLod");
      Tmp = gl4es_inplace_insert(gl4es_getline(Tmp, headline),
                                 textureCubeLodAlt, Tmp, &tmpsize);
    }
  }
  if (!isVertex && (gl4es_find_string(Tmp, "texture2DGradARB"))) {
    if (hardext.shaderlod) {
      Tmp = gl4es_inplace_replace(Tmp, &tmpsize, "texture2DGradARB",
                                  "texture2DGradEXT");
    } else {
      Tmp = gl4es_inplace_replace(Tmp, &tmpsize, "texture2DGradARB",
                                  "_gl4es_texture2DGrad");
      Tmp = gl4es_inplace_insert(gl4es_getline(Tmp, headline), texture2DGradAlt,
                                 Tmp, &tmpsize);
    }
  }
  if (!isVertex && (gl4es_find_string(Tmp, "texture2DProjGradARB"))) {
    if (hardext.shaderlod) {
      Tmp = gl4es_inplace_replace(Tmp, &tmpsize, "texture2DProjGradARB",
                                  "texture2DProjGradEXT");
    } else {
      Tmp = gl4es_inplace_replace(Tmp, &tmpsize, "texture2DProjGradARB",
                                  "_gl4es_texture2DProjGrad");
      Tmp = gl4es_inplace_insert(gl4es_getline(Tmp, headline),
                                 texture2DProjGradAlt, Tmp, &tmpsize);
    }
  }
  if (!isVertex && (gl4es_find_string(Tmp, "textureCubeGradARB"))) {
    if (hardext.shaderlod) {
      if (!hardext.cubelod)
        Tmp = gl4es_inplace_replace(Tmp, &tmpsize, "textureCubeGradARB",
                                    "textureCubeGradEXT");
    } else {
      Tmp = gl4es_inplace_replace(Tmp, &tmpsize, "textureCubeGradARB",
                                  "_gl4es_textureCubeGrad");
      Tmp = gl4es_inplace_insert(gl4es_getline(Tmp, headline),
                                 textureCubeGradAlt, Tmp, &tmpsize);
    }
  }
  ...
  Tmp = gl4es_inplace_replace(Tmp, &tmpsize, "gl_FragDepth",
                              (hardext.fragdepth) ? "gl_FragDepthEXT"
                                                  : "fakeFragDepth");
  // builtin attribs
  if (isVertex) {
    ...
    if (strstr(Tmp, "ftransform(")) {
      ...
      // don't increment headline count, as all variying and attributes should
      // be created before
    }
    ...
    for (int i = 0; i < n; i++) {
      if (strstr(Tmp, builtin_attrib[i].glname)) {
        ...
        Tmp = gl4es_inplace_replace(Tmp, &tmpsize, builtin_attrib[i].glname,
        ...
        Tmp = gl4es_inplace_insert(gl4es_getline(Tmp, headline++), def, Tmp,
                                   &tmpsize);
      }
    }
    if (strstr(Tmp, gl_VertexAttrib)) {
      ...
      for (int i = 0; i < MAX_VATTRIB; ++i) {
        ...
        if (gl4es_find_string(Tmp, gl_VA[i])) {
          ...
          Tmp = gl4es_inplace_insert(gl4es_getline(Tmp, headline++), A, Tmp,
                                     &tmpsize);
        }
      }
    }
  ...
  if (strstr(Tmp, "gl_FrontColor") || need->need_color) {
    ...
    nvarying += 1;
    Tmp = gl4es_inplace_insert(gl4es_getline(Tmp, headline),
                               gl4es_frontColorSource, Tmp, &tmpsize);
    headline += gl4es_countline(gl4es_frontColorSource);
    Tmp = gl4es_inplace_replace(Tmp, &tmpsize, "gl_FrontColor",
                                "_gl4es_FrontColor");
  }
  if (strstr(Tmp, "gl_BackColor") || (need->need_color == 2)) {
    ...
    nvarying += 1;
    Tmp = gl4es_inplace_insert(gl4es_getline(Tmp, headline),
                               gl4es_backColorSource, Tmp, &tmpsize);
    headline += gl4es_countline(gl4es_backColorSource);
    Tmp = gl4es_inplace_replace(Tmp, &tmpsize, "gl_BackColor",
                                "_gl4es_BackColor");
  }
  ...
  if (strstr(Tmp, "gl_FrontSecondaryColor") || need->need_secondary) {
    ...
    nvarying += 1;
    Tmp = gl4es_inplace_insert(gl4es_getline(Tmp, headline),
                               gl4es_frontSecondaryColorSource, Tmp, &tmpsize);
    headline += gl4es_countline(gl4es_frontSecondaryColorSource);
    Tmp = gl4es_inplace_replace(Tmp, &tmpsize, "gl_FrontSecondaryColor",
  ...
  if (strstr(Tmp, "gl_BackSecondaryColor") || (need->need_secondary == 2)) {
    ...
    nvarying += 1;
    Tmp = gl4es_inplace_insert(gl4es_getline(Tmp, headline),
                               gl4es_backSecondaryColorSource, Tmp, &tmpsize);
    headline += gl4es_countline(gl4es_backSecondaryColorSource);
    Tmp = gl4es_inplace_replace(Tmp, &tmpsize, "gl_BackSecondaryColor",
  ...
  if (strstr(Tmp, "gl_FogFragCoord") || need->need_fogcoord) {
    ...
    nvarying += 1;
    Tmp = gl4es_inplace_insert(gl4es_getline(Tmp, headline),
                               gl4es_fogcoordSource, Tmp, &tmpsize);
    headline += gl4es_countline(gl4es_fogcoordSource);
    Tmp = gl4es_inplace_replace(Tmp, &tmpsize, "gl_FogFragCoord",
                                "_gl4es_FogFragCoord");
  }
    ...
    if (notexarray) {
      for (int k = 0; k < ntex + 1; k++) {
        ...
        if (strstr(Tmp, d2)) {
          ...
          Tmp = gl4es_inplace_insert(gl4es_getline(Tmp, headline), d, Tmp,
                                     &tmpsize);
      ...
      Tmp =
          gl4es_inplace_insert(gl4es_getline(Tmp, headline), d, Tmp, &tmpsize);
      headline += gl4es_countline(d);
      Tmp = gl4es_inplace_replace(Tmp, &tmpsize, "gl_TexCoord",
                                  "_gl4es_TexCoord");
  ...
  {
    if (strstr(Tmp, "transpose(") || strstr(Tmp, "transpose ") ||
        strstr(Tmp, "transpose\t")) {
      Tmp = gl4es_inplace_insert(gl4es_getline(Tmp, headline), gl4es_transpose,
                                 Tmp, &tmpsize);
    ...
    // check for builtin matrix uniform...
    {
      ...
      int ntex = -1;
      // Try to determine max Texture matrice used, for each transposed inverse
      // or regular...
      for (int i = 0; i < 4; ++i) {
        char *p = Tmp;
      ...
      for (int i = 0; i < n; i++) {
        if (strstr(Tmp, builtin_matrix[i].glname)) {
          ...
          // replace gl_name by _gl4es_ one
          Tmp = gl4es_inplace_replace(Tmp, &tmpsize, builtin_matrix[i].glname,
  ...
  // Handling of gl_LightSource[x].halfVector =>
  // normalize(gl_LightSource[x].position - gl_Vertex), but what if in the
  // FragShader ?
  /*  if(strstr(Tmp, "halfVector"))
    {
      ...
      while((p=strstr(p, "gl_LightSource["))) {
        ...
        if (p2 && !strncmp(p2, "].halfVector", strlen("].halfVector"))) {
          ...
          sprintf(p5, "normalize(normalize(%s.position.xyz) + vec3(0.,
    0., 1.))", p3); Tmp = gl4es_inplace_replace(Tmp, &tmpsize, p4, p5); p = Tmp;
        } else
          ++p;
      }
    }*/
  ...
  if (strstr(Tmp, "gl_LightSourceParameters") ||
      strstr(Tmp, "gl_LightSource")) {
    Tmp =
        gl4es_inplace_insert(gl4es_getline(Tmp, headline),
                             gl4es_LightSourceParametersSource, Tmp, &tmpsize);
    headline += gl4es_countline(gl4es_LightSourceParametersSource);
    Tmp = gl4es_inplace_replace(Tmp, &tmpsize, "gl_LightSourceParameters",
                                "_gl4es_LightSourceParameters");
  }
  if (strstr(Tmp, "gl_LightModelParameters") || strstr(Tmp, "gl_LightModel")) {
    Tmp = gl4es_inplace_insert(gl4es_getline(Tmp, headline),
                               gl4es_LightModelParametersSource, Tmp, &tmpsize);
    headline += gl4es_countline(gl4es_LightModelParametersSource);
    Tmp = gl4es_inplace_replace(Tmp, &tmpsize, "gl_LightModelParameters",
                                "_gl4es_LightModelParameters");
  }
  if (strstr(Tmp, "gl_LightModelProducts") ||
      strstr(Tmp, "gl_FrontLightModelProduct") ||
      strstr(Tmp, "gl_BackLightModelProduct")) {
    Tmp = gl4es_inplace_insert(gl4es_getline(Tmp, headline),
                               gl4es_LightModelProductsSource, Tmp, &tmpsize);
    headline += gl4es_countline(gl4es_LightModelProductsSource);
    Tmp = gl4es_inplace_replace(Tmp, &tmpsize, "gl_LightModelProducts",
                                "_gl4es_LightModelProducts");
  }
  if (strstr(Tmp, "gl_LightProducts") || strstr(Tmp, "gl_FrontLightProduct") ||
      strstr(Tmp, "gl_BackLightProduct")) {
    Tmp = gl4es_inplace_insert(gl4es_getline(Tmp, headline),
                               gl4es_LightProductsSource, Tmp, &tmpsize);
    headline += gl4es_countline(gl4es_LightProductsSource);
    Tmp = gl4es_inplace_replace(Tmp, &tmpsize, "gl_LightProducts",
                                "_gl4es_LightProducts");
  }
  if (strstr(Tmp, "gl_MaterialParameters ") ||
      (strstr(Tmp, "gl_FrontMaterial")) || strstr(Tmp, "gl_BackMaterial")) {
    Tmp = gl4es_inplace_insert(gl4es_getline(Tmp, headline),
                               gl4es_MaterialParametersSource, Tmp, &tmpsize);
    headline += gl4es_countline(gl4es_MaterialParametersSource);
    Tmp = gl4es_inplace_replace(Tmp, &tmpsize, "gl_MaterialParameters",
                                "_gl4es_MaterialParameters");
  }
  if (strstr(Tmp, "gl_LightSource")) {
    Tmp = gl4es_inplace_replace(Tmp, &tmpsize, "gl_LightSource",
                                "_gl4es_LightSource");
  }
  if (strstr(Tmp, "gl_LightModel"))
    Tmp = gl4es_inplace_replace(Tmp, &tmpsize, "gl_LightModel",
                                "_gl4es_LightModel");
  if (strstr(Tmp, "gl_FrontLightModelProduct"))
    Tmp = gl4es_inplace_replace(Tmp, &tmpsize, "gl_FrontLightModelProduct",
                                "_gl4es_FrontLightModelProduct");
  if (strstr(Tmp, "gl_BackLightModelProduct"))
    Tmp = gl4es_inplace_replace(Tmp, &tmpsize, "gl_BackLightModelProduct",
                                "_gl4es_BackLightModelProduct");
  if (strstr(Tmp, "gl_FrontLightProduct"))
    Tmp = gl4es_inplace_replace(Tmp, &tmpsize, "gl_FrontLightProduct",
                                "_gl4es_FrontLightProduct");
  if (strstr(Tmp, "gl_BackLightProduct"))
    Tmp = gl4es_inplace_replace(Tmp, &tmpsize, "gl_BackLightProduct",
                                "_gl4es_BackLightProduct");
  if (strstr(Tmp, "gl_FrontMaterial"))
    Tmp = gl4es_inplace_replace(Tmp, &tmpsize, "gl_FrontMaterial",
                                "_gl4es_FrontMaterial");
  if (strstr(Tmp, "gl_BackMaterial"))
    Tmp = gl4es_inplace_replace(Tmp, &tmpsize, "gl_BackMaterial",
                                "_gl4es_BackMaterial");
  if (strstr(Tmp, "gl_MaxLights")) {
    Tmp = gl4es_inplace_insert(gl4es_getline(Tmp, 2), gl4es_MaxLightsSource,
                               Tmp, &tmpsize);
    headline += gl4es_countline(gl4es_MaxLightsSource);
    Tmp = gl4es_inplace_replace(Tmp, &tmpsize, "gl_MaxLights",
                                "_gl4es_MaxLights");
  }
  if (strstr(Tmp, "gl_NormalScale")) {
    Tmp = gl4es_inplace_insert(gl4es_getline(Tmp, headline),
                               gl4es_normalscaleSource, Tmp, &tmpsize);
    headline += gl4es_countline(gl4es_normalscaleSource);
    Tmp = gl4es_inplace_replace(Tmp, &tmpsize, "gl_NormalScale",
                                "_gl4es_NormalScale");
  }
  if (strstr(Tmp, "gl_InstanceID") || strstr(Tmp, "gl_InstanceIDARB")) {
    Tmp = gl4es_inplace_insert(gl4es_getline(Tmp, headline), gl4es_instanceID,
                               Tmp, &tmpsize);
    headline += gl4es_countline(gl4es_instanceID);
    Tmp = gl4es_inplace_replace(Tmp, &tmpsize, "gl_InstanceIDARB",
                                "_gl4es_InstanceID");
    Tmp = gl4es_inplace_replace(Tmp, &tmpsize, "gl_InstanceID",
                                "_gl4es_InstanceID");
  }
  if (strstr(Tmp, "gl_ClipPlane")) {
    Tmp = gl4es_inplace_insert(gl4es_getline(Tmp, headline),
                               gl4es_clipplanesSource, Tmp, &tmpsize);
    headline += gl4es_countline(gl4es_clipplanesSource);
    Tmp = gl4es_inplace_replace(Tmp, &tmpsize, "gl_ClipPlane",
                                "_gl4es_ClipPlane");
  }
  if (strstr(Tmp, "gl_MaxClipPlanes")) {
    Tmp = gl4es_inplace_insert(gl4es_getline(Tmp, 2), gl4es_MaxClipPlanesSource,
                               Tmp, &tmpsize);
    headline += gl4es_countline(gl4es_MaxClipPlanesSource);
    Tmp = gl4es_inplace_replace(Tmp, &tmpsize, "gl_MaxClipPlanes",
                                "_gl4es_MaxClipPlanes");
  }

  if (strstr(Tmp, "gl_PointParameters") || strstr(Tmp, "gl_Point")) {
    Tmp = gl4es_inplace_insert(gl4es_getline(Tmp, headline),
                               gl4es_PointSpriteSource, Tmp, &tmpsize);
    headline += gl4es_countline(gl4es_PointSpriteSource);
    Tmp = gl4es_inplace_replace(Tmp, &tmpsize, "gl_PointParameters",
  ...
  if (strstr(Tmp, "gl_FogParameters") || strstr(Tmp, "gl_Fog")) {
    Tmp = gl4es_inplace_insert(gl4es_getline(Tmp, headline),
                               hardext.highp ? gl4es_FogParametersSourceHighp
                                             : gl4es_FogParametersSource,
                               Tmp, &tmpsize);
    headline += gl4es_countline(gl4es_FogParametersSource);
    Tmp = gl4es_inplace_replace(Tmp, &tmpsize, "gl_FogParameters",
  ...
  if (strstr(Tmp, "gl_TextureEnvColor")) {
    Tmp = gl4es_inplace_insert(gl4es_getline(Tmp, headline),
                               gl4es_texenvcolorSource, Tmp, &tmpsize);
    headline += gl4es_countline(gl4es_texenvcolorSource);
    Tmp = gl4es_inplace_replace(Tmp, &tmpsize, "gl_TextureEnvColor",
                                "_gl4es_TextureEnvColor");
  }
  if (strstr(Tmp, "gl_EyePlaneS")) {
    Tmp = gl4es_inplace_insert(gl4es_getline(Tmp, headline),
                               gl4es_texgeneyeSource[0], Tmp, &tmpsize);
    headline += gl4es_countline(gl4es_texgeneyeSource[0]);
    Tmp = gl4es_inplace_replace(Tmp, &tmpsize, "gl_EyePlaneS",
                                "_gl4es_EyePlaneS");
  }
  if (strstr(Tmp, "gl_EyePlaneT")) {
    Tmp = gl4es_inplace_insert(gl4es_getline(Tmp, headline),
                               gl4es_texgeneyeSource[1], Tmp, &tmpsize);
    headline += gl4es_countline(gl4es_texgeneyeSource[1]);
    Tmp = gl4es_inplace_replace(Tmp, &tmpsize, "gl_EyePlaneT",
                                "_gl4es_EyePlaneT");
  }
  if (strstr(Tmp, "gl_EyePlaneR")) {
    Tmp = gl4es_inplace_insert(gl4es_getline(Tmp, headline),
                               gl4es_texgeneyeSource[2], Tmp, &tmpsize);
    headline += gl4es_countline(gl4es_texgeneyeSource[2]);
    Tmp = gl4es_inplace_replace(Tmp, &tmpsize, "gl_EyePlaneR",
                                "_gl4es_EyePlaneR");
  }
  if (strstr(Tmp, "gl_EyePlaneQ")) {
    Tmp = gl4es_inplace_insert(gl4es_getline(Tmp, headline),
                               gl4es_texgeneyeSource[3], Tmp, &tmpsize);
    headline += gl4es_countline(gl4es_texgeneyeSource[3]);
    Tmp = gl4es_inplace_replace(Tmp, &tmpsize, "gl_EyePlaneQ",
                                "_gl4es_EyePlaneQ");
  }
  if (strstr(Tmp, "gl_ObjectPlaneS")) {
    Tmp = gl4es_inplace_insert(gl4es_getline(Tmp, headline),
                               gl4es_texgenobjSource[0], Tmp, &tmpsize);
    headline += gl4es_countline(gl4es_texgenobjSource[0]);
    Tmp = gl4es_inplace_replace(Tmp, &tmpsize, "gl_ObjectPlaneS",
                                "_gl4es_ObjectPlaneS");
  }
  if (strstr(Tmp, "gl_ObjectPlaneT")) {
    Tmp = gl4es_inplace_insert(gl4es_getline(Tmp, headline),
                               gl4es_texgenobjSource[1], Tmp, &tmpsize);
    headline += gl4es_countline(gl4es_texgenobjSource[1]);
    Tmp = gl4es_inplace_replace(Tmp, &tmpsize, "gl_ObjectPlaneT",
                                "_gl4es_ObjectPlaneT");
  }
  if (strstr(Tmp, "gl_ObjectPlaneR")) {
    Tmp = gl4es_inplace_insert(gl4es_getline(Tmp, headline),
                               gl4es_texgenobjSource[2], Tmp, &tmpsize);
    headline += gl4es_countline(gl4es_texgenobjSource[2]);
    Tmp = gl4es_inplace_replace(Tmp, &tmpsize, "gl_ObjectPlaneR",
                                "_gl4es_ObjectPlaneR");
  }
  if (strstr(Tmp, "gl_ObjectPlaneQ")) {
    Tmp = gl4es_inplace_insert(gl4es_getline(Tmp, headline),
                               gl4es_texgenobjSource[3], Tmp, &tmpsize);
    headline += gl4es_countline(gl4es_texgenobjSource[3]);
    Tmp = gl4es_inplace_replace(Tmp, &tmpsize, "gl_ObjectPlaneQ",
                                "_gl4es_ObjectPlaneQ");
  }

  if (strstr(Tmp, "gl_MaxTextureUnits")) {
    Tmp = gl4es_inplace_insert(gl4es_getline(Tmp, 2),
                               gl4es_MaxTextureUnitsSource, Tmp, &tmpsize);
    headline += gl4es_countline(gl4es_MaxTextureUnitsSource);
    Tmp = gl4es_inplace_replace(Tmp, &tmpsize, "gl_MaxTextureUnits",
                                "_gl4es_MaxTextureUnits");
  }
  if (strstr(Tmp, "gl_MaxTextureCoords")) {
    Tmp = gl4es_inplace_insert(gl4es_getline(Tmp, 2),
                               gl4es_MaxTextureCoordsSource, Tmp, &tmpsize);
    headline += gl4es_countline(gl4es_MaxTextureCoordsSource);
    Tmp = gl4es_inplace_replace(Tmp, &tmpsize, "gl_MaxTextureCoords",
                                "_gl4es_MaxTextureCoords");
  }
  if (strstr(Tmp, "gl_ClipVertex")) {
    Tmp = gl4es_inplace_insert(gl4es_getline(Tmp, 2), gl4es_ClipVertex, Tmp,
                               &tmpsize);
    headline += gl4es_countline(gl4es_ClipVertex);
    Tmp = gl4es_inplace_replace(Tmp, &tmpsize, "gl_ClipVertex",
    ...
    char *p = strchr(gl4es_find_string_nc(Tmp, "main"),
                     '{'); // find the openning curly bracket of main
    if (p) {
  ...
  if (gl4es_find_string(Tmp, gl_ProgramEnv)) {
    ...
    if (noarray_ok) {
      ...
      for (int i = 0; i <= maxind; ++i) {
        ...
        sprintf(T, "_gl4es_%s_ProgramEnv_%d", isVertex ? "Vertex" : "Fragment",
                i);
        ...
        if (gl4es_find_string(Tmp, T)) {
          ...
          Tmp = gl4es_inplace_insert(gl4es_getline(Tmp, headline), U, Tmp,
                                     &tmpsize);
          headline += 1;
        }
      }
      ...
      sprintf(U, "uniform vec4 %s[%d];\n", T,
              isVertex ? MAX_VTX_PROG_ENV_PARAMS : MAX_FRG_PROG_ENV_PARAMS);
      Tmp =
          gl4es_inplace_insert(gl4es_getline(Tmp, headline), U, Tmp, &tmpsize);
      headline += 1;
  ...
  if (gl4es_find_string(Tmp, gl_ProgramLocal)) {
    ...
    if (noarray_ok) {
      ...
      for (int i = 0; i <= maxind; ++i) {
        ...
        sprintf(T, "_gl4es_%s_ProgramLocal_%d",
                isVertex ? "Vertex" : "Fragment", i);
        Tmp = gl4es_inplace_replace(Tmp, &tmpsize, F, T);
        if (gl4es_find_string(Tmp, T)) {
          // add the uniform declaration if needed
          sprintf(U, "uniform vec4 %s;\n", T);
      ...
      sprintf(U, "uniform vec4 %s[%d];\n", T,
              isVertex ? MAX_VTX_PROG_LOC_PARAMS : MAX_FRG_PROG_LOC_PARAMS);
      Tmp =
          gl4es_inplace_insert(gl4es_getline(Tmp, headline), U, Tmp, &tmpsize);
      headline += 1;
...
int isBuiltinMatrix(const char *name) {
  ...
  for (int i = 0; i < n && ret == -1; i++) {
    if (strncmp(builtin_matrix[i].name, name, strlen(builtin_matrix[i].name)) ==
        0) {
      int l = strlen(builtin_matrix[i].name);
      if (strlen(name) == l ||
          (strlen(name) == l + 3 && name[l] == '[' &&
           builtin_matrix[i].texarray) ||
          (strlen(name) == l + 4 && name[l] == '[' &&
           builtin_matrix[i].texarray)) {
