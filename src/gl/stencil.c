...
void APIENTRY_GL4ES gl4es_glStencilMask(GLuint mask) {
  ...
  if (glstate->stencil.mask[0] == glstate->stencil.mask[1] &&
      glstate->stencil.mask[0] == mask) {
    noerrorShim();
    return;
  }
  FLUSH_BEGINEND;
...
void APIENTRY_GL4ES gl4es_glStencilMaskSeparate(GLenum face, GLuint mask) {
  ...
  if ((face == GL_FRONT && glstate->stencil.mask[0] == mask) ||
      (face == GL_BACK && glstate->stencil.mask[1] == mask)) {
    noerrorShim();
    return;
  }
  ...
  if (gles_glStencilMaskSeparate) {
    ...
    // fake function..., call it only for front or front_and_back, just ignore
    // back (crappy, I know)
    if (face == GL_FRONT)
      gl4es_glStencilMask(mask);
    else
...
void APIENTRY_GL4ES gl4es_glStencilFunc(GLenum func, GLint ref, GLuint mask) {
  ...
  if (glstate->stencil.func[0] == glstate->stencil.func[1] &&
      glstate->stencil.func[0] == func &&
      glstate->stencil.f_ref[0] == glstate->stencil.f_ref[1] &&
      glstate->stencil.f_ref[0] == ref &&
      glstate->stencil.f_mask[0] == glstate->stencil.f_mask[1] &&
      glstate->stencil.f_mask[0] == mask) {
    noerrorShim();
    return;
  }
...
void APIENTRY_GL4ES gl4es_glStencilFuncSeparate(GLenum face, GLenum func,
  ...
  if (glstate->stencil.func[idx] == func &&
...
void APIENTRY_GL4ES gl4es_glStencilOp(GLenum fail, GLenum zfail, GLenum zpass) {
  ...
  if (glstate->stencil.sfail[0] == glstate->stencil.sfail[1] &&
      glstate->stencil.sfail[0] == fail &&
      glstate->stencil.dpfail[0] == glstate->stencil.dpfail[1] &&
      glstate->stencil.dpfail[0] == zfail &&
      glstate->stencil.dppass[0] == glstate->stencil.dppass[1] &&
      glstate->stencil.dppass[0] == zpass) {
    noerrorShim();
    return;
  }
...
void APIENTRY_GL4ES gl4es_glStencilOpSeparate(GLenum face, GLenum sfail,
  ...
  int idx = (face == GL_FRONT) ? 0 : 1;
  if (glstate->stencil.sfail[idx] == sfail &&
