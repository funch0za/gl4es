...
int samplerParameterfv(glsampler_t *sampler, GLenum pname,
  ...
  DBG(printf("samplerParameterfv(%p(%d), %s, [%f(%s)...])\n", sampler,
             sampler->glname, PrintEnum(pname), params[0],
             PrintEnum(params[0]));)
  GLint param = *params;
  switch (pname) {
  ...
  case GL_TEXTURE_WRAP_R:
    ...
    if (pname == GL_TEXTURE_WRAP_S)
      sampler->wrap_s = param;
    if (pname == GL_TEXTURE_WRAP_T)
      sampler->wrap_t = param;
    if (pname == GL_TEXTURE_WRAP_R)
      sampler->wrap_r = param;
    break;
...
void gl4es_glSamplerParameterIuiv(GLuint sampler, GLenum pname,
                                  GLuint *params) {
  glsampler_t *s = find_sampler(sampler);
  if (!s) {
    errorShim(GL_INVALID_VALUE);
...
void gl4es_glGetSamplerParameterfv(GLuint sampler, GLenum pname,
                                   GLfloat *params) {
  glsampler_t *s = find_sampler(sampler);
  if (!s) {
    errorShim(GL_INVALID_VALUE);
...
void gl4es_glGetSamplerParameteriv(GLuint sampler, GLenum pname,
                                   GLfloat *params) {
  glsampler_t *s = find_sampler(sampler);
  if (!s) {
    errorShim(GL_INVALID_VALUE);
...
void gl4es_glGetSamplerParameterIiv(GLuint sampler, GLenum pname,
                                    GLint *params) {
  glsampler_t *s = find_sampler(sampler);
  if (!s) {
    errorShim(GL_INVALID_VALUE);
...
void gl4es_glGetSamplerParameterIuiv(GLuint sampler, GLenum pname,
                                     GLuint *params) {
  glsampler_t *s = find_sampler(sampler);
  if (!s) {
    errorShim(GL_INVALID_VALUE);
