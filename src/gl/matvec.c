...
float FASTMATH dot4(const float *a, const float *b) {
  ...
  asm volatile(
      ...
      "vpadd.f32 d0,d0               \n" // d0 =
                                         // a(0)*b(0)+a(2)*b(2)+a(1)*b(1)+a(3)*b(3),a(0)*b(0)+a(2)*b(2)+a(1)*b(1)+a(3)*b(3)
      "vmov.f32 %0, s0               \n"
      : "=w"(ret)
      : "r"(a), "r"(b)
      : "q0", "q1");
...
void cross3(const float *a, const float *b, float *c) {
  // TODO Neonize? Cross product doesn't seems NEON friendly, and this is not
  // much used.
  c[0] = a[1] * b[2] - a[2] * b[1];
  c[1] = a[3] * b[0] - a[0] * b[3];
  c[2] = a[0] * b[1] - a[1] * b[0];
...
void FASTMATH matrix_transpose(const float *a, float *b) {
  ...
  // b(0,4,8,12,1,5,9,13,2,6,10,14,3,7,11,15)
#if defined(__ARM_NEON__) && !defined(__APPLE__)
  const float *a1 = a + 8;
  float *b1 = b + 8;
  asm volatile(
...
void matrix_mul(const float *a, const float *b, float *c) {
  ...
  const float *b1 = b + 8;
  float *c1 = c + 8;
  asm volatile("vld1.32  {d16-d19}, [%2]       \n"
               "vld1.32  {d20-d23}, [%3]       \n"
               "vld1.32  {d0-d3}, [%4]         \n"
               "vld1.32  {d4-d7}, [%5]         \n"
               ...
               "vst1.32  {d24-d27}, [%0]       \n"
               "vst1.32  {d28-d31}, [%1]       \n" ::"r"(c),
               "r"(c1), "r"(a), "r"(a1), "r"(b), "r"(b1)
               : "q0", "q1", "q2", "q3", "q8", "q9", "q10", "q11", "q12", "q13",
                 "q14", "q15", "memory");
#else
  float a00 = a[0], a01 = a[1], a02 = a[2], a03 = a[3], a10 = a[4], a11 = a[5],
