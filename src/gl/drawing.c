...
#define T2(AA, A, B)                                                           \
  if (glstate->vao->AA != glstate->vao->B.enabled)                             \
    return GL_FALSE;                                                           \
  if (glstate->vao->B.enabled &&                                               \
      memcmp(&glstate->vao->vertexattrib[A], &glstate->vao->B.state,           \
             sizeof(vertexattrib_t)))                                          \
    return GL_FALSE;
#define TEST(A, B) T2(vertexattrib[A].enabled, A, B)
#define TESTA(A, B, I) T2(vertexattrib[A + i].enabled, A + i, B[i])

  if (glstate->vao == glstate->defaultvao)
    return GL_FALSE;
  if (count > glstate->vao->cache_count)
    return GL_FALSE;
...
#define T2(AA, A, B)                                                           \
  if (glstate->vao->AA != (list->B != NULL))                                   \
    return GL_FALSE;
#define TEST(A, B) T2(vertexattrib[A].enabled, A, B)
#define TESTA(A, B, I) T2(vertexattrib[A + i].enabled, A + i, B[i])

  if (list->post_color && !list->color)
    return GL_FALSE;
  if (list->post_normal && !list->normal)
    return GL_FALSE;
  TEST(ATT_VERTEX, vert)
  TEST(ATT_COLOR, color)
  TEST(ATT_SECONDARY, secondary)
...
#define G2(AA, A, B)                                                           \
  ...
  if (glstate->vao->B.enabled)                                                 \
    memcpy(&glstate->vao->B.state, &glstate->vao->vertexattrib[A],             \
           sizeof(vertexattrib_t));
...
#undef G2
    ...
    if (glstate->vao->vertexattrib[ATT_SECONDARY]
            .enabled /* && glstate->enable.color_array*/) {
      if (glstate->vao->shared_arrays) {
        if (glstate->vao->vertexattrib[ATT_SECONDARY].size == GL_BGRA)
          ...
          glstate->vao->secondary.ptr = copy_gl_pointer(
              &glstate->vao->vertexattrib[ATT_SECONDARY], 4, 0,
        ...
        if (glstate->vao->vertexattrib[ATT_SECONDARY].size == GL_BGRA)
          ...
          list->secondary = copy_gl_pointer(
              &glstate->vao->vertexattrib[ATT_SECONDARY], 4, skip,
              count); // alpha chanel is always 0 for secondary...
      }
    }
      ...
      if (glstate->vao->vertexattrib[ATT_MULTITEXCOORD0 + i].enabled) {
        if (glstate->vao->shared_arrays) {
          glstate->vao->tex[i].ptr = copy_gl_pointer_tex(
              &glstate->vao->vertexattrib[ATT_MULTITEXCOORD0 + i], 4, 0, count);
          list->tex[i] = glstate->vao->tex[i].ptr + 4 * skip;
        } else
          list->tex[i] = copy_gl_pointer_tex(
              &glstate->vao->vertexattrib[ATT_MULTITEXCOORD0 + i], 4, skip,
              count);
      }
    }
  }
  for (int i = 0; i < hardext.maxtex; i++)
    if (list->tex[i] && list->maxtex < i + 1)
      list->maxtex = i + 1;
  return list;
}
static renderlist_t *arrays_add_renderlist(renderlist_t *a, GLenum mode,
  ...
  DBG(LOGD("arrays_add_renderlist(%p, %s, %d, %d, %p, %d)\n", a,
           PrintEnum(mode), skip, count, indices, ilen_b);)
  // check cache if any
  if (glstate->vao->shared_arrays) {
    if (!is_cache_compatible(count))
      VaoSharedClear(glstate->vao);
  ...
  if (a->len + len_b >= cap)
    cap += len_b + DEFAULT_RENDER_LIST_CAPACITY;
  ...
  if (glstate->vao->shared_arrays) {
    if (a->vert)
      memcpy(a->vert + a->len * 4, glstate->vao->vert.ptr + skip * 4,
             len_b * 4 * sizeof(GLfloat));
    if (a->normal)
      memcpy(a->normal + a->len * 3, glstate->vao->normal.ptr + skip * 3,
             len_b * 3 * sizeof(GLfloat));
    if (a->color)
      memcpy(a->color + a->len * 4, glstate->vao->color.ptr + skip * 4,
             len_b * 4 * sizeof(GLfloat));
    if (a->secondary)
      memcpy(a->secondary + a->len * 4, glstate->vao->secondary.ptr + skip * 4,
             len_b * 4 * sizeof(GLfloat));
    if (a->fogcoord)
      memcpy(a->fogcoord + a->len * 1, glstate->vao->fog.ptr + skip * 1,
             len_b * 1 * sizeof(GLfloat));
    for (int i = 0; i < a->maxtex; i++)
      if (a->tex[i])
        memcpy(a->tex[i] + a->len * 4, glstate->vao->tex[i].ptr + skip * 4,
               len_b * 4 * sizeof(GLfloat));
  } else {
    if (a->vert)
      copy_gl_pointer_tex_noalloc(a->vert + a->len * 4,
                                  &glstate->vao->vertexattrib[ATT_VERTEX], 4,
                                  skip, count);
    if (a->normal)
      copy_gl_pointer_raw_noalloc(a->normal + a->len * 3,
                                  &glstate->vao->vertexattrib[ATT_NORMAL], 3,
                                  skip, count);
    if (a->color) {
      if (glstate->vao->vertexattrib[ATT_COLOR].size == GL_BGRA)
        ...
        copy_gl_pointer_color_noalloc(a->color + a->len * 4,
                                      &glstate->vao->vertexattrib[ATT_COLOR], 4,
                                      skip, count);
    }
    if (a->secondary)
      if (glstate->vao->vertexattrib[ATT_SECONDARY].size == GL_BGRA)
        ...
        copy_gl_pointer_noalloc(
            ...
            count); // alpha chanel is always 0 for secondary...
    if (a->fogcoord)
      copy_gl_pointer_raw_noalloc(a->fogcoord + a->len * 1,
                                  &glstate->vao->vertexattrib[ATT_FOGCOORD], 1,
                                  skip, count);
    for (int i = 0; i < a->maxtex; i++)
      if (a->tex[i])
  ...
  if (!a->mode_inits)
    list_add_modeinit(a, a->mode_init);
    ...
    for (int aa = 0; aa < hardext.maxtex; aa++) {
      if (glstate->enable.texture[aa]) {
        if ((hardext.esversion == 1) &&
            ((glstate->enable.texgen_s[aa] || glstate->enable.texgen_t[aa] ||
...
GLuint len_indices(const GLushort *sindices, const GLuint *iindices,
                   GLsizei count) {
  GLuint len = 0;
  if (sindices) {
    for (int i = 0; i < count; i++)
      if (len < sindices[i])
        len = sindices[i]; // get the len of the arrays
  } else {
    for (int i = 0; i < count; i++)
      if (len < iindices[i])
        len = iindices[i]; // get the len of the arrays
  }
  return len + 1; // length is max(indices) + 1 !
}

static void glDrawElementsCommon(GLenum mode, GLint first, GLsizei count,
                                 GLuint len, const GLushort *sindices,
...
if(count>500000) return;
  ...
  // of course, GL_SELECT with shader will just not work if not using standard
  // transformation method... Instance count is ignored also
  if (glstate->render_mode == GL_SELECT) {
    // TODO handling uint indices
    if (!sindices && !iindices)
      select_glDrawArrays(&glstate->vao->vertexattrib[ATT_VERTEX], mode, first,
                          count);
    else
      select_glDrawElements(&glstate->vao->vertexattrib[ATT_VERTEX], mode,
                            count,
...
#define GetP(A) (&glstate->vao->vertexattrib[A])
      // secondary color and color sizef != 4 are "intercepted" and draw using a
      // list, unless using ES>1.1
      ...
      for (int aa = 0; aa < hardext.maxtex; aa++) {
        client_state(ATT_MULTITEXCOORD0 + aa, GL_TEXTURE_COORD_ARRAY,
                     TEXTURE(aa););
        ...
        if (itarget >= 0) {
          ...
          if (p->enabled) {
            ...
            if (changes && !len)
              len = len_indices(sindices, iindices, count);
            tex_setup_texcoord(len, changes, itarget, p);
          } else
            gles_glMultiTexCoord4f(GL_TEXTURE0 + aa, glstate->texcoord[aa][0],
                                   glstate->texcoord[aa][1],
                                   glstate->texcoord[aa][2],
                                   glstate->texcoord[aa][3]);
        }
      }
#undef GetP
    ...
    if (instancecount == 1 || hardext.esversion == 1) {
      if (!iindices && !sindices)
        ...
        gles_glDrawElements(
            mode, count, (sindices) ? GL_UNSIGNED_SHORT : GL_UNSIGNED_INT,
            (sindices ? ((void *)sindices) : ((void *)iindices)));
    } else {
      if (!iindices && !sindices)
...
void APIENTRY_GL4ES gl4es_glDrawRangeElements(GLenum mode, GLuint start,
                                              GLuint end, GLsizei count,
                                              GLenum type,
                                              const void *indices) {
  DBG(printf("glDrawRangeElements(%s, %i, %i, %i, %s, @%p), inlist=%i, "
             "pending=%d\n",
  ...
  if (compiling) {
    ...
    if (globals4es.mergelist && list->stage >= STAGE_DRAW &&
      ...
      if (list->vert) {
        glstate->list.active =
            arrays_add_renderlist(list, mode, start, end + 1, sindices, count);
    ...
    glstate->list.active = list =
        arrays_to_renderlist(list, mode, start, end + 1);
...
void APIENTRY_GL4ES gl4es_glDrawElements(GLenum mode, GLsizei count,
  ...
  // special check for QUADS and TRIANGLES that need multiple of 4 or 3
  // vertex...
  count = adjust_vertices(mode, count);
  ...
  if (compiling) {
    ...
    if (globals4es.mergelist && list->stage >= STAGE_DRAW &&
      ...
      list = NewDrawStage(list, mode);
      glstate->list.active =
          arrays_add_renderlist(list, mode, min, max + 1, sindices, count);
      NewStage(glstate->list.active, STAGE_POSTDRAW);
      return;
    }
    ...
    glstate->list.active = list =
        arrays_to_renderlist(list, mode, min, max + 1);
    list->indices = sindices;
    list->ilen = count;
    list->indice_cap = count;
    // end_renderlist(list);
...
void APIENTRY_GL4ES gl4es_glDrawArrays(GLenum mode, GLint first,
  ...
  DBG(printf("glDrawArrays(%s, %d, %d), list=%p pending=%d\n", PrintEnum(mode),
             first, count, glstate->list.active, glstate->list.pending);)
  // special check for QUADS and TRIANGLES that need multiple of 4 or 3
  // vertex...
  count = adjust_vertices(mode, count);
  ...
  if (glstate->list.active) {
    ...
    if (globals4es.mergelist && list->stage >= STAGE_DRAW &&
      ...
      if (list->vert) {
        glstate->list.active =
            arrays_add_renderlist(list, mode, first, count + first, NULL, 0);
        NewStage(glstate->list.active, STAGE_POSTDRAW);
        return;
      }
    }
...
void APIENTRY_GL4ES gl4es_glMultiDrawArrays(GLenum mode, const GLint *firsts,
  ...
  DBG(printf("glMultiDrawArrays(%s, %p, %p, %d), list=%p pending=%d\n",
             PrintEnum(mode), firsts, counts, primcount, glstate->list.active,
             glstate->list.pending);)
  if (!primcount) {
    noerrorShim();
    return;
  ...
  if (!compiling) {
    if (!intercept && glstate->list.pending &&
        maxcount >
            MAX_BATCH) // too large and will not intercept, stop the BATCH
      gl4es_flush();
  ...
  for (int i = 0; i < primcount; i++) {
    ...
    if (compiling) {
      if (globals4es.mergelist && glstate->list.active->stage >= STAGE_DRAW &&
        ...
        glstate->list.active = arrays_add_renderlist(
            glstate->list.active, mode, first, count + first, NULL, 0);
        NewStage(glstate->list.active, STAGE_POSTDRAW);
        continue;
      }

      NewStage(glstate->list.active, STAGE_DRAW);
      glstate->list.active = arrays_to_renderlist(glstate->list.active, mode,
                                                  first, count + first);
      NewStage(glstate->list.active, STAGE_POSTDRAW);
...
void APIENTRY_GL4ES gl4es_glMultiDrawElementsBaseVertex(
    GLenum mode, GLsizei *counts, GLenum type, const void *const *indices,
    GLsizei primcount, const GLint *basevertex) {
  DBG(printf("glMultiDrawElementsBaseVertex(%s, %p, %s, @%p, %d, @%p), "
             "inlist=%i, pending=%d\n",
             PrintEnum(mode), counts, PrintEnum(type), indices, primcount,
             basevertex, (glstate->list.active) ? 1 : 0,
             glstate->list.pending);)
  ...
  for (int i = 0; i < primcount; i++) {
    ...
    if (type == GL_UNSIGNED_INT && hardext.elementuint && !compiling &&
        !intercept)
      iindices =
          copy_gl_array((glstate->vao->elements)
                            ? (void *)((char *)glstate->vao->elements->data +
    ...
    if (compiling) {
      ...
      normalize_indices_us(sindices, &max, &min, count);
      list = arrays_to_renderlist(list, mode, min + basevertex[i],
                                  max + basevertex[i] + 1);
      list->indices = sindices;
      list->ilen = count;
      list->indice_cap = count;
    ...
    if (intercept) {
      ...
      if (iindices)
        for (int i = 0; i < count; i++)
          iindices[i] += basevertex[i];
      else
        for (int i = 0; i < count; i++)
          sindices[i] += basevertex[i];
      GLuint old_index = wantBufferIndex(0);
      glDrawElementsCommon(mode, 0, count, 0, sindices, iindices, 1);
      if (iindices)
        free(iindices);
...
void APIENTRY_GL4ES gl4es_glDrawElementsBaseVertex(GLenum mode, GLsizei count,
                                                   ...
                                                   const void *indices,
                                                   GLint basevertex) {
  DBG(printf("glDrawElementsBaseVertex(%s, %d, %s, %p, %d), vtx=%p map=%p, "
             "pending=%d\n",
  ...
  if (basevertex == 0)
    ...
    if (compiling) {
      ...
      normalize_indices_us(sindices, &max, &min, count);
      list = arrays_to_renderlist(list, mode, min + basevertex,
                                  max + basevertex + 1);
      list->indices = sindices;
      list->ilen = count;
      list->indice_cap = count;
...
void APIENTRY_GL4ES gl4es_glDrawRangeElementsBaseVertex(
  ...
  DBG(printf("glDrawRangeElementsBaseVertex(%s, %i, %i, %i, %s, @%p, %d), "
             "inlist=%i, pending=%d\n",
             PrintEnum(mode), start, end, count, PrintEnum(type), indices,
             basevertex, (glstate->list.active) ? 1 : 0,
             glstate->list.pending);)
  if (basevertex == 0) {
    ...
    if (compiling) {
      ...
      for (int i = 0; i < count; i++)
        sindices[i] -= start;
      list = arrays_to_renderlist(list, mode, start + basevertex,
                                  end + basevertex + 1);
      list->indices = sindices;
      list->ilen = count;
      list->indice_cap = count;
    ...
    if (intercept) {
      ...
      if (iindices)
        ...
        for (int i = 0; i < count; i++)
          sindices[i] += basevertex;
      GLuint old_index = wantBufferIndex(0);
      glDrawElementsCommon(mode, 0, count, end + basevertex + 1, sindices,
                           iindices, 1);
      if (iindices)
        free(iindices);
      else
...
void APIENTRY_GL4ES gl4es_glDrawArraysInstanced(GLenum mode, GLint first,
  ...
  DBG(printf("glDrawArraysInstanced(%s, %d, %d, %d), list=%p pending=%d\n",
             PrintEnum(mode), first, count, primcount, glstate->list.active,
             glstate->list.pending);)
  count = adjust_vertices(mode, count);
  ...
  if (glstate->list.active) {
    NewStage(glstate->list.active, STAGE_DRAW);
    glstate->list.active =
        arrays_to_renderlist(glstate->list.active, mode, first, count + first);
  ...
  if (intercept) {
    ...
    if (mode == GL_QUADS) {
      ...
      GLuint old_buffer = wantBufferIndex(0);
      glDrawElementsCommon(GL_TRIANGLES, 0, count * 3 / 2, count,
                           indices + (first - indfirst) * 3 / 2, NULL,
                           primcount);
      wantBufferIndex(old_buffer);
      return;
    }
...
AliasExport(void, glDrawArraysInstanced, ,
            (GLenum mode, GLint first, GLsizei count, GLsizei primcount));
AliasExport(void, glDrawArraysInstanced, ARB,
            (GLenum mode, GLint first, GLsizei count, GLsizei primcount));
...
void APIENTRY_GL4ES gl4es_glDrawElementsInstanced(GLenum mode, GLsizei count,
                                                  ...
                                                  GLsizei primcount) {
  DBG(printf(
          "glDrawElementsInstanced(%s, %d, %s, %p, %d), list=%p pending=%d\n",
...
void APIENTRY_GL4ES gl4es_glDrawElementsInstancedBaseVertex(
  ...
  DBG(printf("glDrawElementsInstanceBaseVertex(%s, %d, %s, %p, %d, %d), vtx=%p "
             "map=%p, pending=%d\n",
             PrintEnum(mode), count, PrintEnum(type), indices, primcount,
             basevertex,
             (glstate->vao->vertex) ? glstate->vao->vertex->data : NULL,
