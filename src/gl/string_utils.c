...
char *gl4es_inplace_replace(char *pBuffer, int *size, const char *S,
  ...
  pBuffer = gl4es_resize_if_needed(pBuffer, size,
                                   (lD - lS) * gl4es_count_string(pBuffer, S));
  char *p = pBuffer;
  while ((p = strstr(p, S))) {
...
char *gl4es_inplace_replace_simple(char *pBuffer, int *size, const char *S,
                                   const char *D) {
  int lS = strlen(S), lD = strlen(D);
  pBuffer = gl4es_resize_if_needed(
      pBuffer, size, (lD - lS) * gl4es_countstring_simple(pBuffer, S));
  char *p = pBuffer;
  while ((p = strstr(p, S))) {
    // found an occurrence of S
    // move out rest of string
