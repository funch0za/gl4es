...
glframebuffer_t *find_framebuffer(GLuint framebuffer) {
  ...
  if (framebuffer == 0)
    return glstate->fbo.fbo_0; // NULL or fbo_0 ?
  khint_t k;
...
void readfboBegin() {
  ...
  DBG(printf("readfboBegin, fbo status read=%u, draw=%u, main=%u, current=%u\n",
             glstate->fbo.fbo_read->id, glstate->fbo.fbo_draw->id,
             glstate->fbo.mainfbo_fbo, glstate->fbo.current_fb->id);)
...
void readfboEnd() {
  ...
  DBG(printf("readfboEnd, fbo status read=%p, draw=%p, main=%u, current=%p\n",
             glstate->fbo.fbo_read, glstate->fbo.fbo_draw,
             glstate->fbo.mainfbo_fbo, glstate->fbo.current_fb);)
...
void APIENTRY_GL4ES gl4es_glDeleteFramebuffers(GLsizei n,
          ...
          if (k != kh_end(glstate->fbo.framebufferlist)) {
            ...
            for (int j = 0; j < MAX_DRAW_BUFFERS; ++j) {
              if (fb->color[j] && fb->t_color[j] != GL_RENDERBUFFER) {
                gltexture_t *tex =
                    gl4es_getTexture(fb->t_color[j], fb->color[j]);
                if (tex) {
                  tex->binded_fbo = 0;
  ...
  if (globals4es.recyclefbo) {
    ...
    if (glstate->fbo.old->cap == 0) {
      ...
      glstate->fbo.old->fbos =
          (GLuint *)malloc(glstate->fbo.old->cap * sizeof(GLuint));
    }
    if (glstate->fbo.old->nbr + n > glstate->fbo.old->cap) {
      glstate->fbo.old->cap += n;
      glstate->fbo.old->fbos = (GLuint *)realloc(
          glstate->fbo.old->fbos, glstate->fbo.old->cap * sizeof(GLuint));
    }
    memcpy(glstate->fbo.old->fbos + glstate->fbo.old->nbr, framebuffers,
           n * sizeof(GLuint));
...
void APIENTRY_GL4ES gl4es_glBindFramebuffer(GLenum target, GLuint framebuffer) {
  DBG(printf("glBindFramebuffer(%s, %u), list=%s, glstate->fbo.current_fb=%d "
             "(draw=%d, read=%d)\n",
             PrintEnum(target), framebuffer,
             glstate->list.active ? "active" : "none",
             glstate->fbo.current_fb->id, glstate->fbo.fbo_draw->id,
  ...
  //    glstate->fbo.fb_status =
  //    (framebuffer==0)?GL_FRAMEBUFFER_COMPLETE:gles_glCheckFramebufferStatus(target);
}
...
GLenum ReadDraw_Push(GLenum target) {
  ...
  if (target == GL_DRAW_FRAMEBUFFER) {
    if (glstate->fbo.current_fb != glstate->fbo.fbo_draw)
      gles_glBindFramebuffer(GL_FRAMEBUFFER, (glstate->fbo.fbo_draw->id)
  ...
  if (target == GL_READ_FRAMEBUFFER) {
    if (glstate->fbo.current_fb != glstate->fbo.fbo_read)
      gles_glBindFramebuffer(GL_FRAMEBUFFER, (glstate->fbo.fbo_read->id)
                                                 ? glstate->fbo.fbo_read->id
                                                 : glstate->fbo.mainfbo_fbo);
    return GL_FRAMEBUFFER;
  }
...
void SetAttachment(glframebuffer_t *fb, GLenum attachment, GLenum atttarget,
                   GLuint att, int level) {
  switch (attachment) {
...
void APIENTRY_GL4ES gl4es_glFramebufferTexture2D(GLenum target,
  ...
  DBG(printf("glFramebufferTexture2D(%s, %s, %s, %u, %i) "
             "glstate->fbo.current_fb=%d (draw=%d, read=%d)\n",
             PrintEnum(target), PrintEnum(attachment), PrintEnum(textarget),
             texture, level, glstate->fbo.current_fb->id,
  ...
  if (texture) {
    ...
    if (!tex) {
      ...
      texture = tex->glname;
      tex->fbtex_ratio =
          (globals4es.fbtexscale > 0.0f) ? globals4es.fbtexscale : 0.0f;

      // check if texture was shrunk or if fb texture is being scaled...
      if (globals4es.fbtexscale > 0.0f || tex->shrink || tex->useratio ||
          (tex->adjust && (hardext.npot == 1 || hardext.npot == 2) &&
           !globals4es.potframebuffer)) {
        LOGD("%s texture for FBO\n", (tex->useratio)
                                         ? "going back to npot size pot'ed"
                                         : "unshrinking shrinked");
        ...
        tex->nwidth = (hardext.npot > 0 || hardext.esversion > 1)
                          ? tex->width
                          : npot(tex->width);
        tex->nheight = (hardext.npot > 0 || hardext.esversion > 1)
                           ? tex->height
                           : npot(tex->height);
        tex->adjustxy[0] = (float)tex->width / tex->nwidth;
        tex->adjustxy[1] = (float)tex->height / tex->nheight;
        tex->adjust =
            (tex->width != tex->nwidth || tex->height != tex->nheight);
        tex->shrink = 0;
        tex->useratio = 0;
        ...
        gltexture_t *bound =
            glstate->texture
                .bound[0 /*glstate->texture.active*/][ENABLED_TEX2D];
        GLuint oldtex = bound->glname;
        if (oldtex != tex->glname)
          gles_glBindTexture(GL_TEXTURE_2D, tex->glname);
        gles_glTexImage2D(GL_TEXTURE_2D, 0, tex->format, tex->nwidth,
                          tex->nheight, 0, tex->format, tex->type, NULL);
        if (oldtex != tex->glname)
          gles_glBindTexture(GL_TEXTURE_2D, oldtex);
        if (oldactive)
          gles_glActiveTexture(GL_TEXTURE0 + oldactive);
      }
      int need_change = (globals4es.potframebuffer &&
                         (npot(twidth) != twidth || npot(theight) != theight))
                            ? 1
                            : 0;
      ...
      if (need_change) {
        ...
        LOGD("Recreate a texture for a FBO (%s%s%s)\n",
             (need_change & 1) ? "POT" : "", (need_change == 3) ? " & " : "",
             (need_change & 2) ? "Format/Type" : "");
        if (need_change & 1) {
      ...
      /*           if ((tex->width<32) || (tex->height<32)) {
                     ...
                     tex->shrink = 0;
                     gltexture_t *bound =
         glstate->texture.bound[glstate->texture.active][ENABLED_TEX2D]; GLuint
         oldtex = bound->glname; if (oldtex!=tex->glname)
         gles_glBindTexture(GL_TEXTURE_2D, tex->glname);
                     gles_glTexImage2D(GL_TEXTURE_2D, 0, tex->format,
         tex->nwidth, tex->nheight, 0, tex->format, tex->type, NULL); if
         (oldtex!=tex->glname) gles_glBindTexture(GL_TEXTURE_2D, oldtex);
                 }*/

      fb->width = twidth;
      fb->height = theight;

      DBG(printf("found texture, glname=%u, size=%ix%i(%ix%i), "
                 "format/type=%s/%s\n",
                 texture, tex->width, tex->height, tex->nwidth, tex->nheight,
                 PrintEnum(tex->format), PrintEnum(tex->type));)
    }
  }
  ...
  if (tex) {
    DBG(printf("Attach Texture %d to FBO %d as Attachement %s\n", tex->glname,
               fb->id, PrintEnum(attachment));)
    tex->binded_fbo = fb->id;
    tex->binded_attachment = attachment;
  }

  if ((old_attachment_type == textarget) &&
      (old_attachment == (tex ? tex->texture : texture))) {
    // no need to reattach
    noerrorShim();
    return;
  }
  ...
  if (attachment >= GL_COLOR_ATTACHMENT0 &&
      attachment < (GL_COLOR_ATTACHMENT0 + hardext.maxcolorattach) && tex) {
    gltexture_t *bound =
        glstate->texture.bound[0 /*glstate->texture.active*/][ENABLED_TEX2D];
    if ((hardext.npot == 1 || hardext.npot == 2) &&
        (!tex->actual.wrap_s || !tex->actual.wrap_t ||
         !wrap_npot(tex->actual.wrap_s) || !wrap_npot(tex->actual.wrap_t))) {
      tex->sampler.wrap_s = tex->sampler.wrap_t = GL_CLAMP_TO_EDGE;
      tex->adjust = 0;
    }
    // npot==2 and 3 should support that, but let's ignore that for now and
    // force no mipmap for texture attached to fbo...
  ...
  if (attachment == GL_DEPTH_ATTACHMENT /*&& hardext.depthtex==0*/) {
    ...
    if (hardext.depthtex && (tex || !texture)) {
      ...
      if (tex && !(tex->format == GL_DEPTH_COMPONENT ||
        ...
        if (tex->type != GL_UNSIGNED_INT && tex->type != GL_UNSIGNED_SHORT &&
            tex->type != GL_FLOAT)
        ...
        gles_glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S,
                             GL_CLAMP_TO_EDGE);
        gles_glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T,
                             GL_CLAMP_TO_EDGE);
        gles_glTexImage2D(GL_TEXTURE_2D, 0, tex->format, tex->nwidth,
                          tex->nheight, 0, tex->format, tex->type, NULL);
        if (oldtex != tex->glname)
      ...
      gles_glFramebufferTexture2D(ntarget, attachment, GL_TEXTURE_2D, texture,
                                  0);
    } else {
      // let's create a renderbuffer and attach it instead of the (presumably)
      // depth texture
      if (tex && !tex->renderdepth) {
        gl4es_glGenRenderbuffers(1, &tex->renderdepth);
        gl4es_glBindRenderbuffer(GL_RENDERBUFFER, tex->renderdepth);
        gl4es_glRenderbufferStorage(GL_RENDERBUFFER, GL_DEPTH_COMPONENT16,
                                    twidth, theight);
        gl4es_glBindRenderbuffer(GL_RENDERBUFFER, 0);
      }
      gl4es_glFramebufferRenderbuffer(ntarget, GL_DEPTH_ATTACHMENT,
                                      GL_RENDERBUFFER,
  ...
  if (attachment == GL_STENCIL_ATTACHMENT /*&& hardext.depthtex==0*/) {
    ...
    if (level != 0)
      return;
    // this one is tricky, as it can be GL_DEPTH_STENCIL with
    // DEPTH+DEPTH_STENCIL extension of GL_STENCIL_INDEX8 with STENCIL extension
    // and having DEPTH+DEPTH_STENCIL extension doesn't grant STENCIL8 texture
    // extension!
    if ((tex || !texture) &&
        (hardext.stenciltex || (hardext.depthtex && hardext.depthstencil))) {
      // depth texture supported, so are stencil one then!
      // check if texture needs to be re-created as true depth texture
      if (tex && !(tex->format == GL_STENCIL_INDEX8 ||
                   tex->format == GL_DEPTH_STENCIL)) {
        if (tex->format == GL_DEPTH_ATTACHMENT) {
          // TODO: need to create a new texture, as the depth one is probably
          // used
          ...
          if (oldtex != tex->glname)
            gles_glBindTexture(GL_TEXTURE_2D, tex->glname);
          gles_glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER,
                               GL_NEAREST);
          gles_glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER,
                               GL_NEAREST);
          gles_glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER,
                               GL_NEAREST);
          gles_glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S,
                               GL_CLAMP_TO_EDGE);
          gles_glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T,
                               GL_CLAMP_TO_EDGE);
          tex->format =
              (hardext.stenciltex) ? GL_STENCIL_INDEX8 : GL_DEPTH_STENCIL;
          tex->type =
              (hardext.stenciltex) ? GL_UNSIGNED_BYTE : GL_UNSIGNED_INT_24_8;
          gl4es_glTexImage2D(GL_TEXTURE_2D, 0, tex->format, nwidth, nheight, 0,
          ...
          if (tex->type != GL_UNSIGNED_BYTE && tex->type != GL_UNSIGNED_SHORT &&
              tex->type != GL_FLOAT)
            tex->type = GL_UNSIGNED_BYTE;
          tex->fpe_format = FPE_TEX_DEPTH;
          realize_textures(0);
      ...
      if (tex && !tex->renderstencil) {
        ...
        gl4es_glRenderbufferStorage(GL_RENDERBUFFER, GL_STENCIL_INDEX8, twidth,
                                    theight);
        gl4es_glBindRenderbuffer(GL_RENDERBUFFER, 0);
      }
      gl4es_glFramebufferRenderbuffer(ntarget, GL_STENCIL_ATTACHMENT,
                                      GL_RENDERBUFFER,
  ...
  if (attachment == GL_DEPTH_STENCIL_ATTACHMENT /*&& hardext.depthtex==0*/) {
    ...
    if (hardext.depthstencil /*&& !(hardext.depthtex && tex)*/) {
      if (hardext.depthtex && (tex || !texture)) {
        ...
        gles_glFramebufferTexture2D(ntarget, GL_DEPTH_ATTACHMENT, GL_TEXTURE_2D,
                                    texture, 0);
        gles_glFramebufferTexture2D(ntarget, GL_STENCIL_ATTACHMENT,
                                    GL_TEXTURE_2D, texture, 0);
      } else {
        if (tex && !tex->renderdepth) {
          gl4es_glGenRenderbuffers(1, &tex->renderdepth);
          gl4es_glBindRenderbuffer(GL_RENDERBUFFER, tex->renderdepth);
          gl4es_glRenderbufferStorage(GL_RENDERBUFFER, GL_DEPTH24_STENCIL8,
                                      twidth, theight);
          gl4es_glBindRenderbuffer(GL_RENDERBUFFER, 0);
        }
        errorGL();
  ...
  if (level != 0) {
    ...
    if ((scrap_width != twidth) || (scrap_height != theight)) {
      ...
      gltexture_t *bound =
          glstate->texture.bound[glstate->texture.active][ENABLED_TEX2D];
      GLuint oldtex = bound->glname;
      if (oldtex != scrap_tex)
        gles_glBindTexture(GL_TEXTURE_2D, scrap_tex);
      gles_glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, scrap_width, scrap_height, 0,
  ...
  if (textarget >= GL_TEXTURE_CUBE_MAP_POSITIVE_X &&
      textarget < GL_TEXTURE_CUBE_MAP_POSITIVE_X + 6)
...
void APIENTRY_GL4ES gl4es_glFramebufferTexture1D(GLenum target,
                                                 GLenum attachment,
                                                 GLenum textarget,
                                                 GLuint texture, GLint level) {
  gl4es_glFramebufferTexture2D(target, attachment, textarget, texture, level);
}
void APIENTRY_GL4ES gl4es_glFramebufferTexture3D(GLenum target,
                                                 GLenum attachment,
                                                 GLenum textarget,
                                                 GLuint texture, GLint level,
                                                 GLint layer) {
...
void APIENTRY_GL4ES gl4es_glGenRenderbuffers(GLsizei n, GLuint *renderbuffers) {
  ...
  for (int i = 0; i < n; ++i) {
    ...
    glrenderbuffer_t *rend = kh_value(list, k) =
        malloc(sizeof(glrenderbuffer_t));
...
void APIENTRY_GL4ES gl4es_glFramebufferRenderbuffer(GLenum target,
  ...
  DBG(printf("glFramebufferRenderbuffer(%s, %s, %s, %u)\n", PrintEnum(target),
             PrintEnum(attachment), PrintEnum(renderbuffertarget),
             renderbuffer);)
  ...
  if (attachment >= GL_COLOR_ATTACHMENT0 &&
    ...
    if (rend->renderbuffer) {
      ...
      if (!rend->secondarytexture) {
        ...
        gl4es_glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S,
                              GL_CLAMP_TO_EDGE);
        gl4es_glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T,
                              GL_CLAMP_TO_EDGE);
        ...
        gl4es_glTexImage2D(GL_TEXTURE_2D, 0, format, width, height, 0, GL_RGBA,
                           GL_UNSIGNED_BYTE, 0);
        gl4es_glBindTexture(GL_TEXTURE_2D, oldtex);
        rend->secondarytexture = newtex;
      }
      gl4es_glFramebufferTexture2D(target, attachment, GL_TEXTURE_2D,
                                   rend->secondarytexture, 0);
      // end of cleanup
      if (oldactive)
        gl4es_glActiveTexture(GL_TEXTURE0 + oldactive);
    } else {
  ...
  if (attachment == GL_DEPTH_STENCIL_ATTACHMENT) {
    ...
    gl4es_glFramebufferRenderbuffer(target, GL_DEPTH_ATTACHMENT,
                                    renderbuffertarget, renderbuffer);
    gl4es_glFramebufferRenderbuffer(target, GL_STENCIL_ATTACHMENT,
                                    renderbuffertarget, renderbuffer);
    return;
  }
  ...
  if ((GetAttachmentType(fb, attachment) == GL_RENDERBUFFER) &&
      (GetAttachment(fb, attachment) == renderbuffer)) {
    // no need to reattach
    noerrorShim();
    return;
  }
  ...
  errorGL();
  gles_glFramebufferRenderbuffer(ntarget, attachment, renderbuffertarget,
                                 renderbuffer);
  DBG(CheckGLError(1);)
  ReadDraw_Pop(target);
}
...
void APIENTRY_GL4ES gl4es_glRenderbufferStorage(GLenum target,
  ...
  DBG(printf("glRenderbufferStorage(%s, %s, %i, %i)\n", PrintEnum(target),
             PrintEnum(internalformat), width, height);)
  ...
  errorGL();
  width =
      (hardext.npot > 0 && !globals4es.potframebuffer) ? width : npot(width);
  height =
      (hardext.npot > 0 && !globals4es.potframebuffer) ? height : npot(height);
  ...
  if ((internalformat == GL_DEPTH24_STENCIL8 &&
       (hardext.depthstencil == 0 ||
        ((hardext.vendor & VEND_IMGTEC) == VEND_IMGTEC)))) {
    internalformat =
        (hardext.depth24) ? GL_DEPTH_COMPONENT24 : GL_DEPTH_COMPONENT16;
    ...
    use_secondarybuffer = 1;
             ...
             internalformat ==
                 GL_DEPTH_COMPONENT32) // Not much is supported on GLES...
  ...
  if (rend->secondarytexture) {
    ...
    gltexture_t *bound =
        glstate->texture.bound[0 /*glstate->texture.active*/][ENABLED_TEX2D];
    GLuint oldtex = bound->glname;
    if (oldtex != rend->secondarytexture)
      gles_glBindTexture(GL_TEXTURE_2D, rend->secondarytexture);
    tex->nwidth = tex->width = width;
    tex->nheight = tex->height = height;
    gles_glTexImage2D(GL_TEXTURE_2D, 0, tex->format, tex->nwidth, tex->nheight,
...
void APIENTRY_GL4ES gl4es_glBindRenderbuffer(GLenum target,
  ...
  DBG(printf("glBindRenderbuffer(%s, %u), binded Fbo=%u\n", PrintEnum(target),
             renderbuffer, glstate->fbo.current_fb->id);)
  LOAD_GLES2_OR_OES(glBindRenderbuffer);
...
void APIENTRY_GL4ES gl4es_glGenerateMipmap(GLenum target) {
  ...
  if (globals4es.automipmap != 3) {
    ...
    bound->mipmap_auto = 1;
    /*if(bound->sampler.min_filer != bound->actual.min_filter)  // mainly for
       S3TC textures... gl4es_glTexParameteri(target, GL_TEXTURE_MIN_FILTER,
       bound->sampler.min_filer);*/
  }
}
...
void APIENTRY_GL4ES gl4es_glGetFramebufferAttachmentParameteriv(
  ...
  if (pname == GL_FRAMEBUFFER_ATTACHMENT_TEXTURE_CUBE_MAP_FACE) {
    ...
    if (tmp != 0 && tmp != GL_RENDERBUFFER) {
      noerrorShim();
      *params = (tmp >= GL_TEXTURE_CUBE_MAP_POSITIVE_X &&
                 tmp <= GL_TEXTURE_CUBE_MAP_NEGATIVE_Z)
                    ? tmp
                    : 0;
    } else {
      errorShim(GL_INVALID_ENUM);
  ...
  if(pname==GL_FRAMEBUFFER_ATTACHMENT_LAYERED) {
      GLenum tmp = GetAttachmentType(fb, attachment);
      noerrorShim();
      *params = (tmp==GL_TEXTURE_CUBE_MAP ||
  tmp==GL_TEXTURE_3D)?GL_TRUE:GL_FALSE; return;
  }
  */
  ...
  if (attachment == GL_DEPTH_ATTACHMENT &&
    ...
    if (hardext.depthtex == 0) {
      ...
      *params = GetAttachment(fb, attachment);
      // TODO: Check all this?!
      if (*params)
        *params = 16; // Depth buffer is 16 on GLES. No check for 24 bits
                      // here...
      return;
    }
    // check if it's depth/stencil 16/8, in that case, lie by spoofing 24/8 or
    // some FNA game will fail
    int depth, stencil;
    GLenum ntarget = ReadDraw_Push(target);
    gles_glGetFramebufferAttachmentParameteriv(
        ntarget, GL_STENCIL_ATTACHMENT, GL_FRAMEBUFFER_ATTACHMENT_STENCIL_SIZE,
        &stencil);
    gles_glGetFramebufferAttachmentParameteriv(
        ntarget, GL_DEPTH_ATTACHMENT, GL_FRAMEBUFFER_ATTACHMENT_DEPTH_SIZE,
        &depth);
    errorGL();
    ReadDraw_Pop(target);
  ...
  errorGL();
  gles_glGetFramebufferAttachmentParameteriv(ntarget, attachment, pname,
                                             params);
  ReadDraw_Pop(target);
}
...
void createMainFBO(int width, int height) {
  ...
  // If there is already a Framebuffer created, let's delete it.... unless it's
  // already the right size!
  int createIt = 1;
  if (glstate->fbo.mainfbo_fbo) {
    if (width == glstate->fbo.mainfbo_width &&
  ...
  glstate->fbo.mainfbo_nheight = height =
      hardext.npot > 0 ? height : npot(height);
  ...
  gles_glTexImage2D(GL_TEXTURE_2D, 0, globals4es.fbo_noalpha ? GL_RGB : GL_RGBA,
                    width, height, 0, globals4es.fbo_noalpha ? GL_RGB : GL_RGBA,
  ...
  gles_glRenderbufferStorage(GL_RENDERBUFFER, GL_DEPTH_COMPONENT24, width,
                             height);
  ...
  gles_glBindTexture(GL_TEXTURE_2D,
                     glstate->texture.bound[0][ENABLED_TEX2D]->glname);
  if (glstate->texture.active != 0)
  ...
  if (status != GL_FRAMEBUFFER_COMPLETE) {
    ...
    if (glstate->fbo.current_fb->id == 0)
      gles_glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT |
...
void APIENTRY_GL4ES gl4es_glFramebufferTextureLayer(GLenum target,
                                                    GLenum attachment,
                                                    GLuint texture, GLint level,
                                                    GLint layer) {
  gl4es_glFramebufferTexture2D(
      target, attachment, GL_TEXTURE_2D, texture,
...
#endif
void APIENTRY_GL4ES gl4es_glBlitFramebuffer(GLint srcX0, GLint srcY0,
                                            GLint srcX1, GLint srcY1,
                                            GLint dstX0, GLint dstY0,
                                            GLint dstX1, GLint dstY1,
                                            GLbitfield mask, GLenum filter) {
  // mask will be ignored
  // filter will be taken only for ReadFBO has no Texture attached (so readpixel
  // is used)
  DBG(printf("glBlitFramebuffer(%d, %d, %d, %d,  %d, %d, %d, %d,  0x%04X, %s) "
             "fbo_read=%d, fbo_draw=%d\n",
             srcX0, srcY0, srcX1, srcY1, dstX0, dstY0, dstX1, dstY1, mask,
             PrintEnum(filter), glstate->fbo.fbo_read->id,
             glstate->fbo.fbo_draw->id);)

  if ((mask & GL_COLOR_BUFFER_BIT) == 0)
    return; // cannot copy DEPTH or STENCIL data on GLES, only COLOR_BUFFER...

  if (glstate->fbo.fbo_read == glstate->fbo.fbo_draw && srcX0 == dstX0 &&
      srcX1 == dstX1 && srcY0 == dstY0 && srcY1 == dstY1)
  ...
  GLuint texture = (glstate->fbo.fbo_read->id == 0 && glstate->fbo.mainfbo_fbo)
                       ? glstate->fbo.mainfbo_tex
                       : glstate->fbo.fbo_read->color[0];

  int created =
      (texture == 0 || (glstate->fbo.fbo_read == glstate->fbo.fbo_draw));
  ...
  if (created) {
    ...
    gl4es_glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER,
                          (filter == GL_LINEAR) ? GL_LINEAR : GL_NEAREST);
    gl4es_glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER,
                          (filter == GL_LINEAR) ? GL_LINEAR : GL_NEAREST);
    gl4es_glCopyTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, srcX0, srcY0,
                           srcX1 - srcX0, srcY1 - srcY0, 0);
    srcX1 -= srcX0;
    srcX0 = 0.f;
    srcY1 -= srcY0;
    srcY0 = 0.f;
  ...
  if (texture == glstate->fbo.mainfbo_tex) {
    ...
    if (tex) {
      ...
      glname = tex->glname;
      if (!created) {
        if ((tex->actual.min_filter != filter) ||
            (tex->actual.mag_filter != filter)) {
          gltexture_t *old = glstate->texture.bound[ENABLED_TEX2D][0];
    ...
    if (globals4es.blitfb0 /* || (globals4es.usefb && !globals4es.usefbo)*/)
      ...
      if ((glstate->fbo.mainfbo_width == abs(dstX1 - dstX0)) &&
        ...
        if (gl4es_getMainFBSize) {
          gl4es_getMainFBSize(&glstate->fbo.mainfbo_width,
  ...
  gl4es_blitTexture(glname, srcX0, srcY0, srcW, srcH, nwidth, nheight, zoomx,
                    zoomy, 0, 0, dstX0, dstY0, BLIT_OPAQUE);
  gl4es_glViewport(vp[0], vp[1], vp[2], vp[3]);
  if (created) {
...
#ifndef NOX11
  if (blitfullscreen) // hack, force a swapbuffer (help wine d3d show stuff on
                      // certain games)
...
GLuint gl4es_getCurrentFBO() {
  return (glstate->fbo.current_fb->id) ? glstate->fbo.current_fb->id
                                       : glstate->fbo.mainfbo_fbo;
}
...
void APIENTRY_GL4ES gl4es_glNamedFramebufferDrawBuffers(GLuint framebuffer,
                                                        GLsizei n,
                                                        const GLenum *bufs) {
  if (n < 0 || n > hardext.maxdrawbuffers) {
    errorShim(GL_INVALID_VALUE);
...
void APIENTRY_GL4ES gl4es_glClearBufferiv(GLenum buffer, GLint drawbuffer,
                                          const GLint *value) {
  noerrorShim();
  GLenum attch;
  switch (buffer) {
  case GL_COLOR:
    ...
    if (!(attch >= GL_COLOR_ATTACHMENT0 &&
          attch < GL_COLOR_ATTACHMENT0 + hardext.maxdrawbuffers)) {
      errorShim(GL_INVALID_VALUE);
      return;
    } else {
      GLfloat oldclear[4];
      ...
      gl4es_glClearColor(value[0] / 127.0f, value[1] / 127.0f,
                         value[2] / 127.0f, value[3] / 127.0f);
      gl4es_glClear(GL_COLOR_BUFFER_BIT);
      gl4es_glClearColor(oldclear[0], oldclear[1], oldclear[2], oldclear[3]);
      // put back the draw buffers...
      if (hardext.drawbuffers)
        gles_glDrawBuffers(glstate->fbo.fbo_draw->n_draw,
                           glstate->fbo.fbo_draw->drawbuff);
      return;
    }
    break;
  case GL_STENCIL:
...
void APIENTRY_GL4ES gl4es_glClearBufferuiv(GLenum buffer, GLint drawbuffer,
                                           const GLuint *value) {
  noerrorShim();
  GLenum attch;
  switch (buffer) {
  case GL_COLOR:
    ...
    if (!(attch >= GL_COLOR_ATTACHMENT0 &&
      ...
      gl4es_glGetFloatv(GL_COLOR_CLEAR_VALUE, oldclear);
      // how to convert the value? Most FB will be 8bits / component for now...
      gl4es_glClearColor(value[0] / 255.0f, value[1] / 255.0f,
...
void APIENTRY_GL4ES gl4es_glClearBufferfv(GLenum buffer, GLint drawbuffer,
                                          const GLfloat *value) {
  noerrorShim();
  GLenum attch;
  switch (buffer) {
...
void APIENTRY_GL4ES gl4es_glClearBufferfi(GLenum buffer, GLint drawbuffer,
...
void APIENTRY_GL4ES gl4es_glClearNamedFramebufferiv(GLuint framebuffer,
                                                    GLenum buffer,
                                                    GLint drawbuffer,
                                                    const GLint *value) {
  GLuint oldf = glstate->fbo.fbo_draw->id;
  GLenum target = (glstate->fbo.fbo_draw == glstate->fbo.fbo_read)
                      ? GL_FRAMEBUFFER
                      : GL_DRAW_FRAMEBUFFER;
...
void APIENTRY_GL4ES gl4es_glClearNamedFramebufferuiv(GLuint framebuffer,
                                                     GLenum buffer,
                                                     GLint drawbuffer,
                                                     const GLuint *value) {
  GLuint oldf = glstate->fbo.fbo_draw->id;
...
void APIENTRY_GL4ES gl4es_glClearNamedFramebufferfv(GLuint framebuffer,
                                                    GLenum buffer,
                                                    GLint drawbuffer,
                                                    const GLfloat *value) {
  GLuint oldf = glstate->fbo.fbo_draw->id;
...
void APIENTRY_GL4ES gl4es_glClearNamedFramebufferfi(GLuint framebuffer,
                                                    GLenum buffer,
                                                    GLint drawbuffer,
                                                    GLfloat depth,
                                                    GLint stencil) {
...
void APIENTRY_GL4ES gl4es_glColorMaskIndexed(GLuint framebuffer, GLboolean red,
...
void gl4es_saveCurrentFBO() {
  ...
  if (framebuffer) {
    ...
    if (hardext.vendor & VEND_ARM)
      gl4es_glFinish(); // MALI seems to need a flush commandbefore unbinding
...
AliasExport(void, glFramebufferRenderbuffer, EXT,
            (GLenum target, GLenum attachment, GLenum renderbuffertarget,
             GLuint renderbuffer));
AliasExport(void, glDeleteRenderbuffers, EXT,
            (GLsizei n, GLuint *renderbuffers));
AliasExport(void, glRenderbufferStorage, EXT,
            (GLenum target, GLenum internalformat, GLsizei width,
             GLsizei height));
AliasExport(void, glBindRenderbuffer, EXT,
...
AliasExport(void, glGetFramebufferAttachmentParameteriv, EXT,
            (GLenum target, GLenum attachment, GLenum pname, GLint *params));
AliasExport(void, glGetRenderbufferParameteriv, EXT,
            (GLenum target, GLenum pname, GLint *params));
...
AliasExport(void, glNamedFramebufferDrawBuffers, ,
            (GLuint framebuffer, GLsizei n, const GLenum *bufs));
AliasExport(void, glNamedFramebufferDrawBuffers, EXT,
            (GLuint framebuffer, GLsizei n, const GLenum *bufs));

// ClearBuffer...
AliasExport(void, glClearBufferiv, ,
            (GLenum buffer, GLint drawbuffer, const GLint *value));
AliasExport(void, glClearBufferuiv, ,
            (GLenum buffer, GLint drawbuffer, const GLuint *value));
AliasExport(void, glClearBufferfv, ,
            (GLenum buffer, GLint drawbuffer, const GLfloat *value));
AliasExport(void, glClearBufferfi, ,
