...
void APIENTRY_GL4ES fpe_ReleventState_DefaultVertex(fpe_state_t *dest,
                                                    fpe_state_t *src,
                                                    shaderconv_need_t *need) {
  // filter out some non relevant state (like texture stuff if texture is
  // disabled)
  ...
  for (int i = 0; i < MAX_TEX; i++) {
    ...
    if ((dest->texenv[i].texenv < FPE_COMBINE) ||
...
void APIENTRY_GL4ES fpe_ReleventState(fpe_state_t *dest, fpe_state_t *src,
                                      int fixed) {
  // filter out some non relevant state (like texture stuff if texture is
  // disabled)
  memcpy(dest, src, sizeof(fpe_state_t));
...
void APIENTRY_GL4ES fpe_oldprogram(fpe_state_t *state) {
  ...
  if (state->vertex_prg_id) {
    gl4es_glShaderSource(glstate->fpe->vert, 1,
                         fpe_CustomVertexShader(old_vtx->shader->source, state,
                                                state->fragment_prg_id ? 0 : 1),
                         NULL);
    ...
    if (status != GL_TRUE) {
      ...
      if (globals4es.logshader)
        printf("LIBGL: FPE ARB Vertex program compile failed: ARB source "
               "is\n%s\n=======\nGLSL source is\n%s\nError is: %s\n",
    ...
    gl4es_glShaderSource(glstate->fpe->vert, 1,
                         fpe_VertexShader(&old_frg->shader->need, state), NULL);
    gl4es_glCompileShader(glstate->fpe->vert);
    gl4es_glGetShaderiv(glstate->fpe->vert, GL_COMPILE_STATUS, &status);
    if (status != GL_TRUE) {
      char buff[1000];
  ...
  if (state->fragment_prg_id) {
    ...
    if (status != GL_TRUE) {
      ...
      if (globals4es.logshader)
        printf("LIBGL: FPE ARB Fragment program compile failed: ARB source "
               "is\n%s\n=======\nGLSL source is\n%s\nError is: %s\n",
    ...
    gl4es_glShaderSource(glstate->fpe->frag, 1,
                         fpe_FragmentShader(&old_vtx->shader->need, state),
                         NULL);
    gl4es_glCompileShader(glstate->fpe->frag);
    gl4es_glGetShaderiv(glstate->fpe->frag, GL_COMPILE_STATUS, &status);
    if (status != GL_TRUE) {
  ...
  if (status != GL_TRUE) {
    ...
    if (globals4es.logshader)
      printf("LIBGL: FPE ARB Program link failed: %s\n with vertex %s%s%s%s%s "
             "and fragment %s%s%s%s%s\n",
             buff, state->vertex_prg_id ? "custom:\n" : "default",
  ...
  DBG(printf("Created program %d, with vertex=%d (old=%d) fragment=%d "
             "(old=%d), alpha=%d/%d\n",
             glstate->fpe->prog, glstate->fpe->vert, state->vertex_prg_id,
             glstate->fpe->frag, state->fragment_prg_id, state->alphatest,
             state->alphafunc);)
}
...
void APIENTRY_GL4ES fpe_program(int ispoint) {
  ...
  if (glstate->fpe == NULL ||
      memcmp(&glstate->fpe->state, &state, sizeof(fpe_state_t))) {
    ...
    if (fpe_GetProgramPSA(glstate->fpe->prog, &state) == 0) {
      ...
      if (state.vertex_prg_id || state.fragment_prg_id) {
        ...
        if (status != GL_TRUE) {
          ...
          gles_glGetShaderInfoLog(glstate->fpe->vert, 1000, NULL, buff);
          if (globals4es.logshader)
            printf("LIBGL: FPE Vertex shader compile failed: source "
                   "is\n%s\n\nError is: %s\n",
        ...
        glstate->fpe->frag = gl4es_glCreateShader(GL_FRAGMENT_SHADER);
        gl4es_glShaderSource(glstate->fpe->frag, 1,
                             fpe_FragmentShader(NULL, glstate->fpe_state),
                             NULL);
        gl4es_glCompileShader(glstate->fpe->frag);
        gl4es_glGetShaderiv(glstate->fpe->frag, GL_COMPILE_STATUS, &status);
        if (status != GL_TRUE) {
          ...
          gles_glGetShaderInfoLog(glstate->fpe->frag, 1000, NULL, buff);
          if (globals4es.logshader)
            printf("LIBGL: FPE Fragment shader compile failed: source "
                   "is\n%s\n\nError is: %s\n",
        ...
        if (status != GL_TRUE) {
          ...
          gles_glGetProgramInfoLog(glstate->fpe->prog, 1000, NULL, buff);
          if (globals4es.logshader) {
            printf("LIBGL: FPE Program link failed: source of vertex shader "
                   "is\n%s\n\n",
...
program_t *APIENTRY_GL4ES fpe_CustomShader(program_t *glprogram,
                                           fpe_state_t *state) {
  // state is not empty and glprogram already has some cache (it may be empty,
  // but kh'thingy is initialized)
  ...
  if (fpe->glprogram == NULL) {
    ...
    {
      attribloc_t *al;
      LOAD_GLES2(glBindAttribLocation); // using real one to avoid overwriting
                                        // of attribloc...
      kh_foreach_value(
          glprogram->attribloc, al,
...
program_t *APIENTRY_GL4ES fpe_CustomShader_DefaultVertex(
    program_t *glprogram, fpe_state_t *state_vertex) {
  // state is not empty and glprogram already has some cache (it may be empty,
  // but kh'thingy is initialized)
  // TODO: what if program is composed of more then 1 vertex or fragment shader?
  fpe_fpe_t *fpe =
      fpe_GetCache((fpe_cache_t *)glprogram->fpe_cache, state_vertex, 0);
  if (fpe->glprogram == NULL) {
    GLint status;
    fpe->vert = gl4es_glCreateShader(GL_VERTEX_SHADER);
    gl4es_glShaderSource(
        fpe->vert, 1, fpe_VertexShader(glprogram->default_need, state_vertex),
        NULL);
    gl4es_glCompileShader(fpe->vert);
    gl4es_glGetShaderiv(fpe->vert, GL_COMPILE_STATUS, &status);
    if (status != GL_TRUE) {
...
void APIENTRY_GL4ES fpe_SyncUniforms(uniformcache_t *cache,
                                     program_t *glprogram) {
  // TODO: Optimize this...
  ...
  kh_foreach(
      uniforms, k, m, if (m->parent_size) {
        ...
        case GL_FLOAT_MAT4:
          ...
          break;
        default:
          printf("LIBGL: Warning, sync uniform on father/son program with "
...
void APIENTRY_GL4ES fpe_EnableDisableClientState(GLenum cap, GLboolean val) {
  ...
  if (hardext.esversion == 1) {
    ...
    DBG(printf("glstate->vao->vertexattrib[%d].enabled (was %d) = %d "
               "(hardware=%d)\n",
               att, glstate->vao->vertexattrib[att].enabled, val,
...
void APIENTRY_GL4ES fpe_glMultiTexCoord4f(GLenum target, GLfloat s, GLfloat t,
                                          GLfloat r, GLfloat q) {}

void APIENTRY_GL4ES fpe_glSecondaryColorPointer(GLint size, GLenum type,
                                                GLsizei stride,
  ...
  glstate->vao->vertexattrib[ATT_SECONDARY].normalized =
...
void APIENTRY_GL4ES fpe_glVertexPointer(GLint size, GLenum type, GLsizei stride,
                                        const GLvoid *pointer) {
  DBG(printf("fpe_glVertexPointer(%d, %s, %d, %p), vertex_buffer=%p\n", size,
             PrintEnum(type), stride, pointer, glstate->vao->vertex);)
...
void APIENTRY_GL4ES fpe_glColorPointer(GLint size, GLenum type, GLsizei stride,
                                       const GLvoid *pointer) {
  ...
  glstate->vao->vertexattrib[ATT_COLOR].normalized =
      (type == GL_FLOAT) ? GL_FALSE : GL_TRUE;
...
void APIENTRY_GL4ES fpe_glNormalPointer(GLenum type, GLsizei stride,
                                        const GLvoid *pointer) {
  DBG(printf("fpe_glNormalPointer(%s, %d, %p)\n", PrintEnum(type), stride,
             pointer);)
  ...
  glstate->vao->vertexattrib[ATT_NORMAL].normalized =
      (type == GL_FLOAT) ? GL_FALSE : GL_TRUE;
...
void APIENTRY_GL4ES fpe_glTexCoordPointer(GLint size, GLenum type,
                                          GLsizei stride,
                                          const GLvoid *pointer) {
  fpe_glTexCoordPointerTMU(size, type, stride, pointer,
                           glstate->texture.client);
}

void APIENTRY_GL4ES fpe_glTexCoordPointerTMU(GLint size, GLenum type,
                                             GLsizei stride,
                                             const GLvoid *pointer, int TMU) {
  DBG(printf("fpe_glTexCoordPointer(%d, %s, %d, %p) on tmu=%d\n", size,
             PrintEnum(type), stride, pointer, TMU);)
  ...
  glstate->vao->vertexattrib[ATT_MULTITEXCOORD0 + TMU].buffer =
      glstate->vao->vertex;
}

void APIENTRY_GL4ES fpe_glFogCoordPointer(GLenum type, GLsizei stride,
                                          const GLvoid *pointer) {
  ...
  glstate->vao->vertexattrib[ATT_FOGCOORD].normalized =
      (type == GL_FLOAT) ? GL_FALSE : GL_TRUE;
...
void APIENTRY_GL4ES fpe_glColor4f(GLfloat red, GLfloat green, GLfloat blue,
                                  GLfloat alpha) {
  noerrorShim();
}
...
void APIENTRY_GL4ES fpe_glDrawElements(GLenum mode, GLsizei count, GLenum type,
  ...
  if (glstate->vao->elements && glstate->vao->elements->real_buffer &&
      ...
      indices <= ((void *)((char *)glstate->vao->elements->data +
                           glstate->vao->elements->size))) {
    use_vbo = 1;
    bindBuffer(GL_ELEMENT_ARRAY_BUFFER, glstate->vao->elements->real_buffer);
    indices = (GLvoid *)((uintptr_t)indices -
                         (uintptr_t)(glstate->vao->elements->data));
...
void APIENTRY_GL4ES fpe_glDrawArraysInstanced(GLenum mode, GLint first,
                                              GLsizei count,
                                              GLsizei primcount) {
    ...
    for (int i = 0; i < hardext.maxvattrib; i++)
      if (glprogram->va_size[i]) // only check used VA...
      {
        ...
        if (w->divisor && w->enabled) {
          char *current =
              (char *)((uintptr_t)w->pointer +
                       ((w->buffer) ? (uintptr_t)w->buffer->data : 0));
          int stride = w->stride;
          if (!stride)
            stride = gl_sizeof(w->type) * w->size;
          ...
          if (memcmp(glstate->gleshard->vavalue[i], current,
                     4 * sizeof(GLfloat))) {
...
void APIENTRY_GL4ES fpe_glDrawElementsInstanced(GLenum mode, GLsizei count,
                                                GLenum type,
                                                const GLvoid *indices,
                                                GLsizei primcount) {
...
void APIENTRY_GL4ES fpe_glLightfv(GLenum light, GLenum pname,
                                  const GLfloat *params) {
  noerrorShim();
}
void APIENTRY_GL4ES fpe_glMaterialfv(GLenum face, GLenum pname,
                                     const GLfloat *params) {
  noerrorShim();
}
void APIENTRY_GL4ES fpe_glMaterialf(GLenum face, GLenum pname,
                                    const GLfloat param) {
...
void APIENTRY_GL4ES fpe_glPointParameterfv(GLenum pname,
                                           const GLfloat *params) {
  noerrorShim();
}
...
int fpe_gettexture(int TMU) {
  ...
  int target = -1;
#define GO(A)                                                                  \
  if (IS_##A(state) && glstate->texture.bound[TMU][ENABLED_##A]->valid)        \
  target = ENABLED_##A
  GO(CUBE_MAP);
  else GO(TEX3D);
...
void realize_glenv(int ispoint, int first, int count, GLenum type,
                   const void *indices, scratch_t *scratch) {
  // the handling of GL_BGRA size of GL_DOUBLE using 1 scratch in not ideal, and
  // a waste when dealing with Buffers
  // TODO: have the scratch buffer part of the VBO, and tag it dirty when buffer
  // is changed (or always dirty for VBO 0)
  if (hardext.esversion == 1)
    return;
  ...
  if (glstate->fpe_bound_changed && !glstate->glsl->program) {
    for (int i = 0; i < glstate->fpe_bound_changed; i++) {
      ...
      gltexture_t *tex =
          (texunit == -1) ? NULL : glstate->texture.bound[i][texunit];
      if (tex && tex->valid) {
        int fmt;
        ...
        if (texunit == ENABLED_TEXTURE_RECTANGLE)
          glstate->fpe_state->texture[i].texadjust = 1;
  ...
  if (gl4es_glIsProgram(glstate->glsl->program)) {
    ...
    if (glprogram->default_vertex) {
      ...
      glprogram = fpe_CustomShader_DefaultVertex(
          glprogram, &vertex_state); // fetch from cache if exist or create it
      ...
      if (!glprogram->fpe_cache)
        glprogram->fpe_cache = fpe_NewCache();
      glprogram = fpe_CustomShader(
          glprogram, &state); // fetch from cache if exist or create it
      program = glprogram->id;
      DBG(printf("%d\n", program);)
    }
  ...
  while (tu_idx < MAX_TEX && glprogram->texunits[tu_idx].type) {
    ...
    glprogram->texunits[tu_idx].req_tu =
        GetUniformi(glprogram, glprogram->texunits[tu_idx].id);
    glprogram->texunits[tu_idx].act_tu = glprogram->texunits[tu_idx].req_tu;
    ++tu_idx;
  }
  if (globals4es.fbounbind && glstate->fbo.current_fb->id) {
    // check if need to unbind/bind fbo because texture is both attached and
    // used
    tu_idx = 0;
    int need = 0;
    ...
    while (tu_idx < MAX_TEX && glprogram->texunits[tu_idx].type && !need) {
      ...
      if (tex && tex->binded_fbo == glstate->fbo.current_fb->id) {
        DBG(printf("Texture %d is used on Uniform %s (%d) and binded on FBO %d "
                   "Attachement=%s\n",
                   tex->glname,
                   GetUniformName(glprogram, glprogram->texunits[tu_idx].id),
  ...
  if (glprogram->has_builtin_matrix) {
    if (glprogram->builtin_matrix[MAT_MVP] != -1 ||
        glprogram->builtin_matrix[MAT_MVP_I] != -1 ||
        glprogram->builtin_matrix[MAT_MVP_T] != -1 ||
        glprogram->builtin_matrix[MAT_MVP_IT] != -1) {
      GoUniformMatrix4fv(glprogram, glprogram->builtin_matrix[MAT_MVP], 1,
                         GL_FALSE, getMVPMat());
      GoUniformMatrix4fv(glprogram, glprogram->builtin_matrix[MAT_MVP_T], 1,
                         GL_TRUE, getMVPMat());
      if (glprogram->builtin_matrix[MAT_MVP_I] != -1 ||
          glprogram->builtin_matrix[MAT_MVP_IT] != -1) {
        GLfloat invmat[16];
        matrix_inverse(getMVPMat(), invmat);
        GoUniformMatrix4fv(glprogram, glprogram->builtin_matrix[MAT_MVP_I], 1,
                           GL_FALSE, invmat);
        GoUniformMatrix4fv(glprogram, glprogram->builtin_matrix[MAT_MVP_IT], 1,
                           GL_TRUE, invmat);
      }
    }
    if (glprogram->builtin_matrix[MAT_MV] != -1 ||
        glprogram->builtin_matrix[MAT_MV_I] != -1 ||
        glprogram->builtin_matrix[MAT_MV_T] != -1 ||
        glprogram->builtin_matrix[MAT_MV_IT] != -1) {
      GoUniformMatrix4fv(glprogram, glprogram->builtin_matrix[MAT_MV], 1,
                         GL_FALSE, getMVMat());
      GoUniformMatrix4fv(glprogram, glprogram->builtin_matrix[MAT_MV_T], 1,
                         GL_TRUE, getMVMat());
      if (glprogram->builtin_matrix[MAT_MV_I] != -1 ||
          glprogram->builtin_matrix[MAT_MV_IT] != -1) {
        GoUniformMatrix4fv(glprogram, glprogram->builtin_matrix[MAT_MV_I], 1,
                           GL_FALSE, getInvMVMat());
        GoUniformMatrix4fv(glprogram, glprogram->builtin_matrix[MAT_MV_IT], 1,
                           GL_TRUE, getInvMVMat());
      }
    }
    if (glprogram->builtin_matrix[MAT_P] != -1 ||
        glprogram->builtin_matrix[MAT_P_I] != -1 ||
        glprogram->builtin_matrix[MAT_P_T] != -1 ||
        glprogram->builtin_matrix[MAT_P_IT] != -1) {
      GoUniformMatrix4fv(glprogram, glprogram->builtin_matrix[MAT_P], 1,
                         GL_FALSE, getPMat());
      GoUniformMatrix4fv(glprogram, glprogram->builtin_matrix[MAT_P_T], 1,
                         GL_TRUE, getPMat());
      if (glprogram->builtin_matrix[MAT_P_I] != -1 ||
          glprogram->builtin_matrix[MAT_P_IT] != -1) {
        GLfloat invmat[16];
        matrix_inverse(getPMat(), invmat);
        GoUniformMatrix4fv(glprogram, glprogram->builtin_matrix[MAT_P_I], 1,
                           GL_FALSE, invmat);
        GoUniformMatrix4fv(glprogram, glprogram->builtin_matrix[MAT_P_IT], 1,
                           GL_TRUE, invmat);
      }
    }
    ...
    if (glprogram->builtin_matrix[MAT_N] != -1 ||
      ...
      if (glprogram->builtin_normalrescale != -1 &&
          !glstate->fpe_state->rescaling) {
        float tmp = 1.0f;
        GoUniformfv(glprogram, glprogram->builtin_normalrescale, 1, 1, &tmp);
      }
      if (glprogram->builtin_matrix[MAT_N] != -1) {
        GoUniformMatrix3fv(glprogram, glprogram->builtin_matrix[MAT_N], 1,
                           GL_FALSE, getNormalMat());
      }
      if ((glprogram->builtin_normalrescale != -1 &&
           glstate->fpe_state->rescaling)) {
        if (glprogram->builtin_normalrescale != -1) {
    ...
    for (int i = 0; i < MAX_TEX; i++) {
      if (glprogram->builtin_matrix[MAT_T0 + i * 4] != -1 ||
        ...
        GoUniformMatrix4fv(glprogram, glprogram->builtin_matrix[MAT_T0 + i * 4],
                           1, GL_FALSE, getTexMat(i));
        GoUniformMatrix4fv(glprogram,
                           glprogram->builtin_matrix[MAT_T0_T + i * 4], 1,
                           GL_TRUE, getTexMat(i));
  ...
  if (glprogram->has_builtin_light) {
    for (int i = 0; i < MAX_LIGHT; i++) {
      if (glprogram->builtin_lights[i].has) {
        ...
        if (glprogram->builtin_lights[i].halfVector !=
            -1) { // not sure of this, isn't gl_Vertex need to be used at some
                  // point?
          GLfloat tmp[4];
          ...
          GoUniformfv(glprogram, glprogram->builtin_lights[i].halfVector, 1, 1,
                      tmp);
        }
        ...
        GoUniformfv(glprogram, glprogram->builtin_lights[i].constantAttenuation,
                    1, 1, &glstate->light.lights[i].constantAttenuation);
        GoUniformfv(glprogram, glprogram->builtin_lights[i].linearAttenuation,
                    1, 1, &glstate->light.lights[i].linearAttenuation);
        GoUniformfv(glprogram,
                    glprogram->builtin_lights[i].quadraticAttenuation, 1, 1,
                    &glstate->light.lights[i].quadraticAttenuation);
      }
      if (glprogram->builtin_lightprod[0][i].has) {
        GLfloat tmp[4];
        vector4_mult(glstate->material.front.ambient,
                     glstate->light.lights[i].ambient, tmp); // TODO: Check that
        GoUniformfv(glprogram, glprogram->builtin_lightprod[0][i].ambient, 4, 1,
                    tmp);
        vector4_mult(glstate->material.front.diffuse,
                     glstate->light.lights[i].diffuse, tmp);
        GoUniformfv(glprogram, glprogram->builtin_lightprod[0][i].diffuse, 4, 1,
                    tmp);
        vector4_mult(glstate->material.front.specular,
                     glstate->light.lights[i].specular, tmp);
        GoUniformfv(glprogram, glprogram->builtin_lightprod[0][i].specular, 4,
                    1, tmp);
      }
      if (glprogram->builtin_lightprod[1][i].has) {
        GLfloat tmp[4];
        vector4_mult(glstate->material.back.ambient,
                     glstate->light.lights[i].ambient, tmp); // TODO: Check that
        GoUniformfv(glprogram, glprogram->builtin_lightprod[1][i].ambient, 4, 1,
                    tmp);
        vector4_mult(glstate->material.back.diffuse,
                     glstate->light.lights[i].diffuse, tmp);
        GoUniformfv(glprogram, glprogram->builtin_lightprod[1][i].diffuse, 4, 1,
                    tmp);
        vector4_mult(glstate->material.back.specular,
                     glstate->light.lights[i].specular, tmp);
        GoUniformfv(glprogram, glprogram->builtin_lightprod[1][i].specular, 4,
                    1, tmp);
      }
    }
    ...
    if (glprogram->builtin_lightmodelprod[0].sceneColor != -1) {
      ...
      GoUniformfv(glprogram, glprogram->builtin_lightmodelprod[0].sceneColor, 4,
                  1, tmp);
    }
    if (glprogram->builtin_lightmodelprod[1].sceneColor != -1) {
      ...
      GoUniformfv(glprogram, glprogram->builtin_lightmodelprod[1].sceneColor, 4,
                  1, tmp);
    }
  }
  // Instance ID
  if (glprogram->builtin_instanceID != -1) {
    GoUniformiv(glprogram, glprogram->builtin_instanceID, 1, 1,
                &glstate->instanceID);
  }
  // fog parameters
  if (glprogram->builtin_fog.has) {
    GoUniformfv(glprogram, glprogram->builtin_fog.color, 4, 1,
                glstate->fog.color);
    GoUniformfv(glprogram, glprogram->builtin_fog.density, 1, 1,
                &glstate->fog.density);
    GoUniformfv(glprogram, glprogram->builtin_fog.start, 1, 1,
                &glstate->fog.start);
  ...
  if (glprogram->has_builtin_clipplanes) {
    for (int i = 0; i < hardext.maxplanes; i++) {
      GoUniformfv(glprogram, glprogram->builtin_clipplanes[i], 4, 1,
                  glstate->planes[i]);
    }
  }
  ...
  if (glprogram->builtin_pointsprite.has) {
    ...
    GoUniformfv(glprogram, glprogram->builtin_pointsprite.fadeThresholdSize, 1,
                1, &glstate->pointsprite.fadeThresholdSize);
    GoUniformfv(glprogram,
                glprogram->builtin_pointsprite.distanceConstantAttenuation, 1,
                1, glstate->pointsprite.distance + 0);
    GoUniformfv(glprogram,
                glprogram->builtin_pointsprite.distanceLinearAttenuation, 1, 1,
                glstate->pointsprite.distance + 1);
    GoUniformfv(glprogram,
                glprogram->builtin_pointsprite.distanceQuadraticAttenuation, 1,
                1, glstate->pointsprite.distance + 2);
  }
  // texenv
  ...
  if (glprogram->has_builtin_texgen) {
    for (int i = 0; i < hardext.maxtex; i++) {
      GoUniformfv(glprogram, glprogram->builtin_eye[0][i], 4, 1,
                  glstate->texgen[i].S_E);
      GoUniformfv(glprogram, glprogram->builtin_eye[1][i], 4, 1,
                  glstate->texgen[i].T_E);
      GoUniformfv(glprogram, glprogram->builtin_eye[2][i], 4, 1,
                  glstate->texgen[i].R_E);
      GoUniformfv(glprogram, glprogram->builtin_eye[3][i], 4, 1,
                  glstate->texgen[i].Q_E);
      GoUniformfv(glprogram, glprogram->builtin_obj[0][i], 4, 1,
                  glstate->texgen[i].S_O);
      GoUniformfv(glprogram, glprogram->builtin_obj[1][i], 4, 1,
                  glstate->texgen[i].T_O);
      GoUniformfv(glprogram, glprogram->builtin_obj[2][i], 4, 1,
                  glstate->texgen[i].R_O);
      GoUniformfv(glprogram, glprogram->builtin_obj[3][i], 4, 1,
                  glstate->texgen[i].Q_O);
    }
  }
  // fpe
  ...
  if (glprogram->has_builtin_texsampler) {
    for (int i = 0; i < hardext.maxtex; i++)
      GoUniformiv(
          glprogram, glprogram->builtin_texsampler[i], 1, 1,
  ...
  if (glprogram->has_builtin_texadjust) {
    for (int i = 0; i < hardext.maxtex; i++) {
      ...
      if (tex && tex->valid)
        GoUniformfv(glprogram, glprogram->builtin_texadjust[i], 2, 1,
                    tex->adjustxy);
    }
  }
  // oldprograms
  ...
  for (int i = 0; i < hardext.maxvattrib; i++)
    if (glprogram->va_size[i]) // only check used VA...
    {
      ...
      if (enabled && !w->buffer && !w->pointer) {
        DBG(printf("Warning: VA %d Enabled with buffer:0 and NULL pointer, "
                   "disabling\n",
                   i));
        enabled = 0;
      }
      ...
      if (v->enabled != enabled || (v->enabled && w->divisor)) {
        ...
        DBG(printf("VertexAttribArray[%d]:%s, divisor=%d\n", i,
                   (enabled) ? "Enable" : "Disable", w->divisor);)
        if (v->enabled)
      ...
      if (v->enabled) {
        ...
        if (dirty || v->size != w->size || v->type != w->type ||
          ...
          if ((w->size == GL_BGRA || w->type == GL_DOUBLE) &&
              scratch->size < 8) {
            // need to adjust, so first need the min/max (a shame as I already
            ...
            if (w->size == GL_BGRA) {
              ...
              v->pointer = (char *)v->pointer -
                           imin * 4 * sizeof(GLfloat); // adjust for min...
              v->stride = 0;
              v->buffer = NULL;
              v->real_buffer = 0;
              ...
              static int warn = 1;
              if (warn) {
                printf("LIBGL: VertexAttribArray using GL_DOUBLE "
                       "unimplemented!\n");
                warn = 0;
              }
            }
          ...
          if (v->integer) {
            if (gles_glVertexAttribIPointer)
              gles_glVertexAttribIPointer(i, v->size, v->type, v->stride,
                                          v->pointer);
            else
              gles_glVertexAttribPointer(i, v->size, v->type, 0, v->stride,
                                         v->pointer);
            DBG(printf("glVertexAttribIPointer(%d, %d, %s, %d, %p)\n", i,
                       v->size, PrintEnum(v->type), v->stride, v->pointer);)
          } else {
            gles_glVertexAttribPointer(i, v->size, v->type, v->normalized,
                                       v->stride, v->pointer);
            DBG(printf("glVertexAttribPointer(%d, %d, %s, %d, %d, %p)\n", i,
                       v->size, PrintEnum(v->type), v->normalized, v->stride,
                       v->pointer);)
          }
        }
      } else {
        ...
        if (dirty || memcmp(glstate->gleshard->vavalue[i], current,
                            4 * sizeof(GLfloat))) {
          memcpy(glstate->gleshard->vavalue[i], current, 4 * sizeof(GLfloat));
          gles_glVertexAttrib4fv(i, glstate->gleshard->vavalue[i]);
          DBG(printf("glVertexAttrib4fv(%d, %p) => (%f, %f, %f, %f)\n", i,
                     glstate->gleshard->vavalue[i],
                     glstate->gleshard->vavalue[i][0],
                     glstate->gleshard->vavalue[i][1],
                     glstate->gleshard->vavalue[i][2],
...
void builtin_Init(program_t *glprogram) {
  ...
  for (int i = 0; i < MAX_TEX; ++i)
    glprogram->samplers1d[i] = glprogram->samplers2d[i] =
        glprogram->samplers3d[i] = glprogram->samplersCube[i] = -1;
}
...
int builtin_CheckUniform(program_t *glprogram, char *name, GLint id, int size) {
  ...
  if (strncmp(name, lightsource_code, strlen(lightsource_code)) == 0 ||
    ...
    // it's a light! grab it's number - also, fpe or not fpe is the same length.
    // The fpe version avoids the array...
  ...
  if (strncmp(name, lightmodel_code, strlen(lightmodel_code)) == 0) {
    // it's a Light Model
    if (strstr(name, "ambient"))
      glprogram->builtin_lightmodel.ambient = id;
    glprogram->has_builtin_light = 1;
    return 1;
  }
  if (strncmp(name, frontmaterial_code, strlen(frontmaterial_code)) == 0 ||
    ...
    // it's a material
    int n = (strncmp(name, frontmaterial_code, strlen(frontmaterial_code)) == 0)
                ? 0
                : 1;
  ...
  if (strncmp(name, frontlightmodelprod_code,
              strlen(frontlightmodelprod_code)) == 0 ||
      strncmp(name, backlightmodelprod_code, strlen(backlightmodelprod_code)) ==
          0) {
    // it's a front light model product
    int n = (strncmp(name, frontlightmodelprod_code,
                     strlen(frontlightmodelprod_code)) == 0)
                ? 0
                : 1;
    if (strstr(name, ".sceneColor"))
      glprogram->builtin_lightmodelprod[n].sceneColor = id;
    glprogram->has_builtin_light = 1;
    return 1;
  }
  if (strncmp(name, frontlightprod_code, strlen(frontlightprod_code)) == 0 ||
      ...
      strncmp(name, frontlightprod_fpe_code, strlen(frontlightprod_fpe_code)) ==
          0 ||
      strncmp(name, backlightprod_fpe_code, strlen(backlightprod_fpe_code)) ==
          0) {
    // it's a material
    int i =
        (strncmp(name, frontlightprod_code, strlen(frontlightprod_code)) == 0 ||
         strncmp(name, frontlightprod_fpe_code,
                 strlen(frontlightprod_fpe_code)) == 0)
            ? 0
            : 1;
  ...
  if (strncmp(name, clipplanes_code, strlen(clipplanes_code)) == 0) {
    ...
    int n = name[strlen(clipplanes_code)] -
            '0'; // only 6 clip planes, so this works
  ...
  if (strncmp(name, clipplanes_fpe_code, strlen(clipplanes_fpe_code)) == 0) {
    ...
    int n = name[strlen(clipplanes_fpe_code)] -
            '0'; // only 6 clip planes, so this works
    if (n >= 0 && n < hardext.maxplanes) {
      glprogram->builtin_clipplanes[n] = id;
      glprogram->has_builtin_clipplanes = 1;
  ...
  if (strncmp(name, fpetexenvRGBScale_code, strlen(fpetexenvRGBScale_code)) ==
      0) {
    // it a Texture env color
  ...
  if (strncmp(name, fpetexenvAlphaScale_code,
              strlen(fpetexenvAlphaScale_code)) == 0) {
    // it a Texture env color
