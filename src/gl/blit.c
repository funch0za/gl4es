...
void gl4es_blitTexture_gles1(GLuint texture, GLfloat sx, GLfloat sy,
                             GLfloat width, GLfloat height, GLfloat nwidth,
                             GLfloat nheight, GLfloat zoomx, GLfloat zoomy,
                             GLfloat vpwidth, GLfloat vpheight, GLfloat x,
  ...
  if (drawtexok) {
    ...
    GLfloat blit_vert[] = {blit_x1 * w2 - 1.0f, blit_y1 * h2 - 1.0f,
    ...
    GLfloat blit_tex[] = {sw, sh, rw, sh, rw, rh, sw, rh};
  ...
  if (old_cli != 0)
    gles_glClientActiveTexture(GL_TEXTURE0 + old_cli);
}
...
void gl4es_blitTexture_gles2(GLuint texture, GLfloat sx, GLfloat sy,
                             GLfloat width, GLfloat height, GLfloat nwidth,
                             GLfloat nheight, GLfloat zoomx, GLfloat zoomy,
                             GLfloat vpwidth, GLfloat vpheight, GLfloat x,
                             GLfloat y, GLint mode) {
  ...
  if (!glstate->blit) {
    ...
    gles_glShaderSource(glstate->blit->pixelshader, 1, (const char **)src,
                        NULL);
    ...
    gles_glShaderSource(glstate->blit->pixelshader_alpha, 1, (const char **)src,
                        NULL);
    gles_glCompileShader(glstate->blit->pixelshader_alpha);
    gles_glGetShaderiv(glstate->blit->pixelshader_alpha, GL_COMPILE_STATUS,
                       &success);
    if (!success) {
      LOAD_GLES(glGetShaderInfoLog);
      char log[400];
    ...
    gles_glShaderSource(glstate->blit->vertexshader, 1, (const char **)src,
                        NULL);
    gles_glCompileShader(glstate->blit->vertexshader);
    gles_glGetShaderiv(glstate->blit->vertexshader, GL_COMPILE_STATUS,
                       &success);
    if (!success) {
      LOAD_GLES(glGetShaderInfoLog);
      char log[400];
    ...
    gles_glShaderSource(glstate->blit->vertexshader_alpha, 1,
                        (const char **)src, NULL);
    gles_glCompileShader(glstate->blit->vertexshader_alpha);
    gles_glGetShaderiv(glstate->blit->vertexshader_alpha, GL_COMPILE_STATUS,
                       &success);
    if (!success) {
      LOAD_GLES(glGetShaderInfoLog);
      char log[400];
    ...
    gles_glUniform1i(gles_glGetUniformLocation(glstate->blit->program, "uTex"),
                     0);
    ...
    gles_glAttachShader(glstate->blit->program_alpha,
                        glstate->blit->pixelshader_alpha);
    gles_glAttachShader(glstate->blit->program_alpha,
                        glstate->blit->vertexshader_alpha);
    ...
    gles_glUniform1i(
        gles_glGetUniformLocation(glstate->blit->program_alpha, "uTex"), 0);
...
void gl4es_blitTexture(GLuint texture, GLfloat sx, GLfloat sy, GLfloat width,
                       GLfloat height, GLfloat nwidth, GLfloat nheight,
                       GLfloat zoomx, GLfloat zoomy, GLfloat vpwidth,
                       GLfloat vpheight, GLfloat x, GLfloat y, GLint mode) {
  // printf("blitTexture(%d, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %d)
  // customvp=%d, vp=%d/%d/%d/%d\n", texture, sx, sy, width, height, nwidth,
  // nheight, zoomx, zoomy, vpwidth, vpheight, x, y, mode, (vpwidth>0.0),
  ...
  gl4es_glPushAttrib(GL_TEXTURE_BIT | GL_ENABLE_BIT | GL_TRANSFORM_BIT |
  ...
  if (hardext.esversion == 1) {
    ...
    gl4es_blitTexture_gles1(texture, sx, sy, width, height, nwidth, nheight,
                            zoomx, zoomy, vpwidth, vpheight, x, y, mode);
    if (!IS_TEX2D(tmp))
      gles_glDisable(GL_TEXTURE_2D);
    if (IS_CUBE_MAP(tmp))
      gles_glEnable(GL_TEXTURE_CUBE_MAP);
  } else {
    gl4es_blitTexture_gles2(texture, sx, sy, width, height, nwidth, nheight,
                            zoomx, zoomy, vpwidth, vpheight, x, y, mode);
  }
