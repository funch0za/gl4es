...
void push_hit() {
  ...
  if (glstate->selectbuf.hit) {
    if (!glstate->selectbuf.overflow) {
      // Normalize zmin/zmax
      if ((glstate->selectbuf.zmaxoverall - glstate->selectbuf.zminoverall) !=
          0.0f) {
        glstate->selectbuf.zmin =
            (glstate->selectbuf.zmin - glstate->selectbuf.zminoverall) /
            (glstate->selectbuf.zmaxoverall - glstate->selectbuf.zminoverall);
        glstate->selectbuf.zmax =
            (glstate->selectbuf.zmax - glstate->selectbuf.zminoverall) /
            (glstate->selectbuf.zmaxoverall - glstate->selectbuf.zminoverall);
      }
      ...
      if (tocopy > 0)
        glstate->selectbuf.buffer[glstate->selectbuf.pos + 0] =
            glstate->namestack.top;
      if (tocopy > 1)
        glstate->selectbuf.buffer[glstate->selectbuf.pos + 1] =
            (unsigned int)(glstate->selectbuf.zmin * INT_MAX);
      if (tocopy > 2)
        glstate->selectbuf.buffer[glstate->selectbuf.pos + 2] =
            (unsigned int)(glstate->selectbuf.zmax * INT_MAX);
      if (tocopy > 3)
        memcpy(glstate->selectbuf.buffer + glstate->selectbuf.pos + 3,
               glstate->namestack.names, (tocopy - 3) * sizeof(GLuint));
...
GLint APIENTRY_GL4ES gl4es_glRenderMode(GLenum mode) {
  ...
  if (mode == GL_SELECT) {
    if (glstate->selectbuf.buffer ==
        NULL) { // error, cannot use Select Mode without select buffer
      errorShim(GL_INVALID_OPERATION);
      return 0;
    }
...
GLboolean select_segment_in_viewscreen(const GLfloat *a, const GLfloat *b) {
  ...
  */
  ...
  if (select_point_in_viewscreen(a))
    return true;
  if (select_point_in_viewscreen(b))
    return true;
...
GLboolean select_triangle_in_viewscreen(const GLfloat *a, const GLfloat *b,
                                        const GLfloat *c) {
  /*
   Return True is the triangle is in the viewscreen, or completly include, or
   include the viewscreen
  */
  // Check if any segment intersect the viewscreen (include test if any point is
  // inside the viewscreen)
  if (select_segment_in_viewscreen(a, b))
    return true;
  if (select_segment_in_viewscreen(b, c))
    return true;
  if (select_segment_in_viewscreen(c, a))
    return true;
...
void select_glDrawArrays(const vertexattrib_t *vtx, GLenum mode, GLuint first,
                         GLuint count) {
  if (count == 0)
    return;
  if (vtx->pointer == NULL)
  ...
  if (glstate->selectbuf.buffer == NULL)
    return;
  GLfloat *vert = copy_gl_array(vtx->pointer, vtx->type, vtx->size, vtx->stride,
...
#define FOUND()                                                                \
  ...
  for (int i = first; i < count + first; i++) {
    ...
    ZMinMax(&glstate->selectbuf.zminoverall, &glstate->selectbuf.zmaxoverall,
            vert + i * 4);
  }
  ...
  for (int i = 0; i < count; i++) {
    ...
    case GL_TRIANGLES:
      if (i % 3 == 2) {
        if (select_triangle_in_viewscreen(vert2 + (i - 2) * 4,
                                          vert2 + (i - 1) * 4, vert2 + i * 4)) {
    ...
    case GL_TRIANGLE_FAN:
      if (i > 1) {
        if (select_triangle_in_viewscreen(vert2, vert2 + (i - 1) * 4,
                                          vert2 + i * 4)) {
  ...
  free(vert);
  if (found) {
    if (zmin < glstate->selectbuf.zmin)
      glstate->selectbuf.zmin = zmin;
    if (zmax > glstate->selectbuf.zmax)
      glstate->selectbuf.zmax = zmax;
  }
#undef FOUND
}

void select_glDrawElements(const vertexattrib_t *vtx, GLenum mode, GLuint count,
...
#define FOUND()                                                                \
  ...
  if (sind) {
    for (int i = 0; i < count; i++) {
      ...
      case GL_LINES:
        if (i % 2 == 1) {
          if (select_segment_in_viewscreen(vert + sind[(i - 1)] * 4,
                                           vert + sind[i] * 4)) {
      ...
      case GL_TRIANGLES:
        if (i % 3 == 2) {
          if (select_triangle_in_viewscreen(vert + sind[(i - 2)] * 4,
                                            vert + sind[(i - 1)] * 4,
                                            vert + sind[i] * 4)) {
      ...
      case GL_TRIANGLE_FAN:
        if (i > 1) {
          if (select_triangle_in_viewscreen(vert + sind[0] * 4,
                                            vert + sind[(i - 1)] * 4,
                                            vert + sind[i] * 4)) {
    ...
    for (int i = 0; i < count; i++) {
      ...
      case GL_LINES:
        if (i % 2 == 1) {
          if (select_segment_in_viewscreen(vert + iind[(i - 1)] * 4,
                                           vert + iind[i] * 4)) {
      ...
      case GL_TRIANGLES:
        if (i % 3 == 2) {
          if (select_triangle_in_viewscreen(vert + iind[(i - 2)] * 4,
                                            vert + iind[(i - 1)] * 4,
                                            vert + iind[i] * 4)) {
      ...
      case GL_TRIANGLE_FAN:
        if (i > 1) {
          if (select_triangle_in_viewscreen(vert + iind[0] * 4,
                                            vert + iind[(i - 1)] * 4,
                                            vert + iind[i] * 4)) {
