...
void APIENTRY_GL4ES gl4es_glLightModelfv(GLenum pname, const GLfloat *params) {
  ...
  // (state.list.compiling)?"list":"", pname, params[0], params[1], params[2],
  // params[3]);
  ERROR_IN_BEGIN
  if (glstate->list.active)
    if ((glstate->list.compiling)) {
      ...
      /*		if (glstate->list.active->lightmodel)
                  glstate->list.active =
         extend_renderlist(glstate->list.active);*/
  ...
  case GL_LIGHT_MODEL_TWO_SIDE:
    ...
    break;
  case GL_LIGHT_MODEL_COLOR_CONTROL:
    if (params[0] != GL_SINGLE_COLOR &&
        params[0] != GL_SEPARATE_SPECULAR_COLOR) {
      errorShim(GL_INVALID_VALUE);
      return;
    } else {
...
void APIENTRY_GL4ES gl4es_glLightfv(GLenum light, GLenum pname,
  ...
  // params, (params)?params[0]:0.0f, (params)?params[1]:0.0f,
  ...
  switch (pname) {
  case GL_AMBIENT:
    if (memcmp(glstate->light.lights[nl].ambient, params,
               4 * sizeof(GLfloat)) == 0)
      return;
    memcpy(glstate->light.lights[nl].ambient, params, 4 * sizeof(GLfloat));
    break;
  case GL_DIFFUSE:
    if (memcmp(glstate->light.lights[nl].diffuse, params,
               4 * sizeof(GLfloat)) == 0)
      return;
    memcpy(glstate->light.lights[nl].diffuse, params, 4 * sizeof(GLfloat));
    break;
  case GL_SPECULAR:
    if (memcmp(glstate->light.lights[nl].specular, params,
               4 * sizeof(GLfloat)) == 0)
      return;
  ...
  case GL_POSITION:
    ...
    if (memcmp(glstate->light.lights[nl].position, tmp, 4 * sizeof(GLfloat)) ==
        0)
      return;
  ...
  case GL_SPOT_DIRECTION:
    ...
    if (memcmp(glstate->light.lights[nl].spotDirection, tmp,
               3 * sizeof(GLfloat)) == 0)
      return;
...
void APIENTRY_GL4ES gl4es_glMaterialfv(GLenum face, GLenum pname,
  ...
  if (face == GL_BACK &&
      hardext.esversion == 1) { // lets ignore GL_BACK in GLES 1.1
    noerrorShim();
    return;
  }
