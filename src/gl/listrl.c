...
static inline void rlVertexCommon(renderlist_t *list, int idx, int l) {
  if (list->use_glstate) {
    ...
    if (list->normal)
      memcpy(list->normal + idx, list->lastNormal, sizeof(GLfloat) * 3);
    if (list->fogcoord)
      memcpy(list->fogcoord + idx, glstate->fogcoord, sizeof(GLfloat) * 1);
  } else {
    if (!list->vert)
      ...
      resize_renderlist(list);
    if (list->normal)
      memcpy(list->normal + (l * 3), list->lastNormal, sizeof(GLfloat) * 3);
    if (list->fogcoord)
      memcpy(list->fogcoord + (l * 1), glstate->fogcoord, sizeof(GLfloat) * 1);
  }
  // common part
  if (list->color)
    memcpy(list->color + idx, list->lastColors, sizeof(GLfloat) * 4);
  if (list->secondary)
    memcpy(list->secondary + (l * 4), glstate->secondary, sizeof(GLfloat) * 4);
  if (list->tex[0])
    memcpy(list->tex[0] + idx, glstate->texcoord[0], sizeof(GLfloat) * 4);
  if (list->tex[1])
    memcpy(list->tex[1] + idx, glstate->texcoord[1], sizeof(GLfloat) * 4);
  for (int a = 2; a < list->maxtex; a++)
    if (list->tex[a])
      memcpy(list->tex[a] + (l * 4), glstate->texcoord[a], sizeof(GLfloat) * 4);
}

void FASTMATH rlVertex4f(renderlist_t *list, GLfloat x, GLfloat y, GLfloat z,
                         GLfloat w) {
...
void rlEnd(renderlist_t *list) {
  ...
  adj -= adjust_vertices(
      list->merger_mode ? list->merger_mode : list->mode_init, adj);
  // printf("rlEnd(%d), indices=%p, mode_init_len=%d,
  // len/cur_istart(adj)/ilen=%d/%d(%d)/%d, mode/mode_init=%s/%s,
  // merger_mode=%s\n", list, list->indices, list->mode_init_len, list->len,
  // list->cur_istart, adj, list->ilen, PrintEnum(list->mode),
  // PrintEnum(list->mode_init),
  // list->merger_mode?PrintEnum(list->merger_mode):"none");
  list->len -= adj;
  if (!list->mode_inits && list->cur_istart)
    list_add_modeinit(list, list->mode_init);
  if (list->indices && list->merger_mode && list->len - list->cur_istart) {
    ...
    resize_indices_renderlist(list,
                              indices_getindicesize(list->merger_mode, len));
  ...
  list->cur_istart = 0;
  if (list->mode_inits)
    list_add_modeinit(list,
                      list->merger_mode ? list->merger_mode : list->mode_init);
  if (list->color)
...
void FASTMATH rlColor4f(renderlist_t *list, GLfloat r, GLfloat g, GLfloat b,
                        GLfloat a) {
  rlColorCommon(list);
...
void FASTMATH rlSecondary3f(renderlist_t *list, GLfloat r, GLfloat g,
                            GLfloat b) {
  if (list->secondary == NULL) {
    if (list->use_glstate) {
      if (!glstate->merger_secondary)
        glstate->merger_secondary =
            (GLfloat *)malloc(sizeof(GLfloat) * 4 * glstate->merger_cap);
...
void rlMaterialfv(renderlist_t *list, GLenum face, GLenum pname,
  ...
  m->pname = pname;
  int sz = 4;
  if (pname == GL_SHININESS || pname == GL_COLOR_INDEXES)
    sz = 1;
  memcpy(m->color, params, sz * sizeof(GLfloat));
}

void rlLightfv(renderlist_t *list, GLenum which, GLenum pname,
               const GLfloat *params) {
  renderlight_t *m;
  khash_t(light) * map;
  khint_t k;
...
void rlTexGenfv(renderlist_t *list, GLenum coord, GLenum pname,
                const GLfloat *params) {
  rendertexgen_t *m;
  khash_t(texgen) * map;
  khint_t k;
...
void FASTMATH rlMultiTexCoord4f(renderlist_t *list, GLenum target, GLfloat s,
...
void FASTMATH rlMultiTexCoord2fv(renderlist_t *list, GLenum target,
                                 GLfloat *v) {
  const int tmu = target - GL_TEXTURE0;
  rlMultiTexCoordCommon(list, tmu);
...
void FASTMATH rlMultiTexCoord4fv(renderlist_t *list, GLenum target,
                                 GLfloat *v) {
  const int tmu = target - GL_TEXTURE0;
  rlMultiTexCoordCommon(list, tmu);
...
void rlActiveTexture(renderlist_t *list, GLenum texture) {
  ...
  if (list->maxtex < list->tmu + 1)
    list->maxtex = list->tmu + 1;
}
...
void rlTexEnvfv(renderlist_t *list, GLenum target, GLenum pname,
                const GLfloat *params) {
  int n = 1;
  switch (target) {
...
void rlTexEnviv(renderlist_t *list, GLenum target, GLenum pname,
                const GLint *params) {
  GLfloat fparams[4];
  int n = 1;
  switch (target) {
...
renderlist_t *NewDrawStage(renderlist_t *l, GLenum m) {
  if (globals4es.mergelist && !glstate->polygon_mode &&
      ((isempty_renderlist(l) && l->prev && l->prev->open &&
        l->prev->mode == m && l->prev->mode_init == m) ||
