...
void APIENTRY_GL4ES gl4es_glLineStipple(GLuint factor, GLushort pattern) {
  ...
  if (pattern != glstate->linestipple.pattern ||
      factor != glstate->linestipple.factor || !glstate->linestipple.texture) {
    ...
    if (!glstate->linestipple.texture) {
      ...
      gl4es_glTexImage2D(GL_TEXTURE_2D, 0, GL_ALPHA, 16, 1, 0, GL_ALPHA,
                         GL_UNSIGNED_BYTE, glstate->linestipple.data);
    } else {
      gl4es_glBindTexture(GL_TEXTURE_2D, glstate->linestipple.texture);
      gl4es_glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, 16, 1, GL_ALPHA,
                            GL_UNSIGNED_BYTE, glstate->linestipple.data);
    }
...
GLfloat *gen_stipple_tex_coords(GLfloat *vert, GLushort *sindices,
                                modeinit_t *modes, int stride, int length,
                                GLfloat *noalloctex) {
  DBG(printf("Generate stripple tex (stride=%d, noalloctex=%p) length=%d:",
             stride, noalloctex, length);)
  // generate our texture coords
  GLfloat *tex =
      noalloctex
          ? noalloctex
          : (GLfloat *)malloc(modes[length - 1].ilen * 4 * sizeof(GLfloat));
  GLfloat *texPos = tex;
  ...
  // because projected coordinates are from -1. to +1., w and h are to be
  // divided by 2...
  w *= 0.5f;
  h *= 0.5f;
  int i = 0;
  for (int k = 0; k < length; k++) {
    ...
    if (modes[k].ilen < 2)
      continue;
    if (mode == GL_LINES ||
        length > 1) // always line when multiple lines were merged
      for (; i < modes[k].ilen; i += 2) {
        ...
        if (sindices)
          texPos =
              tex +
              texstride * sindices[i + 0]; // it gets written 2*, but that
                                           // should be ok, it's the same value
        ...
        texPos += texstride;
           ...
           // (well, last segment, the "loop" one, will look strange, but I will
           // not add a vertex for that)
      if (sindices)
        vertPos = vert + stride * sindices[i];
      vector_matrix(vertPos, mvp, v);
      x2 = (v[0] / v[3]) * w;
