...
int list2VBO(renderlist_t *list) {
  ...
  if (list->vert) {
    ...
    if (!work[imax].stride)
      work[imax].stride = 4 * 4; // 4*GL_FLOAT
  ...
  if (list->fogcoord) {
    ...
    if (!work[imax].stride)
      work[imax].stride = 1 * 4; // 1*GL_FLOAT
    work[imax].real_size = work[imax].stride * len;
    imax++;
  }
  if (list->normal) {
    ...
    if (!work[imax].stride)
      work[imax].stride = 3 * 4; // 3*GL_FLOAT
    work[imax].real_size = work[imax].stride * len;
    imax++;
  }
  ...
  for (int i = 0; i < imax; ++i) {
    ...
    if (r->vbo_base == r->vbo_basebase)
      gles_glBufferSubData(GL_ARRAY_BUFFER, r->vbo_basebase, r->real_size,
                           (void *)r->real_base);
  }
  // work -> list
  imax = 0;
...
void listActiveVBO(renderlist_t *list, save_vbo_t *saved) {
  if (list->vert) {
    saved[ATT_VERTEX].real_buffer =
        glstate->vao->vertexattrib[ATT_VERTEX].real_buffer;
    saved[ATT_VERTEX].real_pointer =
        glstate->vao->vertexattrib[ATT_VERTEX].real_pointer;
  ...
  if (list->color) {
    saved[ATT_COLOR].real_buffer =
        glstate->vao->vertexattrib[ATT_COLOR].real_buffer;
    saved[ATT_COLOR].real_pointer =
        glstate->vao->vertexattrib[ATT_COLOR].real_pointer;
  ...
  if (list->secondary) {
    saved[ATT_SECONDARY].real_buffer =
        glstate->vao->vertexattrib[ATT_SECONDARY].real_buffer;
    saved[ATT_SECONDARY].real_pointer =
        glstate->vao->vertexattrib[ATT_SECONDARY].real_pointer;
    saved[ATT_SECONDARY].buffer =
        glstate->vao->vertexattrib[ATT_SECONDARY].buffer;
  ...
  if (list->fogcoord) {
    saved[ATT_FOGCOORD].real_buffer =
        glstate->vao->vertexattrib[ATT_FOGCOORD].real_buffer;
    saved[ATT_FOGCOORD].real_pointer =
        glstate->vao->vertexattrib[ATT_FOGCOORD].real_pointer;
    saved[ATT_FOGCOORD].buffer =
        glstate->vao->vertexattrib[ATT_FOGCOORD].buffer;
  ...
  if (list->normal) {
    saved[ATT_NORMAL].real_buffer =
        glstate->vao->vertexattrib[ATT_NORMAL].real_buffer;
    saved[ATT_NORMAL].real_pointer =
        glstate->vao->vertexattrib[ATT_NORMAL].real_pointer;
  ...
  for (int a = 0; a < list->maxtex; ++a) {
    if (list->tex[a]) {
      saved[ATT_MULTITEXCOORD0 + a].real_buffer =
          glstate->vao->vertexattrib[ATT_MULTITEXCOORD0 + a].real_buffer;
      saved[ATT_MULTITEXCOORD0 + a].real_pointer =
          glstate->vao->vertexattrib[ATT_MULTITEXCOORD0 + a].real_pointer;
      saved[ATT_MULTITEXCOORD0 + a].buffer =
          glstate->vao->vertexattrib[ATT_MULTITEXCOORD0 + a].buffer;
      glstate->vao->vertexattrib[ATT_MULTITEXCOORD0 + a].real_buffer =
          list->vbo_array;
      glstate->vao->vertexattrib[ATT_MULTITEXCOORD0 + a].real_pointer =
          list->vbo_tex[a];
...
int fill_lineIndices(modeinit_t *modes, int length, GLenum mode,
                     GLushort *indices, GLushort *ind_line) {
...
void draw_renderlist(renderlist_t *list) {
  ...
  // printf("draw_renderlist %p, size=%i, mode=%s(%s), ilen=%d, next=%p,
  // color=%p, secondarycolor=%p fogcoord=%p\n", list, list->len,
  // PrintEnum(list->mode), PrintEnum(list->mode_init), list->ilen, list->next,
  // list->color, list->secondary, list->fogcoord);
  ...
  do {
    ...
    if (list->render_op) {
      ...
      case 2:
        ...
        break;
      case 3:
        gl4es_glPushName(list->render_arg);
        break;
      case 4:
        gl4es_glLoadName(list->render_arg);
        break;
      }
    }
    if (list->fog_op) {
    ...
    if (list->pointparam_op) {
      ...
      case 1: // GL_POINT_DISTANCE_ATTENUATION
        gl4es_glPointParameterfv(GL_POINT_DISTANCE_ATTENUATION,
                                 list->pointparam_val);
        break;
      }
    }
    ...
    if (list->raster_op) {
      if (list->raster_op == 1) {
        gl4es_glRasterPos3f(list->raster_xyz[0], list->raster_xyz[1],
                            list->raster_xyz[2]);
      } else if (list->raster_op == 2) {
        gl4es_glWindowPos3f(list->raster_xyz[0], list->raster_xyz[1],
                            list->raster_xyz[2]);
    ...
    if (list->raster) {
      ...
      // glBitmap(r->width, r->height, r->xorig, r->yorig, r->xmove, r->ymove,
      // r->raster);
    ...
    if (list->bitmaps) {
      for (int i = 0; i < list->bitmaps->count; i++) {
        ...
        gl4es_glBitmap(l->width, l->height, l->xorig, l->yorig, l->xmove,
                       l->ymove, l->bitmap);
      }
    }
    ...
    if (list->texgen) {
      ...
      rendertexgen_t *m;
      kh_foreach_value(tgn, m, gl4es_glTexGenfv(m->coord, m->pname, m->color);)
    }
    ...
    if (!use_vbo_array &&
        (hardext.esversion == 1 || globals4es.usevbo == 0 || !list->name)) {
      use_vbo_array = 1;
    }
    ...
    if (list->color) {
      ...
      if (glstate->enable.color_sum && (list->secondary) &&
          hardext.esversion == 1 && !list->use_glstate) {
        if (!list->final_colors) {
          list->final_colors =
              (GLfloat *)malloc(list->len * 4 * sizeof(GLfloat));
          if (indices) {
    ...
    if (hardext.esversion > 1) {
      ...
      if (glstate->enable.color_sum && (list->secondary)) {
        ...
        fpe_glSecondaryColorPointer(4, GL_FLOAT, list->secondary_stride,
                                    list->secondary);
      } else {
...
#define TEXTURE(A)                                                             \
  if (cur_tex != A) {                                                          \
    gl4es_glClientActiveTexture(A + GL_TEXTURE0);                              \
    cur_tex = A;                                                               \
  }
    stipple = false;
    if ((list->mode == GL_LINES || list->mode == GL_LINE_STRIP ||
    ...
    if (stipple) {
      ...
      list->tex[stipple_tmu] = gen_stipple_tex_coords(
          list->vert, list->indices, list->mode_inits ? list->mode_inits : &tmp,
...
#define RS(A, len)                                                             \
  ...
  use_texgen[A] = 1
    // cannot use list->maxtex because some TMU can be using TexGen or point
    // sprites...
    if (hardext.esversion == 1) {
      for (int a = 0; a < hardext.maxtex; a++) {
        if (glstate->enable.texture[a] || (stipple && a == stipple_tmu)) {
          const GLint itarget = (stipple && a == stipple_tmu)
                                    ? ENABLED_TEX2D
                                    : get_target(glstate->enable.texture[a]);
          needclean[a] = 0;
          use_texgen[a] = 0;
          if ((glstate->enable.texgen_s[a] || glstate->enable.texgen_t[a] ||
               glstate->enable.texgen_r[a] || glstate->enable.texgen_q[a])) {
            TEXTURE(a);
            RS(a, list->len);
            gen_tex_coords(list->vert, list->normal, &glstate->texgened[a],
                           list->len, &needclean[a], a,
                           (list->ilen < list->len) ? indices : NULL,
                           (list->ilen < list->len) ? list->ilen : 0);
          } else if ((list->tex[a] == NULL) &&
                     !(list->mode == GL_POINT &&
                       glstate->texture.pscoordreplace[a])) {
            RS(a, list->len);
            gen_tex_coords(list->vert, list->normal, &glstate->texgened[a],
                           list->len, &needclean[a], a,
                           (list->ilen < list->len) ? indices : NULL,
          ...
          if ((list->tex[a] || (use_texgen[a] && !needclean[a])) &&
              ((!(globals4es.texmat || glstate->texture_matrix[a]->identity)) ||
               (bound->adjust))) {
            if (!use_texgen[a]) {
              RS(a, list->len);
              if (list->tex_stride[a]) {
                ...
                int stride =
                    list->tex_stride[a] >> 2; // stride need to be a multiple of
                                              // 4 (i.e. sizeof(GLfloat))
                ...
                memcpy(glstate->texgened[a], list->tex[a],
                       4 * sizeof(GLfloat) * list->len);
            }
            if (!(globals4es.texmat || glstate->texture_matrix[a]->identity))
              tex_coord_matrix(glstate->texgened[a], list->len, getTexMat(a));
            if (bound->adjust) {
              tex_coord_npot(glstate->texgened[a], list->len, bound->width,
                             bound->height, bound->nwidth, bound->nheight);
            }
          }
        }
        if ((list->tex[a] ||
             (use_texgen[a] &&
              !needclean[a])) /* && glstate->enable.texture[a]*/) {
          TEXTURE(a);
          fpe_glEnableClientState(GL_TEXTURE_COORD_ARRAY);
          gles_glTexCoordPointer(
              4, GL_FLOAT, (use_texgen[a]) ? 0 : list->tex_stride[a],
              (use_texgen[a]) ? glstate->texgened[a] : list->tex[a]);
        } else {
          if (glstate->gleshard->vertexattrib[ATT_MULTITEXCOORD0 + a].enabled ||
              (hardext.esversion !=
               1)) { // optim for ES1.1 to avoid useless texture switch
            TEXTURE(a);
            fpe_glDisableClientState(GL_TEXTURE_COORD_ARRAY);
          }
          // else if (!glstate->enable.texgen_s[a] &&
          // glstate->enable.texture[a]) printf("LIBGL: texture[%i] without
          // TexCoord, mode=0x%04X (init=0x%04X), listlen=%i\n", a, list->mode,
          // list->mode_init, list->len);
        }
        if (!IS_TEX2D(glstate->enable.texture[a]) &&
            (IS_ANYTEX(glstate->enable.texture[a]))) {
          TEXTURE(a);
      ...
      for (int a = 0; a < hardext.maxtex; a++) {
        if (list->tex[a]) {
          TEXTURE(a);
          fpe_glEnableClientState(GL_TEXTURE_COORD_ARRAY);
          gles_glTexCoordPointer(4, GL_FLOAT, list->tex_stride[a],
                                 list->tex[a]);
        } else {
          TEXTURE(a);
          fpe_glDisableClientState(GL_TEXTURE_COORD_ARRAY);
...
#undef TEXTURE
    ...
    if (indices) {
      if (glstate->render_mode == GL_SELECT) {
        ...
        vtx.size = 4;
        vtx.stride = 0;
        select_glDrawElements(&vtx, list->mode, list->ilen, GL_UNSIGNED_SHORT,
                              indices);
        use_vbo_indices = 1;
      } else {
        GLuint old_index = wantBufferIndex(0);
        if (glstate->polygon_mode == GL_LINE &&
            list->mode_init >= GL_TRIANGLES) {
          int ilen = list->ilen;
          if (!list->ind_lines) {
            GLushort *ind_line = list->ind_lines =
                (GLushort *)malloc(sizeof(GLushort) * ilen * 4 + 2);
            modeinit_t tmp;
            tmp.mode_init = list->mode_init;
            tmp.ilen = list->ilen;
          ...
          gles_glDrawElements(mode, list->ind_line, GL_UNSIGNED_SHORT,
                              list->ind_lines);
          use_vbo_indices = 1;
        } else {
          int vbo_indices = 0;
          if (!use_vbo_indices) {
            ...
            gles_glBufferData(GL_ELEMENT_ARRAY_BUFFER,
                              list->ilen * sizeof(GLushort), indices,
                              GL_STATIC_DRAW);
            use_vbo_indices = 2;
            vbo_indices = 1;
          ...
          if (list->instanceCount == 1)
            gles_glDrawElements(mode, list->ilen, GL_UNSIGNED_SHORT,
                                vbo_indices ? NULL : indices);
          else {
            for (glstate->instanceID = 0;
                 glstate->instanceID < list->instanceCount;
                 ++glstate->instanceID)
              gles_glDrawElements(mode, list->ilen, GL_UNSIGNED_SHORT,
                                  vbo_indices ? NULL : indices);
      ...
      if (glstate->render_mode == GL_SELECT) {
        ...
        if ((glstate->polygon_mode == GL_LINE) &&
          ...
          if (!list->ind_lines) {
            GLushort *ind_line = list->ind_lines =
                (GLushort *)malloc(sizeof(GLushort) * len * 4 + 2);
            modeinit_t tmp;
            tmp.mode_init = list->mode_init;
            tmp.ilen = len;
            int k = fill_lineIndices(list->mode_inits ? list->mode_inits : &tmp,
                                     list->mode_inits ? list->mode_init_len : 1,
                                     list->mode, NULL, list->ind_lines);
            list->ind_line = k;
          }
          bindBuffer(GL_ELEMENT_ARRAY_BUFFER, 0);
          gles_glDrawElements(mode, list->ind_line, GL_UNSIGNED_SHORT,
...
#undef TEXTURE
    ...
    if (list->post_color)
      gl4es_glColor4fv(list->post_colors);
    if (list->post_normal)
      gl4es_glNormal3fv(list->post_normals);
