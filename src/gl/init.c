...
globals4es_t globals4es = {0};
    ...
    (defined(GOA_CLONE) && !defined(__aarch64__))
...
EXPORT
void set_getmainfbsize(void(APIENTRY_GL4ES *new_getMainFBSize)(int *w,
                                                               int *h)) {
  gl4es_getMainFBSize = (void *)new_getMainFBSize;
}

EXPORT
void set_getprocaddress(
    void *(APIENTRY_GL4ES *new_proc_address)(const char *)) {
...
void initialize_gl4es() {
  ...
  if (!dll_inited) {
    LOGE("Windows ES emulator's can't be initialized from DllMain (directX "
         "limitation)\n");
    return;
  }
#endif
...
#define env(name, global, message)                                             \
  ...
  env(LIBGL_XREFRESH, globals4es.xrefresh,
      "xrefresh will be called on cleanup");
  env(LIBGL_STACKTRACE, globals4es.stacktrace,
  ...
  env(LIBGL_NOTEXRECT, globals4es.notexrect,
      "Don't export Text Rectangle extension");
  if (globals4es.usefbo) {
    env(LIBGL_FBONOALPHA, globals4es.fbo_noalpha,
        "Main FBO has no alpha channel");
  }
  ...
  env(LIBGL_NODEPTHTEX, globals4es.nodepthtex,
      "Disable usage of Depth Textures");
...
#if (defined(NOEGL) && !defined(ANDROID) && !defined(__APPLE__)) ||            \
    defined(__EMSCRIPTEN__)
  int gl4es_notest = !gles_getProcAddress;
#else
  int gl4es_notest = IsEnvVarTrue("LIBGL_NOTEST");
#endif
  env(LIBGL_NOHIGHP, globals4es.nohighp,
      "Do not use HIGHP in fragment shader even if detected");
  ...
  case 10:
    SHUT_LOGD("Texture shrink, mode 10 selected (advertise 8192 max texture "
              "size, but >2048 are quadshrinked and > 512 are shrinked), but "
              "not for empty textures\n");
    break;
  case 11:
    SHUT_LOGD(
        "Texture shrink, mode 11 selected (advertise a max texture size *2, "
        "but every texture with one dimension > max texture size will get "
        "shrinked to max texture size), but not for empty textures\n");
    break;
  case 1:
  ...
  case 4:
    ...
    break;
  case 5:
    SHUT_LOGD("Texture shrink, mode 5 selected (every > 256 is downscaled to "
              "256 ), but not for empty textures\n");
    break;
  case 6:
    SHUT_LOGD("Texture shrink, mode 6 selected (only > 128 /2, >=512 is "
              "downscaled to 256 ), but not for empty textures\n");
    break;
  case 7:
    SHUT_LOGD("Texture shrink, mode 7 selected (only > 512 /2 ), but not for "
              "empty textures\n");
    break;
  case 8:
    SHUT_LOGD("Texture shrink, mode 8 selected (advertise 8192 max texture "
              "size, but >2048 are shrinked to 2048)\n");
    break;
  case 9:
    SHUT_LOGD("Texture shrink, mode 9 selected (advertise 8192 max texture "
              "size, but >4096 are quadshrinked and > 512 are shrinked), but "
              "not for empty textures\n");
    break;
  default:
  ...
  env(LIBGL_NOLUMALPHA, globals4es.nolumalpha,
      "GL_LUMINANCE_ALPHA hardware support disabled");
  ...
  env(LIBGL_NOERROR, globals4es.noerror,
  ...
  env(LIBGL_VABGRA, globals4es.vabgra,
      "Export GL_ARB_vertex_array_bgra extension");

  const char *env_version = GetEnvVar("LIBGL_VERSION");
  if (env_version) {
    SHUT_LOGD("Override version string with \"%s\" (should be in the form of "
              "\"1.x\")\n",
              env_version);
  }
  if (env_version) {
    snprintf(globals4es.version, 49, "%s gl4es wrapper %d.%d.%d", env_version,
             MAJOR, MINOR, REVISION);
    SHUT_LOGD("Targeting OpenGL %s\n", env_version);
  } else {
    snprintf(globals4es.version, 49, "%d.%d gl4es wrapper %d.%d.%d",
             globals4es.gl / 10, globals4es.gl % 10, MAJOR, MINOR, REVISION);
  ...
  env(LIBGL_NOTEXMAT, globals4es.texmat,
      "Don't handle Texture Matrice internally");
  ...
  if (GetEnvVarInt("LIBGL_BEGINEND", &env_begin_end, 0)) {
    ...
    case 2:
      globals4es.beginend = 1;
      SHUT_LOGD("Try to merge subsequent glBegin/glEnd blocks, even if there "
                "is a glColor / glNormal in between\n");
      break;
    }
  }
  ...
  env(LIBGL_POTFRAMEBUFFER, globals4es.potframebuffer,
      "Force framebuffers to be on POT size");
  ...
  } else if (env_forcenpot != 0 || (hardext.esversion == 2 &&
                                    (hardext.npot == 1 || hardext.npot == 2))) {
    if (hardext.npot == 3) {
      SHUT_LOGD("NPOT texture handled in hardware\n");
    } else if (hardext.npot == 1) {
      globals4es.forcenpot = 1;
      SHUT_LOGD("Forcing NPOT support by disabling MIPMAP support for NPOT "
                "textures\n");
    } else {
      SHUT_LOGD("WARNING, No Limited or Full NPOT support in hardware, Forcing "
                "NPOT has no effect!\n");
    }
  }

#if defined(GL4ES_COMPILE_FOR_USE_IN_SHARED_LIB) &&                            \
    defined(AMIGAOS4) // temporary workaround for not-working envs
  ...
  if (globals4es.maxbatch == 0) {
    ...
    SHUT_LOGD("Trying to batch subsequent glDrawXXXX of size between %d and %d "
              "vertices\n",
  ...
  if (hardext.esversion == 1)
    globals4es.usevbo =
        0; // VBO on ES1.1 backend will be too messy, so disabling
  else {
    ...
    case 3:
      SHUT_LOGD(
          "Trying to use VBO (special glLockArrays case for idtech3 engine)\n");
      break;
    default:
      globals4es.usevbo = 1;
      break;
  ...
  GetEnvVarInt("LIBGL_FBOFORCETEX", &globals4es.fboforcetex,
  ...
  GetEnvVarInt("LIBGL_BLITFULLSCREEN", &globals4es.blitfullscreen,
               globals4es.blitfullscreen);
  if (globals4es.blitfullscreen)
    SHUT_LOGD("Hack to trigger a SwapBuffers when a Full Framebuffer Blit on "
              "default FBO is done\n");

  env(LIBGL_COMMENTS, globals4es.comments,
      "Keep comments in converted Shaders");

  env(LIBGL_NOARBPROGRAM, globals4es.noarbprogram,
      "Not exposing ARB Program extensions");
  ...
  env(LIBGL_LOGSHADERERROR, globals4es.logshader,
      "Log errors compiling shaders to the console");
  env(LIBGL_SHADERNOGLES, globals4es.shadernogles,
      "Remove GLES part in shader");
  env(LIBGL_NOES2COMPAT, globals4es.noes2,
      "Don't expose GLX_EXT_create_context_es2_profile extension");
  env(LIBGL_NORMALIZE, globals4es.normalize,
  ...
  if (globals4es.dbgshaderconv) {
    ...
    SHUT_LOGD_NOPREFIX(
        "Log all shaders, before and after conversion, to the console: ");
  ...
  env(LIBGL_GLXNATIVE, globals4es.glxnative,
      "Don't filter GLXConfig with GLX_X_NATIVE_TYPE");
#endif
  char cwd[4096];
  ...
  if (hardext.shader_fbfetch) {
    env(LIBGL_SHADERBLEND, globals4es.shaderblend,
        "Blend will be handled in shaders");
  }
  if (hardext.prgbin_n > 0 && !globals4es.notexarray) {
    ...
    SHUT_LOGD("Not using PSA (prgbin_n=%d, notexarray=%d)\n", hardext.prgbin_n,
              globals4es.notexarray);

  env(LIBGL_SKIPTEXCOPIES, globals4es.skiptexcopies,
      "Texture Copies will be skipped");
