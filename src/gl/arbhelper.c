...
#define TST_STR_VAL_PREF(str, pref, inst)                                      \
  (!strncmp(str, #inst, strlen(#inst))) ? pref##inst:
eVariableType STR2VARTYPE(char *str) {
  return TST_STR_VAL_PREF(str, VARTYPE_, ADDRESS)
      TST_STR_VAL_PREF(str, VARTYPE_, ATTRIB)
          TST_STR_VAL_PREF(str, VARTYPE_, PARAM)
              TST_STR_VAL_PREF(str, VARTYPE_, TEMP)
                  TST_STR_VAL_PREF(str, VARTYPE_, ALIAS)
                      TST_STR_VAL_PREF(str, VARTYPE_, OUTPUT) VARTYPE_UNK;
}
...
sVariable *createVariable(eVariableType type) {
  sVariable *var =
      (sVariable *)calloc(1, sizeof(sVariable)); // malloc *should* be fine here
...
void freeStatus(sCurStatus *curStatus) {
  ...
  while ((instPtr =
              (sInstruction *)popArray((sArray *)&curStatus->instructions))) {
    for (int i = 0; i < MAX_OPERANDS; ++i) {
...
int appendString(sCurStatus *curStatusPtr, const char *str, size_t strLen) {
  ...
  ARBCONV_DBG_HEAVY(ARBCONV_DBG(
      char *dup = malloc((strLen + 1) * sizeof(char)); memcpy(dup, str, strLen);
      dup[strLen] = '\0';
      printf("Appending '%s' to %p (%p + %ld = %p)\n", dup,
             curStatusPtr->outputEnd, curStatusPtr->outputString,
             curStatusPtr->outLen,
             curStatusPtr->outputString + curStatusPtr->outLen);
      free(dup);
      fflush(stdout);) if (curStatusPtr->outputEnd !=
                           curStatusPtr->outputString + curStatusPtr->outLen) {
