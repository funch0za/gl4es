...
bool ispurerender_renderlist(renderlist_t *list) {
  ...
  if (list->popattribute)
    return false;
  if (list->material || list->colormat_face || list->light ||
      list->lightmodel || list->texgen || list->texenv)
    return false;
  if (list->fog_op)
    return false;
...
int rendermode_dimensions(GLenum mode) {
  // return 1 for points, 2 for any lines, 3 for any triangles, 4 for any Quad
  // and 5 for polygon
  switch (mode) {
  case GL_POINTS:
    return 1;
...
bool islistscompatible_renderlist(renderlist_t *a, renderlist_t *b) {
  ...
  if (a->mode_init != b->mode_init) {
    ...
    if (a_mode == 4)
      a_mode = 3; // quads are handled as triangles
    if (b_mode == 4)
      b_mode = 3;
    if (a_mode != b_mode)
      return false;
  ...
  if (a->post_normal && b->normal == NULL)
    return false;
  // check instanceCount (maybe it would be better to just check if both are ==
  // 1)
...
void renderlist_lineloop_lines(GLushort *ind, int len, GLushort *indices,
                               int count) {
...
void renderlist_linestrip_lines(GLushort *ind, int len, GLushort *indices,
                                int count) {
...
void renderlist_trianglestrip_triangles(GLushort *ind, int len,
                                        GLushort *indices, int count) {
  int ilen = (len - 2) * 3;
  if (ilen < 0)
    ilen = 0;
  for (int i = 2; i < len; i++) {
...
void renderlist_trianglefan_triangles(GLushort *ind, int len, GLushort *indices,
                                      int count) {
  int ilen = (len - 2) * 3;
  if (ilen < 0)
    ilen = 0;
...
void renderlist_quads_triangles(GLushort *ind, int len, GLushort *indices,
                                int count) {
...
int renderlist_getindicesize(renderlist_t *a) {
  int ilen_a =
      indices_getindicesize(a->mode, ((a->indices) ? a->ilen : a->len));
  return ilen_a;
}
...
void list_add_modeinit(renderlist_t *list, GLenum mode) {
  if (list->mode_init_len + 1 >= list->mode_init_cap) {
    ...
    list->mode_inits = (modeinit_t *)realloc(
        list->mode_inits, list->mode_init_cap * sizeof(modeinit_t));
  }
  list->mode_inits[list->mode_init_len].mode_init = mode;
  list->mode_inits[list->mode_init_len++].ilen =
      list->indices ? list->ilen
                    : (list->cur_istart ? list->cur_istart : list->len);
}
...
void unsharedindices_renderlist(renderlist_t *a, int cap) {
  if (a->shared_indices && ((*a->shared_indices)--) > 0) {
    if (a->indices) {
      ...
      a->indice_cap = cap;
      if (a->indice_cap > 48)
        a->indice_cap = ((a->indice_cap + 512) >> 9) << 9;
...
#define copy_a_indices                                                         \
  ...
  case GL_LINE_LOOP:
    alloc_a_indices;
    renderlist_lineloop_lines(a->indices, a->indices ? a->ilen : a->len, newind,
                              0);
    a->mode = GL_LINES;
    copy_a_indices;
    break;
  case GL_LINE_STRIP:
    alloc_a_indices;
    renderlist_linestrip_lines(a->indices, a->indices ? a->ilen : a->len,
                               newind, 0);
    a->mode = GL_LINES;
    copy_a_indices;
    break;
  ...
  case GL_TRIANGLE_STRIP:
    ...
    renderlist_trianglestrip_triangles(
        a->indices, a->indices ? a->ilen : a->len, newind, 0);
    a->mode = GL_TRIANGLES;
    copy_a_indices;
    break;
  case GL_TRIANGLE_FAN:
  case GL_POLYGON:
    alloc_a_indices;
    renderlist_trianglefan_triangles(a->indices, a->indices ? a->ilen : a->len,
                                     newind, 0);
    a->mode = GL_TRIANGLES;
    copy_a_indices;
    break;
  case GL_QUADS:
    alloc_a_indices;
    renderlist_quads_triangles(a->indices, a->indices ? a->ilen : a->len,
                               newind, 0);
    a->mode = GL_TRIANGLES;
    copy_a_indices;
    break;
  default:
    if (!a->indices) {
      ...
      if (a->indice_cap < capindices) {
        a->indices =
            (GLushort *)realloc(a->indices, capindices * sizeof(GLushort));
        a->indice_cap = capindices;
      }
    }
    break;
...
void doadd_renderlist(renderlist_t *a, GLenum mode, GLushort *indices,
                      int count, int size_to_add) {
  // then append b
  switch (mode) {
  case GL_LINE_LOOP:
  ...
  case GL_QUAD_STRIP:
  case GL_TRIANGLE_STRIP:
    renderlist_trianglestrip_triangles(indices, count, a->indices + a->ilen,
                                       a->len);
    break;
  case GL_POLYGON:
  case GL_TRIANGLE_FAN:
    renderlist_trianglefan_triangles(indices, count, a->indices + a->ilen,
                                     a->len);
    break;
  case GL_QUADS:
...
void append_renderlist(renderlist_t *a, renderlist_t *b) {
  ...
  if (a->len + b->len >= cap)
    cap += b->cap + DEFAULT_RENDER_LIST_CAPACITY;
  ...
  // append arrays
  if (a->vert)
    memcpy(a->vert + a->len * 4, b->vert, b->len * 4 * sizeof(GLfloat));
  if (a->normal)
    memcpy(a->normal + a->len * 3, b->normal, b->len * 3 * sizeof(GLfloat));
  if (a->color)
    memcpy(a->color + a->len * 4, b->color, b->len * 4 * sizeof(GLfloat));
  if (a->secondary)
    memcpy(a->secondary + a->len * 4, b->secondary,
           b->len * 4 * sizeof(GLfloat));
  if (a->fogcoord)
    memcpy(a->fogcoord + a->len * 1, b->fogcoord, b->len * 1 * sizeof(GLfloat));
  for (int i = 0; i < a->maxtex; i++)
    if (a->tex[i])
      memcpy(a->tex[i] + a->len * 4, b->tex[i], b->len * 4 * sizeof(GLfloat));
  // indices
  if (!a->mode_inits)
    list_add_modeinit(a, a->mode_init);
  if (ilen_a || ilen_b || mode_needindices(a->mode) ||
      mode_needindices(b->mode) ||
      (a->mode != b->mode && (a->mode == GL_QUADS || b->mode == GL_QUADS))) {
    // alloc or realloc a->indices first...
    ilen_b = renderlist_getindicesize(b);
    prepareadd_renderlist(a, ilen_b);
    // then append b
    doadd_renderlist(a, b->mode, b->indices, b->indices ? b->ilen : b->len,
                     ilen_b);
  }
  // lenghts
  a->len += b->len;
  if (a->mode_inits)
    list_add_modeinit(a, b->mode);
...
renderlist_t *extend_renderlist(renderlist_t *list) {
  if ((list->prev != NULL) && ispurerender_renderlist(list) &&
    ...
    memcpy(new->lastSecondaryColors, list->lastSecondaryColors,
           3 * sizeof(GLfloat));
    ...
    memcpy(new->lastSecondaryColors, list->lastSecondaryColors,
           3 * sizeof(GLfloat));
    memcpy(new->lastColors,
           (list->post_color) ? list->post_colors : list->lastColors,
           4 * sizeof(GLfloat));
  ...
  while (a) {
    if (ispurerender_renderlist(a) && islistscompatible_renderlist(list, a)) {
      ...
      if (list->calls.len > 0) {
        ...
        /*
        list->calls.calls =
        (packed_call_t**)malloc(sizeof(packed_call_t*)*a->calls.cap); for (int i
        = 0; i < list->calls.len; i++) { list->calls.calls[i] =
        glCopyPackedCall(a->calls.calls[i]);
        }*/
      }
      if (list->len) {
        ++(*list->shared_arrays);
...
#define PROCESS(W, T, C)                                                       \
  if (list->W) {                                                               \
    ...
    khint_t k;                                                                 \
    int ret;                                                                   \
    kh_foreach_value(a->W, m, k = kh_put(W, list->W, C, &ret);                 \
...
void free_renderlist(renderlist_t *list) {
  ...
  do {
    ...
    if ((list->calls.len > 0) &&
        (!list->shared_calls || ((*list->shared_calls)--) == 0)) {
      if (list->shared_calls)
        free(list->shared_calls);
    ...
    if (!list->use_glstate) {
      ...
      if (!list->shared_indices || ((*list->shared_indices)--) == 0) {
        if (list->shared_indices)
          free(list->shared_indices);
        if (list->indices)
          free(list->indices);
      }
    } else
    ...
    if (list->material) {
      rendermaterial_t *m;
      kh_foreach_value(list->material, m, free(m);)
    ...
    if (list->texgen) {
      rendertexgen_t *m;
      kh_foreach_value(list->texgen, m, free(m);)
          kh_destroy(texgen, list->texgen);
    }
    if (list->texenv) {
      rendertexenv_t *m;
      kh_foreach_value(list->texenv, m, free(m);)
...
void resize_renderlist(renderlist_t *list) {
  if (list->use_glstate) {
    if (list->len >= glstate->merger_cap) {
      ...
      realloc_merger_sublist(glstate->merger_master, 4 * 5,
                             glstate->merger_cap);
      ...
      if (list->tex[0])
        list->tex[0] = glstate->merger_master + 4 + 4;
      if (list->tex[1])
        list->tex[1] = glstate->merger_master + 4 + 4 + 4;
      if (list->fogcoord)
      ...
      for (int a = 2; a < list->maxtex; a++)
        if (list->tex[a])
          list->tex[a] = glstate->merger_tex[a - 2];
    }
  } else {
...
void resize_merger_indices(int cap) {
  if (cap < glstate->merger_indice_cap)
    return;
  glstate->merger_indice_cap = ((glstate->merger_indice_cap + cap + 512) >> 9)
                               << 9;
  glstate->merger_indices = (GLushort *)realloc(
      glstate->merger_indices, glstate->merger_indice_cap * sizeof(GLushort));
}
...
void resize_indices_renderlist(renderlist_t *list, int n) {
  ...
  if (list->use_glstate) {
    ...
    list->indices =
        (GLushort *)realloc(list->indices, list->indice_cap * sizeof(GLushort));
  }
}
...
void adjust_renderlist(renderlist_t *list) {
  ...
  if (hardext.esversion == 1) // no more adjustment needed for ES2
    for (int a = 0; a < list->maxtex; a++) {
      ...
      gltexture_t *bound =
          (itarget >= 0) ? glstate->texture.bound[a][itarget] : NULL;
      ...
      if ((list->tex[a]) && (itarget == ENABLED_TEXTURE_RECTANGLE) && (bound)) {
        tex_coord_rect_arb(list->tex[a], list->tex_stride[a] >> 2, list->len,
...
renderlist_t *recycle_renderlist(renderlist_t *list, GLenum mode) {
  if (isempty_renderlist(list) ||
      (ispurerender_renderlist(list) && list->len == 0)) {
    list->mode_init = mode;
    list->mode = mode;
    list->stage = STAGE_DRAW;
    if (list->post_color) {
      list->post_color = 0;
      rlColor4f(list, list->post_colors[0], list->post_colors[1],
                list->post_colors[2], list->post_colors[3]);
    }
    if (list->post_normal) {
      list->post_normal = 0;
      rlNormal3f(list, list->post_normals[0], list->post_normals[1],
                 list->post_normals[2]);
    }
...
#define post_expand                                                            \
  ...
  case GL_LINE_LOOP:
    pre_expand;
    renderlist_lineloop_lines(
        list->indices, list->indices ? list->ilen : list->len, indices, 0);
    post_expand;
    list->mode = GL_LINES;
    break;
  case GL_LINE_STRIP:
    pre_expand;
    renderlist_linestrip_lines(
        list->indices, list->indices ? list->ilen : list->len, indices, 0);
    post_expand;
    list->mode = GL_LINES;
    break;
