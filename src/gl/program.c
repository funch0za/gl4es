...
void APIENTRY_GL4ES gl4es_glAttachShader(GLuint program, GLuint shader) {
  ...
  if (glprogram->attach_cap == glprogram->attach_size) {
    ...
    glprogram->attach = (GLuint *)realloc(
        glprogram->attach, sizeof(GLuint) * glprogram->attach_cap);
  }
...
GLuint APIENTRY_GL4ES gl4es_glCreateProgram(void) {
  ...
  if (k == kh_end(programs)) {
    ...
    glprogram = kh_value(programs, k) =
        (program_t *)calloc(1, sizeof(program_t));
  } else {
    glprogram = kh_value(programs, k);
...
void deleteProgram(program_t *glprogram, khint_t k_program) {
  ...
  if (glprogram->uniform) {
    uniform_t *m;
    kh_foreach_value(glprogram->uniform, m, free(m->name); free(m);)
...
void APIENTRY_GL4ES gl4es_glDeleteProgram(GLuint program) {
  ...
  if (!glstate)
    return; // in case a program delete shaders after deleteing all context
  FLUSH_BEGINEND;
  CHECK_PROGRAM(void, program)
  // send to hardware
  ...
  for (int i = 0; i < glprogram->attach_size; i++) {
    actually_detachshader(
        glprogram->attach[i]); // auto delete if marked as delete!
  }
    ...
    kh_foreach_value(
        glprogram->attribloc, attribloc, if (attribloc->real_index == index) {
          ...
          if (size)
            *size = attribloc->size;
          if (length)
            *length = strlen(attribloc->glname);
          if (bufSize && name) {
    ...
    kh_foreach_value(
        glprogram->uniform, m, if (m->internal_id == index) {
          ...
          DBG(printf(" found %s (%zd), type=%s, size=%d\n", m->name,
                     strlen(m->name), PrintEnum(m->type), m->size);)
          return;
        });
  }
  // end
...
const char *getFakeProgramInfo(program_t *glprogram) {
  return (glprogram->linked) ? ((glprogram->validated) ? validated : linked)
                             : notlinked;
}

void APIENTRY_GL4ES gl4es_glGetProgramInfoLog(GLuint program, GLsizei maxLength,
                                              GLsizei *length,
                                              GLchar *infoLog) {
...
void APIENTRY_GL4ES gl4es_glGetProgramiv(GLuint program, GLenum pname,
  ...
  case GL_ACTIVE_UNIFORM_MAX_LENGTH: {
    ...
    kh_foreach_value(glprogram->uniform, m,
                     if (l < strlen(m->name) + 1) l =
                         strlen(m->name) + 1;) *params = l;
  } break;
...
static void clear_program(program_t *glprogram) {
  ...
  if (glprogram->attribloc) {
    ...
    kh_foreach(glprogram->attribloc, k, m, free(m->name); free(m);
  ...
  if (glprogram->uniform) {
    uniform_t *m;
    khint_t k;
    kh_foreach(glprogram->uniform, k, m, free(m->name); free(m);
...
static void fill_program(program_t *glprogram) {
  ...
  int tu_idx = 0;
  for (int i = 0; i < n; i++) {
    gles_glGetActiveUniform(glprogram->id, i, maxsize, NULL, &size, &type,
                            name);
    DBG(e2 = gles_glGetError();)
    DBG(if (e2 == GL_NO_ERROR)) {
      // remove any ending "[]" that could be present
      if (name[strlen(name) - 1] == ']' && strrchr(name, '['))
        (*strrchr(name, '[')) = '\0';
      ...
      if (id != -1) {
        for (int j = 0; j < size; j++) {
          ...
          gluniform->builtin =
              builtin_CheckUniform(glprogram, name, id, size - j);
          ...
          if (type == GL_SAMPLER_CUBE) {
            ...
            glprogram->texunits[tu_idx].req_tu =
                glprogram->texunits[tu_idx].act_tu = 0;
            ++tu_idx;
          ...
          DBG(printf(" uniform #%d : \"%s\"%s type=%s size=%d\n", id,
                     gluniform->name, gluniform->builtin ? " (builtin) " : "",
                     PrintEnum(gluniform->type), gluniform->size);)
          if (gluniform->size == 1)
            ++glprogram->num_uniform;
          id++;
        }
  ...
  // Maybe Sampler uniform should not be initialized to 0, but to -1, to be sure
  ...
  if (glprogram->uniform) {
    ...
    kh_foreach(glprogram->uniform, k, m,
               if (m->type == GL_SAMPLER_2D || m->type == GL_SAMPLER_CUBE)
                   memset((char *)glprogram->cache.cache + m->cache_offs, 0xff,
                          m->cache_size);)
  }
  ...
  for (int i = 0; i < n; i++) {
    DBG(e2 = gles_glGetError();)
    DBG(if (e2 == GL_NO_ERROR)) {
      gles_glGetActiveAttrib(glprogram->id, i, maxsize, NULL, &size, &type,
                             name);
      ...
      if (id != -1) {
        ...
        DBG(printf(" attrib #%d : \"%s\"%s type=%s size=%d\n", id,
                   glattribloc->name, builtin ? " (builtin) " : "",
                   PrintEnum(glattribloc->type), glattribloc->size);)
      }
    }
...
int gl4es_useProgramBinary(GLuint program, int length, GLenum format,
                           const void *binary) {
...
int gl4es_getProgramBinary(GLuint program, int *length, GLenum *format,
                           void **binary) {
  if (hardext.prgbin_n == 0)
    return 0;
...
void APIENTRY_GL4ES gl4es_glLinkProgram(GLuint program) {
  ...
  if (!glprogram->last_vert) {
    glprogram->default_need =
        (shaderconv_need_t *)malloc(sizeof(shaderconv_need_t));
  ...
  // check if Built-in VA are used, and if so, bind them to their proper
  // location
  if (glprogram->last_vert)
    for (int i = 0; i < ATT_MAX; ++i) {
      const char *attribute =
          hasBuiltinAttrib(glprogram->last_vert->converted, i);
      if (attribute)
...
void APIENTRY_GL4ES gl4es_glValidateProgram(GLuint program) {
  ...
  if (gles_glValidateProgram) {
    ...
    GLenum err = gles_glGetError();
    gles_glGetProgramiv(glprogram->id, GL_VALIDATE_STATUS,
                        (GLint *)&glprogram->valid_result);
    errorShim(err);
...
AliasExport(void, glGetProgramInfoLog, ,
            (GLuint program, GLsizei maxLength, GLsizei *length,
             GLchar *infoLog));
...
AliasExport(GLvoid, glBindAttribLocation, ARB,
...
AliasExport(GLint, glGetAttribLocation, ARB,
...
GLvoid APIENTRY_GL4ES gl4es_glDetachObject(GLhandleARB containerObj,
...
GLvoid APIENTRY_GL4ES gl4es_glAttachObject(GLhandleARB containerObj,
                                           GLhandleARB obj) {
...
GLvoid APIENTRY_GL4ES gl4es_glGetObjectParameterfv(GLhandleARB obj,
                                                   GLenum pname,
                                                   GLfloat *params) {
  if (!obj)
    return;
...
GLvoid APIENTRY_GL4ES gl4es_glGetObjectParameteriv(GLhandleARB obj,
                                                   GLenum pname,
                                                   GLint *params) {
  if (!obj)
    return;
  ...
  // hack, some programs don't take into account that the length returned by
  // GL_OBJECT_INFO_LOG_LENGTH_ARB exclude the '\0'.. so adding it here
  if (pname == GL_INFO_LOG_LENGTH)
    (*params)++;
}

GLvoid APIENTRY_GL4ES gl4es_glGetInfoLog(GLhandleARB obj, GLsizei maxLength,
...
GLvoid APIENTRY_GL4ES gl4es_glGetAttachedObjects(GLhandleARB containerObj,
                                                 GLsizei maxCount,
                                                 GLsizei *count,
                                                 GLhandleARB *obj) {
...
AliasExport(GLvoid, glGetObjectParameterfv, ARB,
            (GLhandleARB obj, GLenum pname, GLfloat *params));
AliasExport(GLvoid, glGetObjectParameteriv, ARB,
            (GLhandleARB obj, GLenum pname, GLint *params));
AliasExport(GLvoid, glGetInfoLog, ARB,
...
AliasExport(GLvoid, glGetAttachedObjects, ARB,
            (GLhandleARB containerObj, GLsizei maxCount, GLsizei *count,
             GLhandleARB *obj));
AliasExport(GLint, glGetUniformLocation, ARB,
            (GLhandleARB programObj, const GLcharARB *name));
