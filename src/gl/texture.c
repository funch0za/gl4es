...
static int is_fake_compressed_rgba(GLenum internalformat) {
  if (internalformat == GL_COMPRESSED_RGBA)
    return 1;
...
static void *swizzle_texture(GLsizei width, GLsizei height, GLenum *format,
                             GLenum *type, GLenum intermediaryformat,
                             GLenum internalformat, const GLvoid *data,
                             gltexture_t *bound) {
  int convert = 0;
  ...
  if (is_fake_compressed_rgb(intermediaryformat))
    intermediaryformat = GL_RGB;
  if (is_fake_compressed_rgba(intermediaryformat))
    intermediaryformat = GL_RGBA;
  if (is_fake_compressed_rgb(internalformat))
    internalformat = GL_RGB;
  if (is_fake_compressed_rgba(internalformat))
    internalformat = GL_RGBA;
  if (intermediaryformat == GL_COMPRESSED_LUMINANCE)
    intermediaryformat = GL_LUMINANCE;
  if (internalformat == GL_COMPRESSED_LUMINANCE)
  ...
  if (*format != intermediaryformat || intermediaryformat != internalformat) {
    ...
    if ((*type) == GL_HALF_FLOAT)
      (*type) =
          GL_HALF_FLOAT_OES; // the define is different between GL and GLES...
    switch (*format) {
    case GL_R:
    case GL_RED:
      if (!hardext.rgtex) {
    ...
    case GL_DEPTH_COMPONENT32:
      if (hardext.depthtex) {
        if (dest_type == GL_UNSIGNED_BYTE) {
          dest_type = (*format == GL_DEPTH_COMPONENT32 ||
  ...
  if (data) {
    if (convert) {
      ...
      if (!pixel_convert(data, &pixels, width, height, *format, *type,
      ...
      *format = dest_format;
      if (dest_format != internalformat) {
        ...
        bound->format = dest_format;
        bound->type = dest_type;
        if (!pixel_convert(pixels, &pix2, width, height, *format, *type,
                           dest_format, dest_type, 0,
                           glstate->texture.unpack_align)) {
          printf("LIBGL: swizzle error: (%s, %s -> %s, %s)\n",
      ...
      if (raster_need_transform())
        if (!pixel_transform(data, &pixels, width, height, *format, *type,
                             glstate->raster.raster_scale,
                             glstate->raster.raster_bias)) {
          printf("LIBGL: swizzle/convert error: (%s, %s -> %s, %s)\n",
                 PrintEnum(*format), PrintEnum(*type), PrintEnum(dest_format),
                 PrintEnum(dest_type));
          pix2 = pixels;
        }
    ...
    bound->inter_type = dest_type;
    if (convert) {
      internal2format_type(internalformat, &dest_format,
                           &dest_type); // in case they are differents
      *type = dest_type;
      *format = dest_format;
    }
    bound->format = dest_format;
...
GLenum swizzle_internalformat(GLenum *internalformat, GLenum format,
  ...
  case GL_RGB5:
    ...
    break;
  case GL_RGB:
    if (globals4es.avoid16bits == 0 && format == GL_RGB &&
        type == GL_UNSIGNED_SHORT_5_6_5) {
      sret = ret = GL_RGB5;
      break;
    }
  ...
  case GL_RGB5_A1:
    ...
    break;
  case GL_RGBA:
    if (globals4es.avoid16bits == 0 && format == GL_RGBA &&
        type == GL_UNSIGNED_SHORT_5_5_5_1) {
      sret = ret = GL_RGB5_A1;
      break;
    }
  ...
  case GL_COMPRESSED_RGBA_S3TC_DXT3_EXT: // not good, but there is no DXT3
                                         // compressor
...
static int get_shrinklevel(int width, int height, int level) {
  ...
  case 7: // only > 512 /2 , but not for empty texture
    if (((mipwidth % 2 == 0) && (mipheight % 2 == 0)) &&
        (((mipwidth > 512) && (mipheight > 8)) ||
         ((mipheight > 512) && (mipwidth > 8)))) {
      shrink = 1;
    }
    break;
  case 3: // only > 256 /2
    if (((mipwidth % 2 == 0) && (mipheight % 2 == 0)) &&
        (((mipwidth > 256) && (mipheight > 8)) ||
         ((mipheight > 256) && (mipwidth > 8)))) {
      shrink = 1;
    }
    break;
  ...
  case 5: // every > 256 is downscaled to 256, but not for empty texture
    if (((mipwidth % 4 == 0) && (mipheight % 4 == 0)) &&
      ...
      if ((mipwidth > 256) || (mipheight > 256)) {
        while (((mipwidth > 256) && (mipheight > 4)) ||
               ((mipheight > 256) && (mipwidth > 4))) {
          width /= 2;
          height /= 2;
          mipwidth /= 2;
          mipheight /= 2;
    ...
    break;
  case 6: // only > 128 /2, >=512 is downscaled to 256, but not for empty
          // texture
    if (((mipwidth % 2 == 0) && (mipheight % 2 == 0)) &&
        (((mipwidth > 128) && (mipheight > 8)) ||
         ((mipheight > 128) && (mipwidth > 8)))) {
      if ((mipwidth >= 512) || (mipheight >= 512)) {
        while (((mipwidth > 256) && (mipheight > 8)) ||
               ((mipheight > 256) && (mipwidth > 8))) {
          width /= 2;
          height /= 2;
          mipwidth /= 2;
  ...
  case 8: // advertise *4 max texture size, but >2048 are shrinked to 2048
    ...
    break;
  case 9: // advertise 8192 max texture size, but >4096 are quadshrinked and
          // >512 are shrinked, but not for empty texture
    if ((mipwidth > hardext.maxsize * 2) || (mipheight > hardext.maxsize * 2)) {
      shrink = 2;
    } else if (mipwidth > (hardext.maxsize >> 2) ||
               mipheight > (hardext.maxsize >> 2)) {
      shrink = 1;
    }
    break;
  case 10: // advertise 8192 max texture size, but >2048 are quadshrinked and
           // >512 are shrinked, but not for empty texture
...
void APIENTRY_GL4ES gl4es_glTexImage2D(GLenum target, GLint level,
                                       ...
                                       const GLvoid *data) {
  DBG(printf(
          "glTexImage2D on target=%s with unpack_row_length(%i), size(%i,%i) "
          "and skip(%i,%i), format(internal)=%s(%s), type=%s, data=%p, "
          "level=%i (mipmap_need=%i, mipmap_auto=%i, base_level=%i, "
          "max_level=%i) => texture=%u (streamed=%i), "
          "glstate->list.compiling=%d\n",
          ...
          (internalformat == 3)
              ? "3"
              : (internalformat == 4 ? "4" : PrintEnum(internalformat)),
          PrintEnum(type), data, level,
          glstate->texture.bound[glstate->texture.active][what_target(target)]
              ->mipmap_need,
          glstate->texture.bound[glstate->texture.active][what_target(target)]
              ->mipmap_auto,
          glstate->texture.bound[glstate->texture.active][what_target(target)]
  ...
  // Special case when resizing an attached to FBO texture, that is attached to
  // depth and/or stencil => resizing is specific then
  if (bound->binded_fbo &&
      (bound->binded_attachment == GL_DEPTH_ATTACHMENT ||
    ...
    if (bound->binded_attachment == GL_DEPTH_ATTACHMENT ||
      ...
      if (bound->renderdepth) {
        ...
        gles_glTexImage2D(GL_TEXTURE_2D, 0, bound->format, bound->nwidth,
                          bound->nheight, 0, bound->format, bound->type, NULL);
      }
    }
    if ((bound->binded_attachment == GL_STENCIL_ATTACHMENT ||
      ...
      gl4es_glRenderbufferStorage(GL_RENDERBUFFER, GL_STENCIL_INDEX8, nwidth,
                                  nheight);
      gl4es_glBindRenderbuffer(GL_RENDERBUFFER, 0);
    }
    // all done, exit
  ...
  if (globals4es.automipmap) {
    if (level > 0)
      if ((globals4es.automipmap == 1) || (globals4es.automipmap == 3) ||
          bound->mipmap_need) {
  ...
  if (((width >> shrink) == 0) && ((height >> shrink) == 0))
    return; // nothing to do
  if (datab) {

    // implements GL_UNPACK_ROW_LENGTH
    if ((glstate->texture.unpack_row_length &&
         glstate->texture.unpack_row_length != width) ||
        glstate->texture.unpack_skip_pixels ||
        glstate->texture.unpack_skip_rows) {
      int imgWidth, pixelSize, dstWidth;
      pixelSize = pixel_sizeof(format, type);
      imgWidth = ((glstate->texture.unpack_row_length)
      ...
      src += glstate->texture.unpack_skip_pixels * pixelSize +
             glstate->texture.unpack_skip_rows * imgWidth;
    ...
    GLvoid *old = pixels;
    pixels = (GLvoid *)swizzle_texture(width, height, &format, &type,
                                       internalformat, new_format, old, bound);
    ...
    if (bound->shrink != 0) {
      ...
      case 11:
        if ((mipwidth > 1) && (mipheight > 1)) {
          ...
          if (ratiox == 0.5f && ratioy == 0.5f && npot(width) == width &&
              npot(height) == height) {
            ...
            bound->ratioy = ratioy;
            bound->useratio = 1;
            pixel_scale(pixels, &out, width, height, newwidth, newheight,
                        format, type);
          }
...
#ifdef TEXSTREAM
    if (globals4es.texstream &&
            (target == GL_TEXTURE_2D || target == GL_TEXTURE_RECTANGLE_ARB) &&
            (width >= 256 && height >= 224) &&
            ((internalformat == GL_RGB) || (internalformat == 3) ||
             (internalformat == GL_RGB8) || (internalformat == GL_BGR) ||
             (internalformat == GL_RGB5) || (internalformat == GL_RGB565)) ||
      ...
      if (bound->streamingID > -1) { // success
        ...
        bound->streamed = true;
        glsampler_t *sampler =
            glstate->samplers.sampler[glstate->texture.active];
        if (!sampler)
          sampler = &bound->sampler;
        ApplyFilterID(bound->streamingID, sampler->min_filter,
                      sampler->mag_filter);
        GLboolean tmp =
            IS_ANYTEX(glstate->enable.texture[glstate->texture.active]);
        LOAD_GLES(glDisable);
        LOAD_GLES(glEnable);
        if (tmp)
          gles_glDisable(GL_TEXTURE_2D);
        ActivateStreaming(
            bound->streamingID); // Activate the newly created texture
        format = GL_RGB;
        type = GL_UNSIGNED_SHORT_5_6_5;
        if (tmp)
...
#endif
    if (!bound->streamed)
      swizzle_texture(width, height, &format, &type, internalformat, new_format,
                      NULL, bound); // convert format even if data is NULL
    if (bound->shrink != 0) {
      switch (globals4es.texshrink) {
      case 1: // everything / 2
      case 11:
  ...
  */
  ...
  int limitednpot = 0;
  {
    ...
    bound->npot =
        (nheight != height ||
         nwidth !=
             width); // hardware that fully support NPOT doesn't care anyway
    if (bound->npot) {
      if (target == GL_TEXTURE_RECTANGLE_ARB && hardext.npot)
        limitednpot = 1;
               ...
               (wrap_npot(bound->sampler.wrap_s) &&
                wrap_npot(bound->sampler.wrap_t)))
        limitednpot = 1;
      else if (hardext.esversion > 1 && hardext.npot == 1 &&
               (!bound->mipmap_auto ||
                !minmag_npot(bound->sampler.min_filter) ||
...
#undef MIN_SIZE
    ...
    if (bound->npot) {
      if (limitednpot && rtarget == GL_TEXTURE_2D) {
      ...
      } else if (!wrap_npot(bound->sampler.wrap_s) ||
                 !wrap_npot(bound->sampler.wrap_t)) {
        // resize to npot boundaries (not ideal if the wrap value is change
        // after upload of the texture)
        ...
        GLvoid *out = pixels;
        if (pixels)
          pixel_scale(pixels, &out, width, height, nwidth, nheight, format,
                      type);
        if (out != pixels && pixels != datab)
          free(pixels);
        pixels = out;
    ...
    if (!(globals4es.texstream && bound->streamed)) {
      ...
      if (height != nheight || width != nwidth) {
        ...
        DBG(CheckGLError(1);)
        if (pixels) {
          gles_glTexSubImage2D(rtarget, level, 0, 0, width, height, format,
                               type, pixels);
          DBG(CheckGLError(1);)
        }
#ifdef NO_1x1
        if (level == 0 && (width == 1 && height == 1 && pixels)) {
          ...
          // also, don't keep the fact we have resized, the non-adjusted
          // coordinates will work (as the texture is enlarged)
          if (width == 1) {
            gles_glTexSubImage2D(rtarget, level, 1, 0, width, height, format,
                                 type, pixels);
            nwidth = 1;
          }
          if (height == 1) {
            gles_glTexSubImage2D(rtarget, level, 0, 1, width, height, format,
                                 type, pixels);
            nheight = 1;
          }
          if (width == 1 &&
              height == 1) { // create a manual mipmap just in case_state
            gles_glTexSubImage2D(rtarget, level, 1, 1, width, height, format,
                                 type, pixels);
            gles_glTexImage2D(rtarget, 1, format, 1, 1, 0, format, type,
                              pixels);
          }
        }
#endif
        ...
        errorGL();
        gles_glTexImage2D(rtarget, level, format, width, height, border, format,
                          type, pixels);
        DBG(CheckGLError(1);)
      }
      // check if base_level is set... and calculate lower level mipmap
      if (bound->base_level == level &&
          !(bound->max_level == level && level == 0)) {
        int leveln = level, nw = width, nh = height, nww = nwidth,
            nhh = nheight;
        ...
        while (leveln) {
          ...
          nhh <<= 1;
          --leveln;
          gles_glTexImage2D(rtarget, leveln, format, nww, nhh, border, format,
                            type, (pot) ? ndata : NULL);
          if (!pot && pixels)
            gles_glTexSubImage2D(rtarget, leveln, 0, 0, nw, nh, format, type,
                                 ndata);
        }
        if (ndata != pixels)
          free(ndata);
      ...
      if (((bound->max_level == level && (level || bound->mipmap_need)) ||
        ...
        if (globals4es.automipmap == 5 && level == 1)
          bound->mipmap_done = 1;
        int leveln = level, nw = nwidth, nh = nheight, nww = width,
            nhh = height;
        int pot = (nh == nhh && nw == nww);
        void *ndata = pixels;
        while (nw != 1 || nh != 1) {
          ...
          nhh = nlevel(nhh, 1);
          ++leveln;
          gles_glTexImage2D(rtarget, leveln, format, nw, nh, border, format,
                            type, (pot) ? ndata : NULL);
          if (!pot && pixels)
            gles_glTexSubImage2D(rtarget, leveln, 0, 0, nww, nhh, format, type,
                                 ndata);
        }
        if (ndata != pixels)
          free(ndata);
      ...
      /*if (bound && bound->mipmap_need && !bound->mipmap_auto &&
         (globals4es.automipmap!=3)) gles_glTexParameteri( rtarget,
         GL_GENERATE_MIPMAP, GL_FALSE );*/
    } else {
      if (pixels)
        gl4es_glTexSubImage2D(
            rtarget, level, 0, 0, width, height, format, type,
  ...
  if ((target == GL_TEXTURE_2D) && globals4es.texcopydata &&
    ...
    if (datab) {
      if (!pixel_convert(pixels, &bound->data, width, height, format, type,
...
void APIENTRY_GL4ES gl4es_glTexSubImage2D(GLenum target, GLint level,
  ...
  noerrorShim();
  DBG(printf(
          "glTexSubImage2D on target=%s with unpack_row_length(%d), "
          "size(%d,%d), pos(%d,%d) and skip={%d,%d}, format=%s, type=%s, "
          "level=%d(base=%d, max=%d), mipmap={need=%d, auto=%d}, texture=%u, "
          "data=%p(vao=%p)\n",
          PrintEnum(target), glstate->texture.unpack_row_length, width, height,
          xoffset, yoffset, glstate->texture.unpack_skip_pixels,
          glstate->texture.unpack_skip_rows, PrintEnum(format), PrintEnum(type),
          level,
          glstate->texture.bound[glstate->texture.active][itarget]->base_level,
  ...
  if (globals4es.texstream && (bound->streamed)) {
    ...
    if (!pixel_convert(old, &tmp, width, height, format, type, GL_RGB,
                       GL_UNSIGNED_SHORT_5_6_5, bound->width,
                       glstate->texture.unpack_align)) {
      printf("LIBGL: swizzle error: (%#4x, %#4x -> GL_RGB, "
...
#endif
  {
    if (!pixel_convert(old, &pixels, width, height, format, type,
                       bound->inter_format, bound->inter_type, 0,
                       glstate->texture.unpack_align)) {
      printf("LIBGL: Error in pixel_convert while glTexSubImage2D\n");
    } else {
      format = bound->inter_format;
      type = bound->inter_type;
      if (bound->inter_format != bound->format ||
          bound->inter_type != bound->type) {
        GLvoid *pix2 = pixels;
        if (!pixel_convert(pixels, &pix2, width, height, format, type,
                           bound->format, bound->type, 0,
                           glstate->texture.unpack_align)) {
          printf("LIBGL: Error in pixel_convert while glTexSubImage2D\n");
        }
  ...
  if (bound->shrink || bound->useratio) {
    ...
    if (bound->useratio) {
      ...
      int newwidth = width * bound->ratiox;
      int newheight = height * bound->ratioy;
      pixel_scale(pixels, &old, width, height, newwidth, newheight, format,
                  type);
      width = newwidth;
      height = newheight;
  ...
  if (globals4es.texstream && bound->streamed) {
    /*    // copy the texture to the buffer
        ...
        for (int yy=0; yy<height; yy++) {
            memcpy(tmp+((yy+yoffset)*bound->width+xoffset)*2,
       pixels+(yy*width)*2, width*2);
        }*/
  } else {
    errorGL();
    gles_glTexSubImage2D(rtarget, level, xoffset, yoffset, width, height,
                         format, type, pixels);
    DBG(CheckGLError(1);)
    // check if base_level is set... and calculate lower level mipmap
    if (bound->base_level == level &&
      ...
      while (leveln) {
        ...
        yy <<= 1;
        --leveln;
        gles_glTexSubImage2D(rtarget, leveln, xx, yy, nw, nh, format, type,
                             ndata);
      }
      if (ndata != pixels)
        free(ndata);
  ...
  /*if (bound->mipmap_need && !bound->mipmap_auto && (globals4es.automipmap!=3)
     && (!globals4es.texstream || (globals4es.texstream && !bound->streamed)))
      gles_glTexParameteri( rtarget, GL_GENERATE_MIPMAP, GL_FALSE );*/

  if ((target == GL_TEXTURE_2D) && globals4es.texcopydata &&
      ((globals4es.texstream && !bound->streamed) || !globals4es.texstream)) {
    // printf("*texcopy* glTexSubImage2D, xy=%i,%i, size=%i,%i=>%i,%i,
    // format=%s, type=%s, tex=%u\n", xoffset, yoffset, width, height,
    // bound->width, bound->height, PrintEnum(format), PrintEnum(type),
    // bound->glname);
    GLvoid *tmp = (char *)bound->data + (yoffset * bound->width + xoffset) * 4;
    if (!pixel_convert(pixels, &tmp, width, height, format, type, GL_RGBA,
                       GL_UNSIGNED_BYTE, bound->width,
                       glstate->texture.unpack_align))
      printf(
...
// 1d stubs
void APIENTRY_GL4ES gl4es_glTexImage1D(GLenum target, GLint level,
                                       GLint internalFormat, GLsizei width,
                                       GLint border, GLenum format, GLenum type,
                                       const GLvoid *data) {

  // TODO: maybe too naive to force GL_TEXTURE_2D here?
  gl4es_glTexImage2D(GL_TEXTURE_1D, level, internalFormat, width, 1, border,
                     format, type, data);
}
void APIENTRY_GL4ES gl4es_glTexSubImage1D(GLenum target, GLint level,
                                          GLint xoffset, GLsizei width,
                                          GLenum format, GLenum type,
                                          const GLvoid *data) {

  gl4es_glTexSubImage2D(GL_TEXTURE_1D, level, xoffset, 0, width, 1, format,
                        type, data);
}
...
void APIENTRY_GL4ES gl4es_glTexStorage1D(GLenum target, GLsizei levels,
                                         GLenum internalformat, GLsizei width) {
  DBG(printf("glTexStorage1D(%s, %d, %s, %d)\n", PrintEnum(target), levels,
             PrintEnum(internalformat), width);)
  gl4es_glTexImage1D(target, 0, internalformat, width, 0, GL_RGBA,
                     GL_UNSIGNED_BYTE, NULL);
}
void APIENTRY_GL4ES gl4es_glTexStorage2D(GLenum target, GLsizei levels,
                                         GLenum internalformat, GLsizei width,
                                         GLsizei height) {
  ...
  if (levels > 1 && isDXTc(internalformat)) {
    ...
    for (int i = 1; i <= mlevel; ++i)
      gl4es_glTexImage2D(target, i, internalformat, nlevel(width, i),
                         nlevel(height, i), 0, bound->format, bound->type,
                         NULL);
    noerrorShim();
    return;
  }
  ...
  for (int i = 1; i < levels; ++i)
    gl4es_glTexImage2D(target, i, internalformat, nlevel(width, i),
                       nlevel(height, i), 0, bound->format, bound->type, NULL);
...
// TexStorage
AliasExport(void, glTexStorage1D, ,
