   ...
   (((str)[1] == '\0') ||                                                      \
    (IS_SWIZ_VALUE((str)[1]) &&                                                \
     (((str)[2] == '\0') ||                                                    \
      (IS_SWIZ_VALUE((str)[2]) &&                                              \
       (((str)[3] == '\0') ||                                                  \
        (IS_SWIZ_VALUE((str)[3]) && ((str)[4] == '\0'))))))))
#define IS_NEW_STR_OR_SWIZZLE(str, t)                                          \
  (((str)[0] == ',') || ((t == 1) && IS_SWIZZLE(str)))
...
int compareTokenWith(const sCurStatus *curStatus, const char *compWith,
                     const int len) {
  return (getTokenLength(curStatus) == len)
             ? strncmp(curStatus->codePtr, compWith, len)
             : 1;
}
...
eToken readNextToken(sCurStatus *curStatus) {
  ...
  case '9':
    ...
    if ((curStatus->endOfToken[0] == '.') &&
        (curStatus->endOfToken[1] == '.')) {
      break;
    }
    ...
    if (*curStatus->endOfToken == '.') {
      ...
      if ((*curStatus->endOfToken >= '0') && (*curStatus->endOfToken <= '9')) {
        ...
        while ((*curStatus->endOfToken >= '0') &&
               (*curStatus->endOfToken <= '9')) {
  ...
  case '.': {
    ...
    if (*curStatus->endOfToken == '.') {
      ...
      break;
    } else if ((*curStatus->endOfToken < '0') ||
               (*curStatus->endOfToken > '9')) {
  ...
  case '$':
    ...
    curStatus->endOfToken = curStatus->codePtr + 1;
    while (
        ((*curStatus->endOfToken >= '0') && (*curStatus->endOfToken <= '9')) ||
    ...
    if ((curStatus->endOfToken == curStatus->codePtr + 3) &&
        (curStatus->codePtr[0] == 'E') && (curStatus->codePtr[1] == 'N') &&
        (curStatus->codePtr[2] == 'D')) {
...
int resolveAttrib(sCurStatus_NewVar *newVar, int vertex) {
  ...
  if (!tok) {
    ...
    if (!tok) {
      ...
      if (!IS_NONE_OR_SWIZZLE) {
        tok = popFIFO((sArray *)newVar);
        if ((tok[0] == '[') && (newVar->strLen >= 2) &&
            (newVar->strParts[1][0] == ']') &&
            (newVar->strParts[0][0] >= '0') &&
    ...
    if (!tok) {
      ...
      free(tok);
      pushArray((sArray *)&newVar->var->init,
                strdup("vec4(gl_FogFragCoord, 0., 0., 1.)"));
...
int resolveOutput(sCurStatus_NewVar *newVar, int vertex,
                  struct sSpecialCases *specialCases) {
  char *tok = popFIFO((sArray *)newVar);
  ...
  if (!tok) {
    ...
    if (!tok) {
      ...
      free(tok);
      pushArray((sArray *)&newVar->var->init,
                strdup("vec4(gl_Point.size, 0., 0., 0.)"));
...
char **resolveParam(sCurStatus_NewVar *newVar, int vertex, int type) {
  ...
  if (!tok) {
    ...
    if (!tok) {
      ...
      matrixNameMallocd =
          (char *)malloc((mtxNameLen + propLen + 2) * sizeof(char));
      ...
      if (newVar->strParts[0][0] == '[') {
        ...
        if ((sln[0] >= '0') && (sln[0] <= '9')) {
          ...
          if (!tok) {
            ...
            free(tok);
            matrixNameMallocd =
                (char *)malloc((4 * strlen(sln) + 149) * sizeof(char));
            sprintf(matrixNameMallocd,
                    "vec4(gl_LightSource[%s].constantAttenuation, "
                    "gl_LightSource[%s].linearAttenuation, "
                    "gl_LightSource[%s].quadraticAttenuation, "
                    "gl_LightSource[%s].spotExponent)",
                    sln, sln, sln, sln);
            char **r = (char **)calloc(2, sizeof(char *));
            r[0] = matrixNameMallocd;
            r[1] = NULL;
            ...
            if (!tok) {
              ARBCONV_DBG_RE(
                  "Failed to get param: state.light[%s].spot(tok NULL)\n", sln)
              return NULL;
          ...
          matrixNameMallocd =
              (char *)malloc((mtxNameLen + strlen(sln) + 8) * sizeof(char));
      ...
      if (!tok) {
        ...
        if (!tok) {
          ARBCONV_DBG_RE(
              "Failed to get param: state.lightmodel.front(tok NULL)\n")
          return NULL;
        } else if (!strcmp(tok, "scenecolor")) {
          free(tok);
          char **r = (char **)calloc(2, sizeof(char *));
        ...
        if (!tok) {
          ARBCONV_DBG_RE(
              "Failed to get param: state.lightmodel.back(tok NULL)\n")
          return NULL;
        } else if (!strcmp(tok, "scenecolor")) {
          free(tok);
          char **r = (char **)calloc(2, sizeof(char *));
      ...
      if (!tok) {
        ...
        free(tok);
        if (newVar->strLen &&
            !IS_NEW_STR_OR_SWIZZLE(newVar->strParts[0], type)) {
          int mvmtx = 0;
          int mvmtxsz = 0;
          if (newVar->strParts[0][0] == '[') {
            free(popFIFO((sArray *)newVar));
          ...
          if (newVar->strLen &&
            ...
            if (!strcmp(tok, "row") &&
              ...
              if (freeLast) {
                if (newVar->strParts[0][0] != ']') {
                  ARBCONV_DBG_RE(
                      "Failed to get param: [%s].row[%d..%d(not ])\n",
        ...
        if (newVar->strLen &&
          ...
          if (newVar->strLen &&
              !IS_NEW_STR_OR_SWIZZLE(newVar->strParts[0], type)) {
            tok = popFIFO((sArray *)newVar);
            if (!strcmp(tok, "row") && (newVar->strLen >= 3) &&
                (newVar->strParts[0][0] == '[') &&
                (newVar->strParts[1][0] >= '0') &&
                (newVar->strParts[1][0] <= '9')) {
              free(tok);
        ...
        if (newVar->strLen &&
          ...
          if (newVar->strLen &&
              !IS_NEW_STR_OR_SWIZZLE(newVar->strParts[0], type)) {
            tok = popFIFO((sArray *)newVar);
            if (!strcmp(tok, "row") && newVar->strLen &&
                (newVar->strParts[0][0] >= '0') &&
                (newVar->strParts[0][0] <= '9')) {
              free(tok);
              tok = popFIFO((sArray *)newVar);
    ...
    do {
      ...
      case TOK_FLOATCONST:
        ...
        ++valuesCnt;
        pseudoSt.curValue.newVar.state =
            3 * (pseudoSt.curValue.newVar.state / 3) + 2;
        break;
    ...
    } while (tok && (pseudoSt.status != ST_ERROR) &&
             (pseudoSt.curToken != TOK_RBRACE));
    free(tok);
    ...
    if (((((valuesCnt != 1) && (valuesCnt != 4))) ||
         appendString(&pseudoSt, ")", 1)) &&
   ...
   * <paramSingleItemDecl>
   ...
   \ V "state" "." "lightprod" "[" <stateLightNumber> "]" "." "front" "."
   "ambient" \ V "state" "." "lightprod" "[" <stateLightNumber> "]" "." "front"
   "." "diffuse" \ V "state" "." "lightprod" "[" <stateLightNumber> "]" "."
   "front" "." "specular" \ V "state" "." "lightprod" "[" <stateLightNumber> "]"
   "." "back" "." "ambient" \ V "state" "." "lightprod" "[" <stateLightNumber>
   "]" "." "back" "." "diffuse" \ V "state" "." "lightprod" "["
   <stateLightNumber> "]" "." "back" "." "specular" \ ! "state" "." "texgen" "."
   "eye" "." "s" \ ! "state" "." "texgen" "." "eye" "." "t" \ ! "state" "."
   "texgen" "." "eye" "." "r" \ ! "state" "." "texgen" "." "eye" "." "q" \ !
   "state" "." "texgen" "." "object" "." "s" \ ! "state" "." "texgen" "."
   "object" "." "t" \ ! "state" "." "texgen" "." "object" "." "r" \ ! "state"
   "." "texgen" "." "object" "." "q" \ ! "state" "." "texgen" "[" <texCoordNum>
   "]" "." "eye" "." "s" \ ! "state" "." "texgen" "[" <texCoordNum> "]" "."
   "eye" "." "t" \ ! "state" "." "texgen" "[" <texCoordNum> "]" "." "eye" "."
   "r" \ ! "state" "." "texgen" "[" <texCoordNum> "]" "." "eye" "." "q" \ !
   "state" "." "texgen" "[" <texCoordNum> "]" "." "object" "." "s" \ ! "state"
   "." "texgen" "[" <texCoordNum> "]" "." "object" "." "t" \ ! "state" "."
   "texgen" "[" <texCoordNum> "]" "." "object" "." "r" \ ! "state" "." "texgen"
   "[" <texCoordNum> "]" "." "object" "." "q" \ * "state" "." "fog" "." "color"
   \ * "state" "." "fog" "." "params"
   \ ! "state" "." "clip" "[" <stateClipPlaneNum> "]" "." "plane"
   \ ! "state" "." "point" "." "size"
   \ ! "state" "." "point" "." "attenuation"
   \ V "state" "." "matrix" "." "modelview" "." "row" "[" <stateMatrixRowNum>
   "]" \ V "state" "." "matrix" "." "modelview" "." "inverse" "." "row" "["
   <stateMatrixRowNum> "]" \ V "state" "." "matrix" "." "modelview" "."
   "transpose" "." "row" "[" <stateMatrixRowNum> "]" \ V "state" "." "matrix"
   "." "modelview" "." "invtrans" "." "row" "[" <stateMatrixRowNum> "]" \ V
   "state" "." "matrix" "." "modelview" "[" <stateModMatNum> "]" "." "row" "["
   <stateMatrixRowNum> "]" \ V "state" "." "matrix" "." "modelview" "["
   <stateModMatNum> "]" "." "inverse" "." "row" "[" <stateMatrixRowNum> "]" \ V
   "state" "." "matrix" "." "modelview" "[" <stateModMatNum> "]" "." "transpose"
   "." "row" "[" <stateMatrixRowNum> "]" \ V "state" "." "matrix" "."
   "modelview" "[" <stateModMatNum> "]" "." "invtrans" "." "row" "["
   <stateMatrixRowNum> "]" \ V "state" "." "matrix" "." "projection" "." "row"
   "[" <stateMatrixRowNum> "]" \ V "state" "." "matrix" "." "projection" "."
   "inverse" "." "row" "[" <stateMatrixRowNum> "]" \ V "state" "." "matrix" "."
   "projection" "." "transpose" "." "row" "[" <stateMatrixRowNum> "]" \ V
   "state" "." "matrix" "." "projection" "." "invtrans" "." "row" "["
   <stateMatrixRowNum> "]" \ V "state" "." "matrix" "." "mvp" "." "row" "["
   <stateMatrixRowNum> "]" \ V "state" "." "matrix" "." "mvp" "." "inverse" "."
   "row" "[" <stateMatrixRowNum> "]" \ V "state" "." "matrix" "." "mvp" "."
   "transpose" "." "row" "[" <stateMatrixRowNum> "]" \ V "state" "." "matrix"
   "." "mvp" "." "invtrans" "." "row" "[" <stateMatrixRowNum> "]" \ V "state"
   "." "matrix" "." "texture" "." "row" "[" <stateMatrixRowNum> "]" \ V "state"
   "." "matrix" "." "texture" "." "inverse" "." "row" "[" <stateMatrixRowNum>
   "]" \ V "state" "." "matrix" "." "texture" "." "transpose" "." "row" "["
   <stateMatrixRowNum> "]" \ V "state" "." "matrix" "." "texture" "." "invtrans"
   "." "row" "[" <stateMatrixRowNum> "]" \ V "state" "." "matrix" "." "texture"
   "[" <texCoordNum> "]" "." "row" "[" <stateMatrixRowNum> "]" \ V "state" "."
   "matrix" "." "texture" "[" <texCoordNum> "]" "." "inverse" "." "row" "["
   <stateMatrixRowNum> "]" \ V "state" "." "matrix" "." "texture" "["
   <texCoordNum> "]" "." "transpose" "." "row" "[" <stateMatrixRowNum> "]" \ V
   "state" "." "matrix" "." "texture" "[" <texCoordNum> "]" "." "invtrans" "."
   "row" "[" <stateMatrixRowNum> "]" \ ? "state" "." "matrix" "." "palette" "["
   <statePaletteMatNum> "]" "." "row" "[" <stateMatrixRowNum> "]" \ ? "state"
   "." "matrix" "." "palette" "[" <statePaletteMatNum> "]" "." "inverse" "."
   "row" "[" <stateMatrixRowNum> "]" \ ? "state" "." "matrix" "." "palette" "["
   <statePaletteMatNum> "]" "." "transpose" "." "row" "[" <stateMatrixRowNum>
   "]" \ ? "state" "." "matrix" "." "palette" "[" <statePaletteMatNum> "]" "."
   "invtrans" "." "row" "[" <stateMatrixRowNum> "]" \ ? "state" "." "matrix" "."
   "program" "[" <stateProgramMatNum> "]" "." "row" "[" <stateMatrixRowNum> "]"
   \ ? "state" "." "matrix" "." "program" "[" <stateProgramMatNum> "]" "."
   "inverse" "." "row" "[" <stateMatrixRowNum> "]" \ ? "state" "." "matrix" "."
   "program" "[" <stateProgramMatNum> "]" "." "transpose" "." "row" "["
   <stateMatrixRowNum> "]" \ ? "state" "." "matrix" "." "program" "["
   <stateProgramMatNum> "]" "." "invtrans" "." "row" "[" <stateMatrixRowNum> "]"
   ...
   \ V "{" <optionalSign> <floatConstant> "," <optionalSign> <floatConstant> ","
   <optionalSign> <floatConstant> "}" \ V "{" <optionalSign> <floatConstant> ","
   <optionalSign> <floatConstant> "," <optionalSign> <floatConstant> ","
   ...
   * <paramMultipleItem>
   ...
   \ V "state" "." "matrix" "." "modelview" "." "row" "[" <stateMatrixRowNum>
   ".." <stateMatrixRowNum> "]" \ V "state" "." "matrix" "." "modelview" "."
   "inverse" "." "row" "[" <stateMatrixRowNum> ".." <stateMatrixRowNum> "]" \ V
   "state" "." "matrix" "." "modelview" "." "transpose" "." "row" "["
   <stateMatrixRowNum> ".." <stateMatrixRowNum> "]" \ V "state" "." "matrix" "."
   "modelview" "." "invtrans" "." "row" "[" <stateMatrixRowNum> ".."
   <stateMatrixRowNum> "]" \ V "state" "." "matrix" "." "modelview" "["
   <stateModMatNum> "]" \ V "state" "." "matrix" "." "modelview" "["
   <stateModMatNum> "]" "." "inverse" \ V "state" "." "matrix" "." "modelview"
   "[" <stateModMatNum> "]" "." "transpose" \ V "state" "." "matrix" "."
   "modelview" "[" <stateModMatNum> "]" "." "invtrans" \ V "state" "." "matrix"
   "." "modelview" "[" <stateModMatNum> "]" "." "row" "[" <stateMatrixRowNum>
   ".." <stateMatrixRowNum> "]" \ V "state" "." "matrix" "." "modelview" "["
   <stateModMatNum> "]" "." "inverse" "." "row" "[" <stateMatrixRowNum> ".."
   <stateMatrixRowNum> "]" \ V "state" "." "matrix" "." "modelview" "["
   <stateModMatNum> "]" "." "transpose" "." "row" "[" <stateMatrixRowNum> ".."
   <stateMatrixRowNum> "]" \ V "state" "." "matrix" "." "modelview" "["
   <stateModMatNum> "]" "." "invtrans" "." "row" "[" <stateMatrixRowNum> ".."
   <stateMatrixRowNum> "]" \ V "state" "." "matrix" "." "projection" \ V "state"
   "." "matrix" "." "projection" "." "inverse" \ V "state" "." "matrix" "."
   "projection" "." "transpose" \ V "state" "." "matrix" "." "projection" "."
   "invtrans" \ V "state" "." "matrix" "." "projection" "." "row" "["
   <stateMatrixRowNum> ".." <stateMatrixRowNum> "]" \ V "state" "." "matrix" "."
   "projection" "." "inverse" "." "row" "[" <stateMatrixRowNum> ".."
   <stateMatrixRowNum> "]" \ V "state" "." "matrix" "." "projection" "."
   "transpose" "." "row" "[" <stateMatrixRowNum> ".." <stateMatrixRowNum> "]"
   \ V "state" "." "matrix" "." "projection" "." "invtrans" "." "row" "["
   <stateMatrixRowNum> ".." <stateMatrixRowNum> "]" \ V "state" "." "matrix" "."
   "mvp" \ V "state" "." "matrix" "." "mvp" "." "inverse" \ V "state" "."
   "matrix" "." "mvp" "." "transpose" \ V "state" "." "matrix" "." "mvp" "."
   "invtrans" \ V "state" "." "matrix" "." "mvp" "." "row" "["
   <stateMatrixRowNum> ".." <stateMatrixRowNum> "]" \ V "state" "." "matrix" "."
   "mvp" "." "inverse" "." "row" "[" <stateMatrixRowNum> ".."
   <stateMatrixRowNum> "]" \ V "state" "." "matrix" "." "mvp" "." "transpose"
   "." "row" "[" <stateMatrixRowNum> ".." <stateMatrixRowNum> "]" \ V "state"
   "." "matrix" "." "mvp" "." "invtrans" "." "row" "[" <stateMatrixRowNum> ".."
   <stateMatrixRowNum> "]" \ V "state" "." "matrix" "." "texture" \ V "state"
   "." "matrix" "." "texture" "." "inverse" \ V "state" "." "matrix" "."
   "texture" "." "transpose" \ V "state" "." "matrix" "." "texture" "."
   "invtrans" \ V "state" "." "matrix" "." "texture" "." "row" "["
   <stateMatrixRowNum> ".." <stateMatrixRowNum> "]" \ V "state" "." "matrix" "."
   "texture" "." "inverse" "." "row" "[" <stateMatrixRowNum> ".."
   <stateMatrixRowNum> "]" \ V "state" "." "matrix" "." "texture" "."
   "transpose" "." "row" "[" <stateMatrixRowNum> ".." <stateMatrixRowNum> "]"
   \ V "state" "." "matrix" "." "texture" "." "invtrans" "." "row" "["
   <stateMatrixRowNum> ".." <stateMatrixRowNum> "]" \ V "state" "." "matrix" "."
   "texture" "[" <texCoordNum> "]" \ V "state" "." "matrix" "." "texture" "["
   <texCoordNum> "]" "." "inverse" \ V "state" "." "matrix" "." "texture" "["
   <texCoordNum> "]" "." "transpose" \ V "state" "." "matrix" "." "texture" "["
   <texCoordNum> "]" "." "invtrans" \ V "state" "." "matrix" "." "texture" "["
   <texCoordNum> "]" "." "row" "[" <stateMatrixRowNum> ".." <stateMatrixRowNum>
   "]" \ V "state" "." "matrix" "." "texture" "[" <texCoordNum> "]" "."
   "inverse" "." "row" "[" <stateMatrixRowNum> ".." <stateMatrixRowNum> "]" \ V
   "state" "." "matrix" "." "texture" "[" <texCoordNum> "]" "." "transpose" "."
   "row" "[" <stateMatrixRowNum> ".." <stateMatrixRowNum> "]" \ V "state" "."
   "matrix" "." "texture" "[" <texCoordNum> "]" "." "invtrans" "." "row" "["
   <stateMatrixRowNum> ".." <stateMatrixRowNum> "]" \ ? "state" "." "matrix" "."
   "palette" "[" <statePaletteMatNum> "]" \ ? "state" "." "matrix" "." "palette"
   "[" <statePaletteMatNum> "]" "." "inverse" \ ? "state" "." "matrix" "."
   "palette" "[" <statePaletteMatNum> "]" "." "transpose" \ ? "state" "."
   "matrix" "." "palette" "[" <statePaletteMatNum> "]" "." "invtrans" \ ?
   "state" "." "matrix" "." "palette" "[" <statePaletteMatNum> "]" "." "row" "["
   <stateMatrixRowNum> ".." <stateMatrixRowNum> "]" \ ? "state" "." "matrix" "."
   "palette" "[" <statePaletteMatNum> "]" "." "inverse" "." "row" "["
   <stateMatrixRowNum> ".." <stateMatrixRowNum> "]" \ ? "state" "." "matrix" "."
   "palette" "[" <statePaletteMatNum> "]" "." "transpose" "." "row" "["
   <stateMatrixRowNum> ".." <stateMatrixRowNum> "]" \ ? "state" "." "matrix" "."
   "palette" "[" <statePaletteMatNum> "]" "." "invtrans" "." "row" "["
   <stateMatrixRowNum> ".." <stateMatrixRowNum> "]" \ ? "state" "." "matrix" "."
   "program" "[" <stateProgramMatNum> "]" \ ? "state" "." "matrix" "." "program"
   "[" <stateProgramMatNum> "]" "." "inverse" \ ? "state" "." "matrix" "."
   "program" "[" <stateProgramMatNum> "]" "." "transpose" \ ? "state" "."
   "matrix" "." "program" "[" <stateProgramMatNum> "]" "." "invtrans" \ ?
   "state" "." "matrix" "." "program" "[" <stateProgramMatNum> "]" "." "row" "["
   <stateMatrixRowNum> ".." <stateMatrixRowNum> "]" \ ? "state" "." "matrix" "."
   "program" "[" <stateProgramMatNum> "]" "." "inverse" "." "row" "["
   <stateMatrixRowNum> ".." <stateMatrixRowNum> "]" \ ? "state" "." "matrix" "."
   "program" "[" <stateProgramMatNum> "]" "." "transpose" "." "row" "["
   <stateMatrixRowNum> ".." <stateMatrixRowNum> "]" \ ? "state" "." "matrix" "."
   "program" "[" <stateProgramMatNum> "]" "." "invtrans" "." "row" "["
   <stateMatrixRowNum> ".." <stateMatrixRowNum> "]" \ V "program" "." "env" "["
   <progEnvParamNum> "]" \ V "program" "." "env" "[" <progEnvParamNum> ".."
   <progEnvParamNum> "]" \ V "program" "." "local" "[" <progLocalParamNum> "]"
   \ V "program" "." "local" "[" <progLocalParamNum> ".." <progLocalParamNum>
   "]" \ V <optionalSign> <floatConstant> \ V "{" <optionalSign> <floatConstant>
   "}" \ V "{" <optionalSign> <floatConstant> "," <optionalSign> <floatConstant>
   "}" \ V "{" <optionalSign> <floatConstant> "," <optionalSign> <floatConstant>
   "," <optionalSign> <floatConstant> "}" \ V "{" <optionalSign> <floatConstant>
   "," <optionalSign> <floatConstant> "," <optionalSign> <floatConstant> ","
   ...
   * <paramSingleItemDecl>
   ...
   "." "back" "." "ambient" \ V "state" "." "lightprod" "[" <stateLightNumber>
   "]" "." "back" "." "diffuse" \ V "state" "." "lightprod" "["
   <stateLightNumber> "]" "." "back" "." "specular" \ ! "state" "." "texenv" "."
   "color" \ ! "state" "." "texenv" "[" <legacyTexUnitNum> "]" "." "color" \ *
   "state" "." "fog" "." "color" \ * "state" "." "fog" "." "params" \ ! "state"
   "." "depth" "." "range" \ V "state" "." "matrix" "." "modelview" "." "row"
   "[" <stateMatrixRowNum> "]" \ V "state" "." "matrix" "." "modelview" "."
   "inverse" "." "row" "[" <stateMatrixRowNum> "]" \ V "state" "." "matrix" "."
   "modelview" "." "transpose" "." "row" "[" <stateMatrixRowNum> "]" \ V "state"
   "." "matrix" "." "modelview" "." "invtrans" "." "row" "[" <stateMatrixRowNum>
   "]" \ V "state" "." "matrix" "." "modelview" "[" <stateModMatNum> "]" "."
   "row" "[" <stateMatrixRowNum> "]" \ V "state" "." "matrix" "." "modelview"
   "[" <stateModMatNum> "]" "." "inverse" "." "row" "[" <stateMatrixRowNum> "]"
   \ V "state" "." "matrix" "." "modelview" "[" <stateModMatNum> "]" "."
   "transpose" "." "row" "[" <stateMatrixRowNum> "]" \ V "state" "." "matrix"
   "." "modelview" "[" <stateModMatNum> "]" "." "invtrans" "." "row" "["
  ...
  if (isMatrix) {
    ...
    for (int i = start; i <= end; ++i) {
      char *buf = (char *)calloc(
          mtxNameLen + 13, sizeof(char)); // Assume 32-bit array = 11 digits max
...
void parseToken(sCurStatus *curStatusPtr, int vertex, char **error_msg,
                struct sSpecialCases *specialCases) {
  if (((curStatusPtr->curToken == TOK_UNKNOWN) &&
       (curStatusPtr->status != ST_LINE_COMMENT)) ||
      (curStatusPtr->curToken == TOK_NULL)) {
    FAIL("Unknown token");
  }

  // TODO: replace '$' in variable names with something else that is also valid
  // in GLSL
  ...
  case ST_LINE_START:
    ...
    case TOK_IDENTIFIER: {
      ...
      eVariableType vtype;
      if ((inst =
               STR2INST(tok, &curStatusPtr->curValue.newInst.inst.saturated)) !=
          INST_UNK) {
        ...
        curStatusPtr->curValue.newInst.inst.codeLocation =
            curStatusPtr->codePtr;
    ...
    case VARTYPE_TEMP:
      ...
      case TOK_IDENTIFIER: {
        ...
        if (!strcmp(tok, "half")) {
          ...
          // Hopefully this doesn't make a free-after-free in case of error
          // (though it shouldn't)
          int ret;
          ...
          kh_val(curStatusPtr->varsMap, varIdx) =
      ...
      case TOK_COMMA: {
        ...
        int ret;
        khint_t varIdx =
            kh_put(variables, curStatusPtr->varsMap,
                   curStatusPtr->curValue.newVar.var->names[0], &ret);
        if (ret < 0) {
          FAIL("Unknown error");
        }
        kh_val(curStatusPtr->varsMap, varIdx) =
        ...
        pushArray((sArray *)&curStatusPtr->variables,
                  curStatusPtr->curValue.newVar.var);
        curStatusPtr->curValue.newVar.var =
            createVariable(curStatusPtr->curValue.newVar.var->type);
        curStatusPtr->curValue.newVar.state = 0;
      } break;
    ...
    case VARTYPE_PARAM_MULT:
      ...
      case TOK_END_OF_INST: {
        ...
        do {
          ...
          char **param =
              resolveParam(&curStatusPtr->curValue.newVar, vertex, 2);
          ...
          for (char **parm = param; *parm; ++parm) {
            ARBCONV_DBG_HEAVY(printf("Resolved param %p: ", *parm);
  ...
  case ST_ALIASING:
    ...
    case TOK_IDENTIFIER: {
      ...
      int ret;
      varIdx = kh_put(variables, curStatusPtr->varsMap,
                      curStatusPtr->curValue.string, &ret);
      if (ret < 0) {
        FAIL("Unknown error");
      }
...
#define STATE_AFTER_SWIZZLE -1
    ...
    sInstruction_Vars *curVarPtr =
        &curStatusPtr->curValue.newInst.inst
             .vars[curStatusPtr->curValue.newInst.curArg];
    ...
    case TOK_NEWLINE:
      ...
      case STATE_AFTER_VALID_LSQBR_SIGN: {
        char *faa = (char *)realloc(curVarPtr->floatArrAddr,
                                    (strlen(curVarPtr->floatArrAddr) +
                                     getTokenLength(curStatusPtr) + 1) *
                                        sizeof(char));
        if (!faa) {
      ...
      case STATE_RBRACE:       // directly to the resolveParam part...
        if (INSTTEX(curStatusPtr->curValue.newInst.inst.type) &&
          ...
          if ((vertex &&
              ...
              (!vertex &&
               !strcmp(curStatusPtr->_fixedNewVar.strParts[0], "fragment"))) {
            ...
            char **resolved =
                resolveParam(&curStatusPtr->_fixedNewVar, vertex, 1);
