...
void APIENTRY_GL4ES gl4es_glPointParameteriv(GLenum pname,
                                             const GLint *params) {
  GLfloat tmp[3];
...
void APIENTRY_GL4ES gl4es_glPointParameterfv(GLenum pname,
  ...
  case GL_POINT_DISTANCE_ATTENUATION:
    ...
    if (memcmp(glstate->pointsprite.distance, params, 3 * sizeof(GLfloat)) ==
        0) {
      noerrorShim();
      return;
    }
...
AliasExport(void, glPointParameterfv, ARB,
            (GLenum pname, const GLfloat *params));
AliasExport(void, glPointParameterfv, EXT,
            (GLenum pname, const GLfloat *params));
