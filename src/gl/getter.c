...
void APIENTRY_GL4ES gl4es_glGetPointerv(GLenum pname, GLvoid **params) {
  ...
  case GL_TEXTURE_COORD_ARRAY_POINTER:
    *params = (void *)glstate->vao
                  ->vertexattrib[ATT_MULTITEXCOORD0 + glstate->texture.client]
                  .pointer;
    break;
...
void BuildExtensionsList() {
  if (!glstate->extensions) {
    ...
    strcpy(extensions,
           "GL_EXT_abgr "
#ifdef AMIGAOS4
           "GL_MGL_packed_pixels " // same as GL_EXT_packed_pixels, but older,
                                   // some old Amiga games may check for this
#endif
           "GL_EXT_packed_pixels "
           "GL_EXT_compiled_vertex_array "
           "GL_EXT_compiled_vertex_arrays " // yes, at least on AmigaOS there
                                            // are progs which check for this
                                            // wrong string; reason is an old
                                            // typo in the MiniGL driver.
    ...
    glstate->extensions_list =
        (GLubyte **)calloc(glstate->num_extensions, sizeof(GLubyte *));
    p = extensions;
...
int gl4es_commonGet(GLenum pname, GLfloat *params) {
  ...
  case GL_MODELVIEW_STACK_DEPTH:
    *params =
        (glstate->modelview_matrix) ? (glstate->modelview_matrix->top + 1) : 1;
    break;
  case GL_PROJECTION_STACK_DEPTH:
    *params = (glstate->projection_matrix)
                  ? (glstate->projection_matrix->top + 1)
                  : 1;
    break;
  case GL_TEXTURE_STACK_DEPTH:
    *params = (glstate->texture_matrix)
                  ? (glstate->texture_matrix[glstate->texture.active]->top + 1)
                  : 1;
    break;
  ...
  case GL_TEXTURE_BINDING_1D:
    *params =
        glstate->texture.bound[glstate->texture.active][ENABLED_TEX1D]
            ? glstate->texture.bound[glstate->texture.active][ENABLED_TEX1D]
                  ->glname
            : 0;
    break;
  ...
  case GL_TEXTURE_BINDING_3D:
    *params =
        glstate->texture.bound[glstate->texture.active][ENABLED_TEX3D]
            ? glstate->texture.bound[glstate->texture.active][ENABLED_TEX3D]
                  ->glname
            : 0;
    break;
  case GL_TEXTURE_BINDING_RECTANGLE_ARB:
    *params =
        glstate->texture
                .bound[glstate->texture.active][ENABLED_TEXTURE_RECTANGLE]
            ? glstate->texture
                  .bound[glstate->texture.active][ENABLED_TEXTURE_RECTANGLE]
                  ->glname
            : 0;
  ...
  case GL_TEXTURE_BINDING_CUBE_MAP:
    *params =
        glstate->texture.bound[glstate->texture.active][ENABLED_CUBE_MAP]
            ? glstate->texture.bound[glstate->texture.active][ENABLED_CUBE_MAP]
                  ->glname
            : 0;
    break;
...
void APIENTRY_GL4ES gl4es_glGetFloatv(GLenum pname, GLfloat *params) {
  ...
  case GL_TEXTURE_MATRIX:
    memcpy(params, TOP(texture_matrix[glstate->texture.active]),
           16 * sizeof(GLfloat));
    break;
  case GL_LIGHT_MODEL_AMBIENT:
  ...
  case GL_CURRENT_NORMAL:
    ...
    break;
  case GL_CURRENT_TEXTURE_COORDS:
    memcpy(params, glstate->texcoord[glstate->texture.active],
           4 * sizeof(GLfloat));
    break;
  case GL_COLOR_WRITEMASK:
...
void APIENTRY_GL4ES gl4es_glGetDoublev(GLenum pname, GLdouble *params) {
  ...
  case GL_MODELVIEW_MATRIX:
    ...
    break;
  case GL_TEXTURE_MATRIX:
    memcpy(tmp, TOP(texture_matrix[glstate->texture.active]),
           16 * sizeof(GLfloat));
    for (int i = 0; i < 16; i++)
      params[i] = tmp[i];
    break;
  ...
  case GL_CURRENT_NORMAL:
    ...
    break;
  case GL_CURRENT_TEXTURE_COORDS:
    memcpy(tmp, glstate->texcoord[glstate->texture.active],
           4 * sizeof(GLfloat));
    for (int i = 0; i < 4; i++)
      params[i] = tmp[i];
    break;
...
void APIENTRY_GL4ES gl4es_glGetLightfv(GLenum light, GLenum pname,
                                       GLfloat *params) {
  DBG(printf("glGetLightfv(%s, %s, %p)\n", PrintEnum(light), PrintEnum(pname),
             params);)
  ...
  case GL_SPOT_DIRECTION:
    memcpy(params, glstate->light.lights[nl].spotDirection,
           3 * sizeof(GLfloat));
    break;
...
AliasExport(void, glGetMinmaxParameteriv, ,
            (GLenum target, GLenum pname, GLint *params));

void gl4es_glGetMinmaxParameterfv(GLenum target, GLenum pname,
                                  GLfloat *params) {
  DBG(printf("unsupported glGetMinmaxParameterfv(%s, %s, %p)\n",
             PrintEnum(target), PrintEnum(pname), params);)
  errorShim(GL_INVALID_VALUE);
}
AliasExport(void, glGetMinmaxParameterfv, ,
