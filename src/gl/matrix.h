...
static inline GLfloat *getTexMat(int tmu) {
  return glstate->texture_matrix[tmu]->stack +
         glstate->texture_matrix[tmu]->top * 16;
}
...
static inline GLfloat *getInvMVMat() {
  if (glstate->inv_mv_matrix_dirty) {
    matrix_inverse(glstate->modelview_matrix->stack +
                       glstate->modelview_matrix->top * 16,
                   glstate->inv_mv_matrix);
...
static inline GLfloat *getNormalMat() {
  if (glstate->normal_matrix_dirty) {
    matrix_inverse3_transpose(glstate->modelview_matrix->stack +
                                  glstate->modelview_matrix->top * 16,
...
static inline GLfloat *getPMat() {
  return glstate->projection_matrix->stack +
         glstate->projection_matrix->top * 16;
}
